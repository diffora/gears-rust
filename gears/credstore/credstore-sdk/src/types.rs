...
//! gateway enforces uniformly — most importantly which [`SharingMode`]s the
...
///
/// All fields are enforced by the gateway on write except
...
pub struct SecretTypeDescriptor {
    ...
    pub name: &'static str,
    /// Full derived GTS type id (stored in the gateway metadata table).
...
impl SecretTypeDescriptor {
    ...
    /// view the gateway resolves from the types-registry for a registered
    ...
    #[must_use]
    pub fn traits(&self) -> crate::gts::SecretTypeTraits {
        crate::gts::SecretTypeTraits {
...
pub const SECRET_TYPE_CATALOG: &[SecretTypeDescriptor] = &[
    SecretTypeDescriptor {
        name: "generic",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.generic.v1~",
        allow_sharing: ALL,
        value_schema: None,
        max_size_bytes: None,
        expirable: false,
    ...
    SecretTypeDescriptor {
        name: "api-key",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.api_key.v1~",
        allow_sharing: ALL,
        value_schema: None,
    ...
    SecretTypeDescriptor {
        name: "personal-token",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.personal_token.v1~",
    ...
    SecretTypeDescriptor {
        name: "oauth2-client",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.oauth2_client.v1~",
    ...
    SecretTypeDescriptor {
        name: "basic-auth",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.basic_auth.v1~",
        allow_sharing: ALL,
    ...
    SecretTypeDescriptor {
        name: "bearer-token",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.bearer_token.v1~",
    ...
    SecretTypeDescriptor {
        name: "certificate",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.certificate.v1~",
        allow_sharing: TENANT_SHARED,
        value_schema: None,
    ...
    SecretTypeDescriptor {
        name: "ssh-key",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.ssh_key.v1~",
        allow_sharing: PRIVATE_TENANT,
        value_schema: None,
        max_size_bytes: Some(64 * 1024),
        expirable: false,
    ...
    SecretTypeDescriptor {
        name: "webhook-hmac",
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.webhook_hmac.v1~",
        allow_sharing: TENANT_SHARED,
        value_schema: None,
        max_size_bytes: Some(8 * 1024),
        expirable: false,
    ...
    SecretTypeDescriptor {
        ...
        gts_id: "gts.cf.core.credstore.secret.v1~cf.core.credstore.connection_string.v1~",
...
pub fn type_uuid(gts_id: &str) -> Option<Uuid> {
    gts::GtsID::new(gts_id).ok().map(|parsed| parsed.to_uuid())
}
...
/// Client-supplied reference to a secret type, resolved to the type's
/// deterministic v5 UUID (the stored representation, and the key the
/// gateway resolves against the types-registry).
///
/// Accepted spellings ([`Self::parse`]):
/// * a built-in catalog short name — `"api-key"`;
/// * a full GTS type id (contains `~`) — covers dynamically registered
///   custom types the compiled-in catalog knows nothing about;
/// * the type UUID itself.
///
/// Parsing only validates the *shape*; whether the type actually exists is
/// decided by the gateway against the types-registry (except catalog short
/// names, which resolve locally).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct SecretTypeRef(Uuid);

impl SecretTypeRef {
    /// Parse a client-supplied type reference (see the type-level docs).
    ///
    /// # Errors
    ///
    /// Returns [`CredStoreError::TypeViolation`] with reason
    /// `UNKNOWN_SECRET_TYPE` when the input is neither a valid GTS type id,
    /// nor a UUID, nor a known catalog short name.
    pub fn parse(input: &str) -> Result<Self, CredStoreError> {
        if input.contains('~') {
            return type_uuid(input)
                .map(Self)
                .ok_or_else(|| CredStoreError::TypeViolation {
                    reason: "UNKNOWN_SECRET_TYPE".to_owned(),
                    detail: format!("not a valid GTS type id: {input}"),
                });
        }
        if let Ok(uuid) = Uuid::parse_str(input) {
            return Ok(Self(uuid));
        }
        SecretType::from_name(input).map(Self::from)
    }

    /// Wrap an already-known deterministic type UUID.
    #[must_use]
    pub const fn from_uuid(uuid: Uuid) -> Self {
        Self(uuid)
    }

    /// The deterministic v5 UUID this reference resolves to.
    #[must_use]
...
impl From<SecretType> for SecretTypeRef {
    fn from(t: SecretType) -> Self {
        Self(t.uuid())
    }
}

impl From<Uuid> for SecretTypeRef {
    fn from(uuid: Uuid) -> Self {
        Self(uuid)
    }
}

impl fmt::Display for SecretTypeRef {
