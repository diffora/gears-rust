...
//! gateway performs a dual write (backend value + DB metadata) with no
...
//! Split knowledge: fingerprints live in the gateway DB, the key lives with
...
//! The fingerprint itself never leaves the gateway (no API field, header,
//! or log line).
