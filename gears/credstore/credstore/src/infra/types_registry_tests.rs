...
/// The credstore secret base type as registered by the SDK: carries the
/// traits schema and the generic trait *values* (`validate_entity_traits`
/// requires the base of a traits-schema chain to carry values), so every
/// descendant inherits complete generic defaults.
...
const CUSTOM_TYPE_ID: &str =
    "gts.cf.core.credstore.secret.v1~acme.connectors.creds.db_password.v1~";
...
#[tokio::test]
async fn resolves_the_envelope_itself_with_generic_traits() {
    ...
    let resolved = resolver(registry)
        ...
        .await
        .expect("base type resolves");

    assert_eq!(resolved.gts_id, SECRET_RESOURCE_TYPE);
    for mode in [
        SharingMode::Private,
        SharingMode::Tenant,
        SharingMode::Shared,
    ] {
        assert!(resolved.traits.allows_sharing(mode));
    }
    assert!(!resolved.traits.expirable);
    assert_eq!(resolved.traits.max_size_bytes, None);
    assert_eq!(resolved.traits.value_schema, None);
}
...
async fn non_secret_type_is_an_unknown_secret_type_violation() {
    ...
    let alien_id = "gts.acme.core.events.type.v1~acme.commerce.orders.order.v1~";
