...
#[tokio::test]
async fn post_accepts_full_gts_id_and_uuid_type_spellings() {
    // The `type` field accepts a full GTS type id and the type's
    // deterministic UUID, not just the catalog short name.
    let api_key = SecretType::from_name("api-key").expect("known");
    for (reference, type_spelling) in [
        ("byid", api_key.gts_id().to_owned()),
        ("byuuid", api_key.uuid().to_string()),
    ] {
        let h = build_harness();
        let req = json_request(
            "POST",
        ...
        assert_eq!(resp.status(), StatusCode::CREATED, "{type_spelling}");

        let get = json_request(
            "GET",
            &format!("/credstore/v1/secrets/{reference}"),
            None,
            test_ctx(),
        );
        ...
        // Built-in types keep the compact wire label on responses.
        assert_eq!(body["metadata"]["type"], "api-key");
    }
}
...
async fn post_unknown_custom_type_returns_400_unknown_secret_type() {
    ...
    let req = json_request(
        ...
        Some(serde_json::json!({
            ...
            "value": "v",
            "sharing": "tenant",
            "type": "gts.cf.core.credstore.secret.v1~acme.connectors.creds.db_password.v1~"
        })),
        test_ctx(),
    );
    let resp = h.router.oneshot(req).await.expect("router");
