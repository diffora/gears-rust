...
use credstore_sdk::{SecretRef, SecretTypeRef, SecretValue, SharingMode, WriteOptions};
...
/// into [`WriteOptions`]: a type reference (catalog short name, full GTS
/// type id, or type UUID — see [`SecretTypeRef::parse`]) and an RFC 3339
/// expiry. Unparsable type references and malformed timestamps are typed
/// 400s; whether a well-formed custom type actually exists is decided by
/// the service against the types-registry.
fn parse_write_options(
    ...
    let secret_type = secret_type
        .map(|input| {
            SecretTypeRef::parse(input).map_err(|_| DomainError::TypeViolation {
                field: "type",
                reason: crate::domain::secret::typing::reasons::UNKNOWN_SECRET_TYPE,
                detail: format!("unknown secret type: {input}"),
            })
        })
        .transpose()?;
