...
| **billingAnchorPolicy** | Pricing-published per-row policy fixing how the recurring billing anchor is derived (the pricing K2 enum incl. the no-drift month-end clamp — a Jan-31 monthly anchor bills Feb-28/29, never drifting to the 28th permanently). **Adoption parked on SEAMS SUB-P9** (2026-08-01, wave-3 review #20): this gear executes the anchor math (pricing D-20) and the K5 joint anchor fixture must exist before code, but the clamp's normative landing here is gated on the SUB-B1/SB1 recurring-WHEN resolution; until then `billingAnchor` derivation rides Contract terms (design slice 01 §3.7). |
