...
- **Collection boundary:** the payer rebind defaults to **next-cycle** — the in-flight billing period stays with the old payer (the recurring fact for it is already keyed **and snapshots `payerTenantId` at cut time**, slice 08 §4.3 — Billing posts to the fact's frozen payer, so the default is enforced by the fact itself, not by posting-time re-resolution); an immediate rebind requires the Billing-side treatment of the mid-period split, which is an open Product/Billing question ([`../PRD.md`](../PRD.md) §15). Cross-currency/region transfers are rejected toward **cancel+new** (slice 02 §4.2, slice 03 §4.3).
