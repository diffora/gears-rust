...
| `renew` (manual) | `active`; `suspended` **only** on the payment-resolution path (slice 04 §4.3a backdated-term rule) | never from grace-exhausted terminal states |
| `convertTrial` | `active` in the trial phase | if `suspended` at the scheduled boundary, the conversion **parks retryable** (slice 01 §4.3 taxonomy) until resume or a terminal state — frozen entitlements stay frozen, no paid issuance while suspended (slice 06 §4.2) |
...
| `resumeCollection` | `active` with an **open** `collectionPaused` window | also fired system-side at window end (slice 04 §4.2a) |
...
- **Terminal sweep (2026-07-28 review fix):** the commit of any transition into a **terminal** state (`cancelled`, `archived`) atomically voids **every** pending `ScheduledIntent` of the aggregate — each emits `SubscriptionIntentUnscheduled` (reason = `superseded_terminal`) in the same outbox batch — so no pending resume/ramp step lingers in read models or fires weeks later into `guard_violation`.
...
- **Retryable** — `oss_unconfirmed` (confirmation timeout) or Policy **unavailable** (the gate could
  ...
  §15). All of the intent's suppressions (renewal + next-term recurring for a pending cancel/opt-out)
