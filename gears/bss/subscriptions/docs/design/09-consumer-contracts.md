...
- The business operations (create/get/list; activate/suspend/resume/cancel incl. void-from-draft; changePlan; add-on; `updateQuantity`; `convertTrial`/`extendTrial`; manual `renew`; `unschedule`; `pauseCollection`/`resumeCollection`; `confirmAcceptance`; `transfer`; `archive` — the `cancelled → archived` retention edge; entitlement issue/revoke — the full PRD §9.1 table incl. the SUB-D-08 set) carry idempotency keys + optimistic concurrency on `version`; resources are `Subscription`, `Entitlement`, `AddOn`, `PlanLink`, `TransitionRequest`, `Approval` ([`../PRD.md`](../PRD.md) §9.1). REST paths/methods/header bindings/OpenAPI are Design.
