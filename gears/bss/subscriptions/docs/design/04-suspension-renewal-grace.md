...
per SUB-D-12 (§4.2) → otherwise payment pre-check → **success**: extend term + fresh snapshot refs
(keyed against double extension); **failure**: `GraceLadder` starts (7-day default), the blocked
...
- Attempts are **keyed to prevent double term extension**: the term-extension effect is idempotent on `(subscriptionId, currentTermSequence)` — the monotonic index of the term being renewed — derived, not client-supplied, so a crashed-and-retried `RenewalJob` firing (or a duplicate manual `renew`) resolves to the **already-extended** term instead of extending twice (the same derive-the-key discipline as the scheduled-intent firing, slice 01 §3.6). **Every** successful renewal — auto and manual alike — creates a new term window with **fresh pricing-side snapshot refs** (2026-07-28 review fix, SUB-D-14: §3.6 and the DESIGN.md sequence always said "fresh snapshot refs" on the auto path; the "manual-only" attribution here and in the PRD FR was the drift). Commercially this is not "auto-renewal at a surprise new price": re-binding at renewal is exactly pricing's grandfathering re-resolution mechanism — the refreshed ref resolves the subscription's **eligible** row, so a protected cohort re-binds to its own generation and only deliberately superseded pricing moves. The `(currency, region)` segment persists (slice 02 §4.2).
