...
- **`BillableItemDelivery`** — the outbound envelope: `RatedCharge` / `Adjustment` + obligations + sealed `pricingSnapshotRef` + `{skuId, planId, priceId}` + rounding-policy id + the delivery idempotency key (usage/correction key).
