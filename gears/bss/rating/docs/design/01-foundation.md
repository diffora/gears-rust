...
| `cpt-cf-bss-rating-fr-non-negative-price` | The `EmissionGuard` clamps/credits a would-be-negative resolved line after steps 4–7 and **before** period-level floor/cap (§4.4); clamp-vs-credit policy is a §15 open. |
...
- **`EvaluationUnit`** — what determinism quantifies over, three kinds (§4.2): a single normalized `UsageRecord` (`per_event`); the window-aggregated `Q` for `(subscription, meter, dimensionKey, window)` — per sub-window slice when the window is split; a **period-driven unit** (recurring lines, capacity-flavor charges, true-up surfacing) keyed `(subscription, priceId, chargeKind, lineKey, AnchorPeriod)`.
...
1. **Non-negative**: after steps 4–7 and before any period-level phase, a would-be-negative line is clamped to zero or emitted as a structured credit (clamp-vs-credit policy: PRD §15 open); a floor never masks a negative line.
