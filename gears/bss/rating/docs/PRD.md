...
| **dimensionKey** | Ordered tuple of pricing-relevant event dimensions that, with `meter`, identifies one charge line. Empty tuple = a meter with no declared dimensions. Invariant: one line per **`(skuId, dimensionKey)`**. Declared dimensions come from the published Plan/SKU revision and are frozen in `pricingSnapshotRef`. Value emission on usage is an OSS/Rating contract. |
...
| **PriceWindow** | Non-overlapping, UTC-bounded interval during which a `Price` row is effective. Step-2 selection is on the pricing **canonical ten-axis scope key** — `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` plus the usage pair `(skuId, dimensionKey)` (pricing design/01 §4.1, D-196 2026-08-06; the rendered absent-axis token `none` on non-usage rows; adopted **verbatim**). `chargeKind` (hybrid recurring vs usage), `priceEligibility`, and `cohort` (grandfathering generation) are additive axes, so multiple rows legitimately coexist at one `(planId, currency, region, phase)`; the non-overlap invariant and 'at most one match' hold only on the **full** key. |
...
> **Manifest extension (PriceWindow coverage)**: manifest §4.1 guarantees non-overlapping windows for a key; this PRD requires that key to be the pricing **ten-axis canonical scope key** — `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` plus the usage pair `(skuId, dimensionKey)` (pricing design/01 §4.1, D-196) — and additionally requires **no gaps** for billable usage at `t` (if no window matches, evaluation MUST fail explicitly — AC 6).
...
| Dimensional pricing — `(skuId, dimensionKey)` lines | `p1` | Critical path for a real IaaS catalog; step 3 + AC 3 + AC 18. Depends on the usage dimension-population contract. |
...
- **`one_time` charge billing** (T-D-18, amended T-D-36 2026-09-20) — not rated: no evaluation unit is synthesized for this `chargeKind` (`one_time_setup` is withdrawn; there are exactly three kinds); Subscriptions/Billing bill it at each **actual phase entry** from the frozen `pricingSnapshotRef` amount, with the **per-phase-entry-occurrence** dedup owned there (the at-sale path of the T-D-14 commitment sale). One-time rows still resolve in step-2 selection for coverage/preview/quote.
...
**Protocol/Format**: the frozen read-model consumer contract (pricing `design/06`): the canonical ten-axis scope key (incl. the `(skuId, dimensionKey)` usage pair, D-196), `modelKind` → formula mapping (pricing §17.2), tier bands, `priceEligibility` + `cohort` (generation), the `prorationBasis` and `billingAnchorPolicy` enums adopted **verbatim**, the prepaid-grant set, and `{skuId, planId, priceId}` rating-compatibility; PriceWindow state via `PriceWindow*` events (incl. `PriceWindowCancelled`); bundle `sum_of_parts` component sets with **normalized effective rev-shares** (`effective_share_bp`) (Design).
...
- **Given** a plan/SKU publish where a single `(skuId, dimensionKey)` maps to more than one charge line
...
**6. Effective windows**
- **Given** only non-overlapping `PriceWindow` rows for the ten-axis canonical scope key — `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` plus the usage pair `(skuId, dimensionKey)` (D-196)
...
- **Given** twelve fragmented 5-minute records for one continuous hour of the same `(skuId, dimensionKey)`
- **When** evaluation computes chargeable quantity
...
- **Then** each distinct `(skuId, dimensionKey)` MUST resolve to its own charge line and price, with no line collision
...
| (Product + OSS/Policy) Free-tier level: per-meter $0 band vs per-account-per-service allowance; boundary behavior and enforcing domain | Product + OSS/Policy | TBD | Current Scope = per-`(skuId, dimensionKey)` $0 band; cross-account allowance is a new aggregate (Follow-on). | — |
...
| Free tier as structural concept | `p2` | Follow-on | Current Scope expresses free only as a per-`(skuId, dimensionKey)` $0 band; cross-account allowance is a new aggregate |
...
| **Capability** | **Priority** | **Status** | **Notes** |
|----------------|--------------|------------|-----------|
| Bilateral pricing (source x destination) | `p2` | Follow-on | `(source, destination)` as `dimensionKey`; consistent with the injective `(skuId, dimensionKey)` rule |
