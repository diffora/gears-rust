...
- **Recurring period-cut ownership (2026-07-28)** — SB1 (§K): subscriptions' `RecurringEmitter` fact per `(subscriptionId, billing period, lineKey)` (SUB-D-07 + SUB-D-19, flagged for veto) vs rating's `PeriodTick` per `(subscription, AnchorPeriod)` (T-D-15) — resolve ownership + joint fixture before `billingAnchorPolicy` is implemented in either gear.
