...
//! [`format_rfc3339_offset`] call `OffsetDateTime::to_offset(UTC)` — a panicking
...
///
/// That call was `to_rfc3339_opts(SecondsFormat::AutoSi, false)`: no fraction on
/// a whole second, else three, six or nine digits — whichever first divides the
/// nanoseconds evenly — and a `+00:00` designator. It is what the inquiry CSV
/// export and the dual-control policy snapshot rendered before the type change,
/// and both are read by consumers as data, so they keep those bytes. This gear
/// applies no quantum, so its instants are microsecond ones and any floor would
/// discard a digit the reader cannot recover. `time`'s `Rfc3339` does not
/// reproduce the form either: it prints minimal digits (`.1`, not `.100`) and
/// `Z`.
#[must_use]
