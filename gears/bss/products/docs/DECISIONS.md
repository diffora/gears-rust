...
- **What is deliberately absent from the filter vocabulary**: `tenant_id` (the `AccessScope`'s, never the caller's), `name_normalized` (the Foundation's operand, computed application-side — a caller comparing it would be comparing a normalization it cannot see), `created_at`/`updated_at` (no declared use), and `mutation_seq` (answered per row, not a surface anyone narrows by). `parent_id` is filterable and **not** orderable: it is nullable, and `read_models::BrowseODataMapper`'s own note gives the reason a nullable column cannot be a keyset order key — the predicate `col > :value` is NULL, hence false, for every root.
- **Owed**: whether the browse door should spend `category × read` when `includeFacets=true` — today a principal holding `product × read` alone reads category **names** off the facet half. The recommendation is to leave browse as it is (a facet is a count over rows the caller may already read, and demanding a second grant would `403` every existing facet caller) and let the narrowed row 234 carry the truth; the owner's call either way. Also owed: a single-node `GET /bss-products/v1/categories/{categoryId}`, which no consumer has asked for.
