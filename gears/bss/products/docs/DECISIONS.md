...
- **Decision**: at **effective quorum zero** a governed act is authorized with `ApprovalDisposition::NoRecord`. No `products_approval` row is written, `entity_version.approval_ref` stays `NULL`, and a publish is **one call**. The gate still matches a `satisfied` record **first**, so the submission door is unchanged and an act that was submitted still spends its record under `inst-gv-one-shot`.
...
- **A `superseded` record is left standing at zero.** The `satisfied` match already excludes superseded rows (`inst-gv-supersede`), so after a frozen-content edit the author publishes under `NoRecord` and the superseded row is orphaned rather than consumed. Stated so the orphan is not read as a leak: at zero there is no approver to be protected from stale content, and the row remains the record that a submission was once made.
