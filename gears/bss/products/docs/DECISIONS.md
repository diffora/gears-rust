...
#### P-D-183 — The usage-type catalog is a port a deployment can fill, it has a pick-list, and a mistyped ref stops reaching publish unseen

- **Date**: 2026-09-22 (owner call, taken while reading the meter-declaration surface against pricing's tax-category port). **Additive on the wire**: one new read, and one new refusal on a save that was previously accepted and would have been refused at publish anyway. **Amends P-D-05 rather than replacing it** — resolvability-only is untouched; what moves is *when* the question is asked and what a non-answer costs at each moment. Consumes **P-D-131** (the fail-closed posture), **P-D-141** (the resolver on `ApiState`), **P-D-90** (the `recognized_set` grant).
- **Problem, measured on the benidorm stand 2026-09-22.** A `MeterDeclaration` is the pair `(unit, usageTypeRef)`. The unit half has a surface — `GET /config/vocabularies/metering_unit` lists a governed local vocabulary — and the ref half had **none**: in the whole deployed contract the only usage-type routes were the collector's own. So an authoring screen reached into another gear or an operator typed a GTS id by hand. And the id was judged **only at publish**: a `PATCH` naming a ref no catalog knows answered **200**, while a `PATCH` naming an unrecognised *unit* answered **400 `UNRECOGNIZED_UNIT`** immediately. `POST …/validate`, the dry-run door, did not resolve it either — `lint_sku_publish` judges the unit — so the one door whose whole job is *"tell me what publish will say"* did not say this.
- **And the seam was bound to the wrong trait.** `gear.rs` took `dyn UsageCollectorClientV1` from `ClientHub`, so a deployment whose usage types come from something other than that collector had to implement the whole of it — records, batch writes, aggregation, deactivation, deletion — to supply a catalog of type ids.
- **Decision**, four clauses.

  **(1) A narrow port, in the SDK.** `bss_products_sdk::usage_types::UsageTypeCatalog` carries `resolve` and `list`, with `UsageTypeAnswer`, `UsageTypeBinding` and `UsageTypePage` beside it; the in-crate `UsageTypeResolver` is replaced, not kept beside it. **Both methods on one trait deliberately**: a supplier that fills the pick-list also answers the publish gate, and splitting them lets a deployment list from one catalog while gating against another — a disagreement that reaches an operator as *"I picked it from your list and publish says it does not exist"*.

  **(2) Four steps, and the provenance decided once.** A registered `dyn UsageTypeCatalog` wins (`source: registry`); else the adapter over a registered `UsageCollectorClientV1` (`usage_collector`); else the config mode — a fabricated `local_dev_static` set behind a name long enough that it cannot be selected by accident, or `unconfigured`. **A registered client always wins over a config mode**, and `source` travels with the `Arc` rather than being re-derived at a call site: both are pricing's `resolve_product_catalog` arguments, adopted whole.

  **(3) The pick-list.** `GET /bss-products/v1/catalog/usage-types?q=&kind=&limit=&cursor=`, on **`recognized_set × read`** — the grant the declaration's *other* half already spends, so no authz pair is minted here; minting one is a joint decision this slice may not take alone. `q` narrows by substring of the id (the distinguishing part of a usage type's GTS path is at its **end**, so a prefix match would find everything or nothing) and `kind` by equality, both onto the collector SDK's existing OData filter — **nothing is added to the collector**. `limit` is refused rather than clamped. **Failure is never an empty success**: 501 with no catalog, 503 when a configured one does not answer, 200 with `items: []` when one is configured and empty, and `source` read before `items` to tell them apart.

  **(4) The ref is judged at three moments, each with its own posture.** At **save**, when the ref *changed* in this save and a catalog is configured: a definitive `Unresolved` is `400 USAGE_TYPE_UNRESOLVED`, a non-answer lets the save through. At **validate**, always, as a finding that refuses nothing — two findings, keeping the catalog's *no* apart from its *silence*. At **publish**, unchanged: fail-closed on both.

- **Why save refuses a no but tolerates a silence, which is the clause worth the most.** The asymmetry this entry narrows has a real cause and the fix keeps it: the unit is a **local table read** and the ref is a **cross-gear call**. A catalog that *says* the id is unknown is a local-quality fact and gets the unit's treatment. A catalog that could not be reached has said nothing, and blocking a draft save on it is the outage coupling this gear deliberately avoids — the resolver's own module calls the publish-path version *"a latency coupling, not a lock"*. The resolve runs **before** the transaction, for the reason the publish path already does: a 503 must leave no idempotency claim behind. And only a **changed** ref is asked about, so an autosave touching anything else spends no call.
- **Not changed**: P-D-05's resolvability-only scope — no lifecycle check, no dimension check; the publish gate's behaviour in every case; the meter pair's bucket ii membership and the correction door that owns it; `inst-mt-recognized` and the unit's own vocabulary; the collector's API.
- **What this costs, named.** The SDK's public surface grows by one trait, three types and two error constructors. A deployment now has two places a catalog can come from, and a third-party supplier gains influence over publication and not only over the list — the deliberate consequence of clause (1), because the alternative is two catalogs that can disagree. And `validate` now makes a cross-gear call it did not make before; it is the only lint caller that does, the bulk worker's per-row lint and the approvals door being left alone on purpose.
- **Propagated**: `design/03-sku-classification.md` (`inst-mt-resolve`'s three moments, §1.7's port name, §1.8's consumed list); `features/sku-classification.md` (the acceptance scenarios). **Slice 01 is deliberately not amended**, measured: `inst-fd-pipeline` runs *"every registered validator for `(kind, field set)`"* and enumerates none by name, so the save-time check is one more registered validator and the Foundation's text already covers it.

#### P-D-182 — The taxonomy carries one default category, seeded `General`, and the create door assigns it

- **Date**: 2026-09-22 (owner call, taken after two alternatives were costed and rejected: a `require_primary_category_at_publish` configuration knob, and an "Uncategorized" bucket)
- **Decision**: `products_category` gains `is_default boolean NOT NULL DEFAULT false` under a partial unique index `UNIQUE (tenant_id) WHERE is_default`; the tenant's first Product create seeds a root category `General` carrying the flag when the tenant has none; `POST /bss-products/v1/categories/{categoryId}/operations` gains a fifth op, `set_default`; and `POST /bss-products/v1/products` gains an optional `primary_category_id`, resolving the default when the caller names none. **`inst-tx-primary-at-publish` is not relaxed** — it stops firing through the **single-create** path because that path leaves no Product uncategorized, which is exactly the shape **P-D-179** recorded for `plan_tier`: *"optional on the wire, always present in the row … the publish arm cannot fire through a door"*. The rule keeps guarding the row against a head written past the doors, and against a caller who clears the set with `"categories": []`.
- **Why a column and not configuration.** A deployment knob would be invisible to the surface that has to render it: a picker must be able to preselect the default, and the read door (**P-D-181**) serves rows. A knob is also single-person editable, where a category is operator data under `category × write`.
- **Why "at most one" is an index.** `uq_products_product_category_primary ON (tenant_id, product_id) WHERE role = 'primary'` is the same statement one table over, and this gear's rule is that at-most-one is a partial index and never a convention (`entity/product_category.rs:26`). Both engines already carry partial indexes on that very migration, so nothing is asked of `SQLite` that it does not already do here.
- **Why the default does not pollute history.** A frozen version renders its assignments as `{"categoryId": …, "role": …}` — **ids, never names** (`domain::taxonomy::assignment_collection`). Renaming `General`, or moving the flag to another category, rewrites no version and invalidates no `CatalogVersion` manifest. This is what separates the decision from the "Uncategorized" bucket that was rejected: the stored fact is *which node*, and the node's display name stays the operator's to change.
- **Why the seed is lazy and not a migration.** The gear has no tenant-bootstrap hook, and §4.2's well-known seeds are already written this way: `well_known_roster` reads the roster, seeds when it is empty and re-reads, *"once per tenant, ever"*. The seed therefore runs at the create door, **before** the Product transaction opens and under the per-tenant taxonomy writer lock every tree write takes, so a concurrent pair is serialized by the lock and decided by the index exactly as a name race is (`classify_category_write`).
- **Four shape decisions.**
  - **`General`, a root, `active`.** A real classification an operator can rename, re-parent or replace — not an absence spelled as a value, which is what this gear refuses elsewhere (**P-D-39**'s `""` for unrestricted, **P-D-88**'s `NULL` parent for a root rather than a sentinel node).
  - **A create naming `primary_category_id` never consults the default.** The caller's answer is the answer; the default exists for silence, not for override.
  - **A create in a tenant with no default writes no assignment and succeeds.** Create must not fail on a taxonomy detail, and the publish rule still guards the entity. The alternative — refusing the create — would let a retire take the create door down with it.
  - **Retiring the flag's holder is refused** (`CATEGORY_REFERENCED`, naming the flag). The flag is moved with `set_default` first, which is what that op is for.
- **`set_default` is material** (owner call, 2026-09-22), riding the same `GovernedLiveOp` envelope, the same gate and the same apply path as the other four acts. Moving the flag re-routes every future uncategorized Product, which is a catalog-wide switch and not a label edit; the door already has one envelope and one apply path, so a sixth act needs no second mechanism. `fr-materiality-gated-publish` enumerated five Category verbs and now enumerates six — the enumeration is extended by this entry rather than read as closed, which is the owner's call to make and is recorded as taken.
- **The act emits no event, and that is decided rather than overlooked.** `inst-tx-event` names five events for five ops, and this sixth one adds none: an event here is a **consumer contract** — a `SCHEMA_REFS` token, a `design/12` §4.2 register row and a broker payload, all of which lint 4 pairs — and no consumer has asked for it; the flag changes no path the browse projection renders, so nothing downstream re-projects on it. The act's trail is the approval record its own gate keeps, materiality being the point of the previous bullet. A `CategoryDefaultSet` event is **owed** below rather than minted against no reader.
- **Three consequences of the placement, each stated rather than found later.** (1) **The bulk-import lane is not covered**: `infra::bulk_worker`'s create calls `infra::create::insert_product_with_event` directly, below the layer the resolver lives in, so a bulk-created Product carries no category and fails publish where the same payload sent one at a time succeeds. Wiring it is **owed** and is a decision of its own — whether an import of 500 rows should silently file 500 Products under the default is not this entry's to assume. (2) **The seed writes a category row under the create door's `product × write` scope**, which is the shape `well_known_roster` already ships: the attribute-definition seeds are written under the same head-act scope, because the act is the **gear's** and not the operator's. Stated because a reader meeting a category write outside `category × write` should find the precedent rather than re-derive it. (3) **A create refused after the seed leaves the seeded node behind** — resolution runs before the create transaction, so a first create that then loses the name race commits `General` and rolls back the Product. Benign and not repaired: the node is one the tenant needs anyway, the seed's condition is the flag so it never runs twice, and moving it inside the transaction would take the taxonomy writer lock inside the create's, which is the ordering hazard the placement exists to avoid.
- **Owed**: the bulk lane above. A `CategoryDefaultSet` event, when a consumer asks — the minting is four paired sites and lint 4 holds them together, which is cheap to do and not free to undo once a token ships. And the pre-existing divergence found while measuring this — `inst-tx-governed-op` calls category **create** material and queues it through the 05 gate, while `api/rest/taxonomy.rs`'s `create_category` writes directly with no approval (measured at `HEAD`). The auto-seed inherits it, but so does every `POST /bss-products/v1/categories` today; it predates this entry, and changing what a shipped door demands belongs in its own entry with its own probe.
- **Propagated**: `design/02-taxonomy-attributes.md` (§2's `inst-tx-default`, §4.1's column and index, §4.2's seed), `design/01-foundation.md` (§3.1's create-door step), `features/taxonomy-attributes.md` (§2's scenarios and `cpt-cf-bss-products-dod-default-category`), `PRD.md` (`fr-materiality-gated-publish`'s Category enumeration).


#### P-D-181 — The category tree becomes readable: one paged door, the `category × read` grant it spends, and row 234's browse claim withdrawn

- **Date**: 2026-09-22 (owner call, taken while seeding categories on the benidorm stand and finding the tree unreadable through every surface)
- **Decision**: one door — `GET /bss-products/v1/categories` — the `category_read` permission instance it spends, and the repository page behind it. **This tree could be written and never enumerated.** Measured the same day against the live stand's `/openapi.json` (66 `bss-products` routes, none of them a category read) and against the code: `repo/taxonomy.rs`' reads were the walk's own parent map (`category_parents`, `category_nodes`) and the validators' `category_states`; `gts/permissions.rs` declared `category_write.v1` and no read twin. A `category_id` therefore existed on the wire exactly once — in the `201` that created it — and an id lost after creation was unrecoverable through the API, while `PATCH /bss-products/v1/products/{id}` requires exactly that id to assign a category.
- **Why the facets were not already the answer, and why row 234 is withdrawn.** `design/05` §3.2 row 234 has said since **P-D-50** that browse *"names `category × read` explicitly"*. Two measurements against `ed180c1da`: the browse handler asks for `product`/`sku × read` and nothing else (`api/rest/read.rs:573-599`), so the grant had **no spender in code**; and a facet bucket is `{value, count}` over the rendered path `"Root > Child"` (`infra/projector.rs:550`), so even where a category appears it is unresolvable to an id, and a category carrying no assignment does not appear at all. P-D-50 closed the "no route declared" contradiction with a route that does not spend the grant and cannot serve a picker. The cell is rewritten here: the facet half spends `product|sku × read`, and this door is the grant's first and only spender.
- **The catalog-version export is not the answer either.** `infra/increment.rs:120` admits seven `capture_kind` values including `category_tree`; `:130-142` builds **three** — `freeze_participant_set`, `metadata_maps`, `reference_producer_set`. The tree capture is declared and unwritten, so no snapshot carries it.
- **Shape decisions, each with the reason it could have gone the other way.**
  - **A flat page of nodes, not a nested tree.** Each row carries `parent_id` and the client assembles. A nested body cannot be keyset-paged — a page boundary inside a subtree either duplicates ancestors or serves orphans — and every other list door on this gear answers `toolkit_odata::PageInfo` (**P-D-165**).
  - **Paged, which is the opposite of P-D-170's call for the vocabularies.** That entry refused paging because a closed vocabulary of tens, read *in order to choose from*, must not hide a choice. The tree is not closed: the shipped ceilings are depth 8 and fan-out 1,000 per node (`config.rs:37,41`), so it is an open-ended surface and P-D-165's contract governs it.
  - **Tombstones are listed, carrying `state: retired`.** Both name-uniqueness indexes are state-agnostic (`m20260901_000018`: neither carries `WHERE state = 'active'`), so a retired sibling still holds its name and the create door still refuses it `DUPLICATE_CATEGORY_NAME`. A list that hid the holder would tell a caller two different things one call apart — P-D-170's tombstone argument on this gear's other roster.
  - **`mutation_seq` rides each row.** It is the live-value door's `If-Match` operand (**P-D-50**) and it had the id's problem: the only surface that answered it was the `PATCH` response. This is not P-D-170's inert-wire case — the value has an asserter, `PATCH /bss-products/v1/categories/{categoryId}/attribute-values`' own `expectedSeq`.
  - **`path` is rendered server-side, by the projection's renderer.** A caller holding one page cannot render an ancestor chain whose nodes are on another page. The door loads the tenant's `(id → parent, name)` map once (`repo::category_nodes`, the projector's own input) and calls the renderer the projector calls, extracted to `domain::taxonomy::render_path` so the two surfaces cannot disagree about a separator.
  - **No `ETag`.** P-D-170's rule kept: no door on this surface accepts a set-level precondition, so a list tag would be a header with no reader.
  - **No SDK type.** P-D-170's reason kept — a type minted against no caller. The consumer is a browser, and `products-sdk` carries in-process contracts.
- **What is deliberately absent from the filter vocabulary**: `tenant_id` (the `AccessScope`'s, never the caller's), `name_normalized` (the Foundation's operand, computed application-side — a caller comparing it would be comparing a normalization it cannot see), `created_at`/`updated_at` (no declared use), and `mutation_seq` (answered per row, not a surface anyone narrows by). `parent_id` is filterable **by value** and **not** orderable: it is nullable, and `read_models::BrowseODataMapper`'s own note gives the reason a nullable column cannot be a keyset order key — the predicate `col > :value` is NULL, hence false, for every root.
- **The roots are not addressable by filter, and that is the platform's limit rather than this door's choice.** Written as `parent_id eq null` and measured on the built door 2026-09-22: the expression parses and then fails the type check — *"Type mismatch for field parent_id: expected Uuid, got null"* — because `toolkit_odata::FieldKind` carries nine scalar variants and no nullable one, so no `Uuid` field on the platform admits a null literal. Two workarounds were available and both are refused: a `roots=true` custom operand puts back the per-door parameter **P-D-165** took away, and declaring the column a `String` compares a uuid as text. What the page gives instead is enough — the tree is served whole and paged, so a caller selects `parent_id == null` over rows it already holds, which is a property of the flat shape this entry chose. The door's own test is kept pointed at the refusal, so the day the platform gains a nullable kind that test is what notices.
- **The `path` rendering costs a whole-tree read per page, and that is a measured trade rather than an oversight.** The door loads `repo::category_nodes` — every node of the tenant — to render one page's ancestor chains, so paging a large tree re-reads it once per page. The two cheaper forms are a recursive-CTE path in the page query and an ancestor-closure read of the page's own rows; both are real and neither is built, because the shipped ceilings bound the tree and no caller has paged one yet. **Owed**, to be decided against a measurement rather than against this sentence. The same read is why a row's `path` can be a beat stale: the page and the map are two statements, and a delete between them renders that row's path as `null` rather than wrongly — `render_path` answers nothing for a chain that does not reach a root.
- **Owed**: a nullable `FieldKind` in `toolkit-odata`, which is the platform's and not this gear's — with it, `parent_id eq null` becomes servable and the paragraph above is withdrawn. Also owed: whether the browse door should spend `category × read` when `includeFacets=true` — today a principal holding `product × read` alone reads category **names** off the facet half. The recommendation is to leave browse as it is (a facet is a count over rows the caller may already read, and demanding a second grant would `403` every existing facet caller) and let the narrowed row 234 carry the truth; the owner's call either way. Also owed: a single-node `GET /bss-products/v1/categories/{categoryId}`, which no consumer has asked for.
- **Propagated**: `design/02-taxonomy-attributes.md` (§2's read flow and `inst-tx-read`), `design/05-governance.md` (§3.2 row 234, rewritten), `features/taxonomy-attributes.md` (§2's flow and `cpt-cf-bss-products-dod-taxonomy-read`), `DESIGN.md` (the route registry).


#### P-D-180 — A quorum-zero act writes no approval record, and the trace it stops leaving is owed to the platform

- **Date**: 2026-09-22 (owner call, taken while seeding usage SKUs on the benidorm stand and meeting the two-call handshake). **Not breaking for a caller that submits first**; it is a behaviour change for one that treats `403 APPROVAL_REQUIRED` at `N = 0` as invariant. Plan-price's counterpart is **D-380**, and the two entries cite each other as P-D-177 and D-380 already do.
- **Decision**: at **effective quorum zero** a governed act **on an `entity_publish` subject** is authorized with `ApprovalDisposition::NoRecord`. No `products_approval` row is written, `entity_version.approval_ref` stays `NULL`, and a publish is **one call**. The gate still matches a `satisfied` record **first**, so the submission door is unchanged and an act that was submitted still spends its record under `inst-gv-one-shot`.
- **What it removes, and what that was buying.** At zero the record carried one fact — *authorized under a zero quorum* — and charged a mandatory second round trip for it. Nothing else on the submission is load-bearing there: `inst-gv-finance-predicate` already does **not** set the finance predicate at zero, so `finance_material` is moot, and materiality still runs and is still recorded wherever a submission is made. Plan-price reached the same posture from the other side and for a different reason — `chk_pricing_approval_approver` admits no `approved` row without an approver, so a record there could only sit `submitted` forever holding the tenant's one proposal slot (**D-380**). This entry converges on the posture, not on the mechanism.
- **The waiver is scoped by subject kind, and four of the six keep demanding a record** (corrected 2026-09-22 by review of the first cut, which keyed the arm on the quorum alone and was wrong for all four). `system_signal` is the one this register already decided against: `inst-gv-one-shot` says *"`N` has no standing over it — the principal is not a tenant principal — so `N = 0` neither weakens nor strengthens it"*, and the first cut let a composition clear run at zero with no signal record at all. `materiality_policy` is the subject that governs `N` itself, so waiving its record at zero would let the count excuse its own change (C4). `bulk_batch`'s authorization **is** the batch's record. `sku_correction`'s door pins an id into its own chain, and under the first cut the gate said yes while the door answered `APPROVAL_REQUIRED` — the gate granting and the door refusing. `governed_live_op` set doors already answer in one call under **P-D-173** by opening their own unit, which is a different mechanism and is left alone.
- **And a door that pins the authorizing id refuses a record-free authorization, whatever the quorum.** Both retire doors write it into `products_scheduled_transition.approval_ref`, which is `NOT NULL` and which the activation runner resolves at `effectiveAt` (P-D-105). The first cut reached `pinned.map_or_else(Uuid::now_v7, …)` in production for the first time and wrote an id naming nothing: the runner defers, then fails, and `uq_products_scheduled_transition_live` blocks a clean replacement, so the retirement wedges for good. They now refuse `APPROVAL_REQUIRED` at the pin, which is exactly their pre-P-D-180 behaviour. **Owed:** making that column admit "no record" and teaching the runner to verify its absence is what would let a retire be record-free too; it is a migration plus a runner change and is not done here.
- **So "publish is one call" is true of a publish and not of a scheduled retire**, and this bullet exists because the first draft of this entry said *"the uncomposed bundle is the one act at `N = 0` that still requires a submission"*, which was measurably false in four more places.
- **The uncomposed bundle is the remaining publish-path carve-out, and it is deliberate.** `NoRecord` carries no override acknowledgment — *"an override nobody granted is not one the door may apply"* is already the `GateAuthorization` doc's own words — so publishing a `bundle` plan-price has not composed still answers `BUNDLE_OVERRIDE_REQUIRED` at zero, and its author still reaches publish through a submission carrying `author_override_ack` (**P-D-68** arm 1, **P-D-13**). **Rejected**: giving the bodiless publish verb a body for that one field, which is the shape plan-price's ADR-0004 refused for a window and refuses here for the same reason; and refusing an uncomposed-bundle publish at zero outright, which takes back what P-D-13 granted.
- **What this costs, named rather than absorbed — and it is the reason this entry exists at all.** This gear has **no success-path carrier** for an authorization. **P-D-21** puts committed acts on the event stream, and `SkuPublished` carries the body core plus `publishedVersion` and **no** `approval_ref`; **C7** declines an in-gear hash chain because sealing is a platform capability. So after this decision a quorum-zero publish leaves **no statement that it was authorized at zero**: a `NULL` `approval_ref` cannot be told apart from an older row or from a defect, and `quorum_reduced` — the flag P-D-177 re-aimed precisely so an auditor could filter on it — has nowhere to be written. The objection was raised before the call and **the owner accepted the cost with the trace recorded as a debt**, which is the next bullet. It is written here so that a reader in six months does not find this as a silent gap and read it as an oversight.
- **Owed, to a named payee.** When the platform audit capability lands (**P-D-08** S1–S9, carried as a PRD §15 open owned by Architecture) a quorum-zero authorization MUST write a sealed audit row carrying `required: 0`, `quorum_reduced: true`, the gate's own reason string, the actor, and the subject at its pinned revision. Until it exists the only trace is the publish event beside a `NULL` `approval_ref`, and **that is insufficient rather than sufficient**. The debt is filed in PRD §15 beside the sealing open, not only here, because a debt recorded in one document and nowhere else is how a decision's obligations go uncollected.
- **Two orphan shapes are left standing at zero, not one.** A `superseded` record, and a `satisfied` record **pinned to another revision**: the `satisfied` match excludes both, so the act publishes record-free and the row stays open. The second was unnamed in this entry's first draft and is the same leak. Both are pinned by probes. The first: The `satisfied` match already excludes superseded rows (`inst-gv-supersede`), so after a frozen-content edit the author publishes under `NoRecord` and the superseded row is orphaned rather than consumed. Stated so the orphan is not read as a leak: at zero there is no approver to be protected from stale content, and the row remains the record that a submission was once made.
- **Where the count comes from, and why the gate reading it is not a re-evaluation.** The gate builder resolves the tenant's policy and passes `approver_count()` into the host. This is **not** the materiality re-evaluation `inst-gv-gate` forbids — the verdict stays the submission's; the question asked here is the different one of whether the tenant has any approver for a record to hold. The same resolve is already performed one door over by 06's composition-clear. `resolve_materiality_policy` cannot answer `Unresolvable` (an absent row resolves to the **default**, P-D-11), so a fresh tenant keeps the default count of **1** and stays fail-safe: this decision moves the **configured** zero only.
- **Not changed**: the floor and the meaning of `N` (**P-D-11**, **P-D-177**); the one-shot same-transaction flip wherever a record *is* matched (`inst-gv-one-shot`); the submission door and every kind that reaches it; materiality evaluation; the `PreAuthorized` mode and its composite acts; every `N >= 1` path, where a publish with no record is still `APPROVAL_REQUIRED`.
- **Propagated**: `design/05-governance.md` (`inst-gv-materiality`'s quorum-zero clause, `inst-gv-gate`'s third `yes`, C1, C7, the bundle-override carve-out), `design/01-foundation.md` (`inst-fd-gate-verdict` — what crosses the seam, which is where the record-free arm's absent id and `false` override belong — and `inst-fd-publish-consume`; **`inst-fd-governance-gate` is deliberately not amended**: `GateMode` is unchanged, this decision adds a disposition the `Gate` mode can return, not a third mode), `PRD.md` §15 (the trace debt), `features/governance.md` (three scenarios). Implementation: `domain/approval.rs` (`StoredApprovalGate` carries the count), `domain/governance.rs` (the `NoRecord` doc, which claimed the real host never gives this answer), `api/rest.rs` and `infra/bulk_worker.rs` (both host call sites).

#### P-D-179 — `plan_tier` is demoted, not removed: a relabel stops being material, and the field keeps its vocabulary

- **Date**: 2026-09-21 (owner call). Not breaking on the wire: nothing is removed from this gear's request or response shapes. Pricing's counterpart is **D-383**.
- **Decision**: `plan_tier` moves from field bucket **iii (material-mutable)** to **iv (descriptive)**. Changing a SKU's tier on a published head stops costing an approval ceremony; everything else about the field stays — the `PlanTier` vocabulary and its governed doors, the closed-set validation, the three `PLAN_TIER_*` codes, `PlanTierUpdated`, the column, and the `standard` default of P-D-131 row 11.
- **The harm this removes, measured on the benidorm stand 2026-09-21.** SKU values are **296 `standard` against 1 `gold`** — the default and nothing else — and no behaviour in either built gear branches on the value: the only `match` sites on it match on *presence* and on the *result* of classification. A ceremony with an independent approver, for relabelling a string nothing reads, is a cost with no control behind it. Bucket iv is *"the same save-and-re-publish path as bucket iii, differing only in the materiality 05 reads off it"*, which is exactly the difference intended here.
- **Bucket iv gains its first member, and that closes a gap rather than opening one.** `domain::materiality`'s `bucket_bearing` records that the DoD's bucket-iv clause *"has no column to probe: `Descriptive` carries no registered member today"*, and that the function *"stays exhaustive when bucket iv gains its first member"*. It now has one.
- **Why the field is not removed, and this is a correction.** Removal was decided first and reversed the same day. The argument for it was *nothing consumes the tier* — true, and measured over **two gears only**. The PRD says otherwise in its own glossary: *"**PlanTier** | Mandatory classification carried on SKUs/Plans … consumed by Subscriptions, `SlaPolicy`, and quota/entitlement"*. And `gears/bss/subscriptions` carries **40 references in 7 files**: its `DESIGN.md` names this registry as the supplier of published `skuId`/`PlanTier`/`CatalogVersion`, builds `02-composition-versioning` on an effective-dated snapshot segment carrying the tier, and requires *"`PlanTier` derivability"*; its PRD requires the tier to be *"derivable for any charge instant"*. The consumer is designed, not hypothetical — it is simply in a gear that is not built. A measurement over the built gears was reported as a measurement over the platform, and that is the error being corrected here.
- **What stays wrong, and is owed rather than fixed.** The seam between this gear and pricing is ungoverned in one direction: this gear holds a closed set while pricing declares the same field *"a registry-owned taxonomy, so a free string"* and validates nothing, so **136 plans carry a tier absent from their own tenant's vocabulary** — `platinum`, `silver`, a capitalised `Standard`. Grounding pricing against this registry is **owed** and is not done here: it obliges those 136 rows to be reconciled first, and it is the subscriptions owner's question whether the derivability their design requires needs a governed set at all or a string suffices.
- **Not changed**: the vocabulary's 263 rows, its doors, its event, its authz resource and grant; the `standard` default; the publish-time requirement that a SKU carries a tier; `metering_unit`, `usage_type_ref` and `sellable`.
- **"Optional" and "required at publish" are not a contradiction, and the field is neither.** It is **defaulted**: optional on the wire, always present in the row. The create door writes `standard` for an omitted value, the bulk path does the same, and a save cannot clear it — `expect_sku_string` refuses a blank after trimming — so **no wire path produces a head without a tier** and the publish arm (`PLAN_TIER_UNKNOWN`, *"plan_tier is absent"*) cannot fire through a door. It guards the **row**, not the request, against a head written around the doors, which is the same fail-closed pair `sku_type` carries and whose own doc says it is *"reached only by a head written past that door"*. Recorded because a reader meeting *optional* in the schema and *required at publish* in the code reads a contradiction, and nothing said otherwise.
- **Propagated**: `design/01-foundation.md` (the bucket-assignment paragraph, which recorded bucket iv as empty) and `design/03-sku-classification.md` (C6's bucket registration, where the tier moves from iii to iv). **Owed**: nothing in this gear's design set; the pricing counterpart's own propagation is **D-383**'s.

#### P-D-178 — `brand_id` becomes optional, and name uniqueness reads `(tenant, name)` until a brand is named

- **Date**: 2026-09-21 (owner call, taken while reading the published create-Product contract). **Breaking on the wire only in the permissive direction**: every request that was accepted before is accepted still.
- **Decision**: `brand_id` is an **optional** payload field on the create door. Absent — or the nil UUID, which is the same statement spelled differently — means **this Product names no brand**, and §4.1's absolute uniqueness therefore reads `(tenant_id, name_normalized)` across every brand-less Product. Naming a brand re-opens the name: two Products may share a name when their brands differ, exactly as before. The column keeps its shape and its index; what changes is that the door no longer demands a value for it.
- **What this supersedes.** P-D-33 made `brand_id` *"a required payload field, validated against the caller's brand claims"*, on two grounds: it is an operand of §4.1's index, and deriving it from claims breaks on a principal holding more than one brand. **Both grounds survive and neither requires the field to be mandatory.** It stays an operand — a caller who names a brand still gets the old behaviour — and nothing is derived from claims here, because nothing is derived at all: absence is taken as the answer rather than guessed at.
- **Why the requirement had to go, measured rather than argued.** The half of P-D-33 that justified the demand — *validated against the caller's brand claims* — was never built, and `api/rest/products.rs`'s own module doc says why: `SecurityContext` carries five things and no brand claim, so there is nothing to validate against, and the half stays owed to the platform-identity owner. What shipped is a presence check. On the benidorm stand, 2026-09-21: **325 Products, 322 distinct `brand_id`s, 0 nil.** Callers mint a fresh UUID per Product to satisfy a field nothing reads — and because brand is an operand of the name index, an invented brand **disables the rule the field exists to serve**. One duplicate name is already on that stand, admitted for exactly that reason. A mandatory unchecked identity operand does not strengthen uniqueness; it dissolves it.
- **Why the nil UUID and not a nullable column.** Three reasons, and the third is the decisive one. (1) The gear already stores absence as a concrete empty value rather than `NULL` — P-D-39's `region_scope`/`brand_scope` are `text NOT NULL DEFAULT ''` with *"absent meaning the empty set"*; this is that rule applied to a `uuid`. (2) No migration: the column stays `NOT NULL`, no `CHECK` mentions it (measured against `pg_constraint`), and the existing index needs no edit to deliver this decision. (3) **`NULL` in a unique index is engine-divergent and would invert the decision.** Postgres holds `NULL`s distinct, so every brand-less Product would be unique to itself and unlimited same-name rows would be admitted — the opposite of what is decided here — while SQLite would have to be checked separately for the same question. A sentinel is compared like any other value, on both engines, and the rule is then the index's rather than the engine's.
- **The nil value changes meaning, and it is free to.** It was refused at the door (`VALIDATION`, *"brand_id is required"*), so no row carries one — 0 on the stand — and no stored value is reinterpreted. An **explicitly** nil `brand_id` is accepted and means what omission means: two spellings of absence that behaved differently would be a trap, not a distinction.
- **Not changed**: `brand_id` remains bucket-i (settable while `published_version = 0`, and after first publish only through 07's correction door); it remains an `$orderby` key and a view field, always present and nil where no brand is named, so the read contract keeps its shape; `brand_scope` is untouched and is a different thing entirely — where the Product may be sold, not who owns it; and the claims-validation half of P-D-33 stays **owed** to the identity owner, unchanged by this entry.
- **Propagated**: `design/01-foundation.md` §3.1 row 4 (the create-door step) and `inst-fd-name-unique`; `api/rest/products.rs` (the create DTO, the module doc's `brand_id` section, the OpenAPI description); `infra/storage/entity/product.rs`; `infra/bulk_worker.rs` (the bulk create path reads the same absence).
...
- **Why the base-role gate goes.** C1's role clause is a real intent and it was made strict for a real reason: `BaseRoleSet`'s own doc records that the permissive reading was what *"I have no data"* silently resolved to, and that under it *"a material change closed on two principals holding neither C1 role"*. The axis it guards is genuine — RBAC says *may you call this door*, the role says *are you a qualified approver* — and it matters most where RBAC grants are coarse, which they are: the downstream suite authors one role object over `gts.cf.bss.products.*` carrying every operation.
  **What failed is the supply, not the intent.** The role axis never got a minter: **P-D-134** row 25 routed the claim shape to the platform-identity owner and it has not arrived, so the strict check has no input on any deployment. Measured 2026-09-21 at `738da4993`: **no principal on the stand can decide any approval**, and the downstream e2e writes the policy row with `psql` to get past it (`tests/e2e/tests/lib/products.py`: *"nothing governed can ever run on a fresh tenant through the wire"*). A control whose operand does not exist is not strict; it is inert, and it blocks the ceremony it was written to qualify.
- **What is *not* done, and why.** The wildcard `*` is **not** read as a role. It is a **permission** wildcard — the gear says so in `approvals_tests.rs` (*"not a harness defect: it is the production posture"*), `ctx_with_roles` carries `*` and a role side by side as two axes, and the gear explicitly refused the same move for scope claims: *"reading a scope claim out of `token_scopes` under a spelling no document declares would be inventing the seam rather than waiting for it"*. Conflating the two axes would overturn that position silently and hand the FinanceReviewer lens to every first-party caller.
- **What replaces it.** The decide door stops asking the token *who you are* and rests on the authority that already answers *may you decide*: RBAC's `approval × decide`. The quorum is settled under [`BaseRoleSet::AnyDecider`] — the second reading the type already carries, chosen out loud at the call site exactly as its doc requires, rather than a check deleted. Self-approval stays refused on identity at every `N ≥ 1`, distinctness stays by principal (C2), and the record still names every decider.
- **What this costs, named rather than absorbed.** C1's floor — *each approver holds CatalogAdmin or FinanceReviewer* — is no longer enforced at the door. Today it enforces nothing, because no principal can hold either; tomorrow, if the platform ever mints role claims, this is the clause that would have to be re-armed, and re-arming it is a one-line change of the `BaseRoleSet` passed at the settle site. **The FinanceReviewer predicate is untouched**: it still reads the claim and still discriminates wherever one exists, so this decision neither strengthens nor weakens it. Giving both the floor and the predicate a supply they can actually have — an RBAC action such as `approval × decide_finance` — is **owed, not built here**: minting a catalog pair is a change this slice may not make alone, the same question `features/governance.md` §7 row 24 already asks.
