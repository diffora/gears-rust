...
**Risks & open items**:
- ~~**Open above this slice: does browse need a separate serving store at all?**~~ **Answered by the build (P-D-150, 2026-09-05; struck at P-D-162)**: `products_read_entity` and its stamp are the serving store, fed by the gear-owned inbox and read by the browse door behind one limiter — the projection is the store, and no second store exists to ask about. *The item's text stood as:* *(P-D-132, 2026-09-03: the workshop's; DRI and date to be named by the Program Lead.)* Raised in review and now a PRD §15 question for the NFR workshop. `fr-cache-first-browse`'s
