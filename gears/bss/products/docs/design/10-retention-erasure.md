...
- Retention gate RED: a candidate version with one live freeze-registration is skipped +
  ...
  beside a stamped `released_at` is still skipped + alarmed (the AC #44 pair, and the 2026-08-26
  correction's own regression).
...
- **Which store holds the audit rows this slice's own rules require?** `inst-er-erase` is "audited
  ...
  **P-D-21** — "only acts that emit no event, in three classes". The erasure act emits
  ...
  eventless. Owner: 01's owner with P-D-21's. *(Two lenses raised it independently — slice-10 first lens pass, 2026-08-28.)*
- **What `actor_ref` attributes an unattended act's audit row?** The age-triggered tombstone and
  ...
  admits a null one. Owner: 01's owner with this slice. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **How is the map read by principal?** §4 states one key, `(tenant_id, actor_ref)`, while three
  ...
  survive erasure. Owner: this slice, with 01 as the consumer of the resolve path. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **Does the retention gate collect a version that has no freeze registrations at all?** The gate
  ...
  gate ranges over it. Owner: 06's owner with this slice. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **Where does a successful restore drill's state live?** `inst-rd-drill` puts "the last-verified
  ...
  Owner: this slice with P-D-21's owner. *(All three lenses raised it independently — slice-10 first lens pass, 2026-08-28.)*
- **What does the drill do on a digest-version mismatch, and what is the corruption alarm
  ...
  arm against. Owner: this slice with the NFR #5 workshop. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **Which surfaces may resolve an identity through the map, and under what grant?**
  ...
  and 08. *(Two lenses raised it independently — slice-10 first lens pass, 2026-08-28.)*
- **Who owns NFR #5's cold re-resolution MUST, and how does the clause split with 06?** Both
  ...
  *(Two lenses raised it independently — slice-10 first lens pass, 2026-08-28.)*
- **Who delivers the DR half of the durability mechanics?** §1.1 promises "storage class, periodic
  ...
  Owner: the design owner, with the NFR workshop's output in hand. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **Which actor holds `compliance × export`?** The door returns real identities, the only
  ...
  here. Owner: Architecture with Legal. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **Is `products_pii_allowlist` itself a PII store?** By construction it holds person-named
  ...
  deliberately did **not** add these two fields. Owner: Legal with the data-protection owner. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **What code does the allow-list's missing-sign-off refusal carry?** `inst-pp-allowlist` refuses
  ...
  or this slice declares its own. Owner: this slice with the error-contract owner. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
- **What does "byte-identical in effect" mean for the age-triggered path?** §5 asserts the age
  ...
  the audit row is part of "effect" or only the map state. Owner: this slice. *(Raised by the slice-10 first lens pass, 2026-08-28.)*
