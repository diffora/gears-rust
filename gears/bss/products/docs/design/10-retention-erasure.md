...
1. [ ] - `p1` - `POST /bss-products/v1/erasure-requests` (`erasure × execute`): resolves the operator identity to its **one live** `actor_ref` in the tenant — `uq_products_identity_ref_active` caps it there, and the shipped resolve is `.one(…)`; ~~`actor_ref`s~~ (§7 row 21, corrected 2026-09-03: the plural named a population the index forbids) — and **overwrites the map entry with a tombstone** (pseudonym retained, identity gone) — one transaction, audited with a reason; no immutable record is touched (C1), and every historical read through the map now renders the tombstone; **erasure completes within one tenant** (**P-D-50**) - `inst-er-erase`
...
- Age-based pseudonymization fires without a request and is byte-identical in effect to the
  requested path — **in the map**: the entry is tombstoned, its payload destroyed and its `principal_ref`
  left standing exactly as a requested erasure leaves it. The **audit row differs by construction**
  and is meant to: no requester and no supplied reason, so it is written under the system principal
  (`gear::system_actor_ref()`, P-D-113) with the age rule's own name as the reason. *(§7 item 14,
  P-D-117, 2026-09-03.)*
