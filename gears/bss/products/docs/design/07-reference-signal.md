...
future-watermark and tripwire alarms (never-received is a verdict flag, not an alarm — C2); the
...
- **This slice's free-text `reason` doors do not invoke the content-PII write block.** 02
  ...
  producer-retirement justification of `inst-pr-retirement`, and records that this slice
...
- **Where does `inst-ws-monotonic`'s set hash come from?** An equal `watermark_at` with an identical
