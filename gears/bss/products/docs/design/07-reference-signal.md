...
1a. [ ] - `p1` - **Retirement can only tighten, never free — the converse of `inst-pr-onboarding`** (item 21 of the 2026-08-26 review). Retiring a producer **narrows the AND-quantifier the fresh-zero predicate runs over**, so with two producers, retiring the *stale* one makes the predicate answer fresh-zero over the remaining fresh one — and a correction that `CORRECTION_REFERENCED` had blocked walks through the **normal** door: no feature flag, no `SkuCorrectionOverride`, no `TripwireCounter` increment, none of the break-glass lane's evidence. Only the last-producer case was guarded. So: retiring a producer whose watermark is **stale or never-received** is refused **`PRODUCER_RETIREMENT_WOULD_FREE`** unless the retiring principal supplies the break-glass ceremony's own justification — the retirement is admissible, its *silence* is not. **And the fresh case is the same defect, not the safe one** (corrected 2026-08-26, PR #14 review — this rule closed with "a producer retired while fresh frees nothing it was not already reporting zero for", which is backwards): the predicate is an AND over the registered set, so retiring a **fresh** producer frees precisely the SKUs it *was* reporting non-zero for, whenever no remaining producer reports them. Retirement is therefore refused `PRODUCER_RETIREMENT_WOULD_FREE` whenever the retiring producer's **current watermark is non-empty**, on the same terms: refused for the ordinary door outright — **only a producer whose current watermark set is empty retires through it**. A non-empty producer retires solely through the break-glass lane, and that lane must **preserve what it removes** (added 2026-08-26, PR #14 review): the retiring producer's final watermark is copied into `products_reference_member` as a **retained conservative source** for those SKUs, so the fresh-zero predicate keeps answering *referenced* for them until an operator clears each one under its own `SkuCorrectionOverride`. Justification alone does not free a SKU; without the copy a uniquely reported SKU turns fresh-zero the instant its only reporter leaves, which is what the ordinary door refuses - `inst-pr-retirement`
