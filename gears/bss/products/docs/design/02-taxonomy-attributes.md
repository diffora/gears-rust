...
1. [ ] - `p1` - The tree is **readable**: `GET /bss-products/v1/categories` answers one keyset page of the tenant's categories under **`category × read`** — the grant's first spender in code (**P-D-181**; row 234 of `05` §3.2 claimed browse spent it, and browse asks for `product|sku × read` alone). A row carries `category_id`, `parent_id` (`null` for a root), `name`, the rendered `path`, `state` and `mutation_seq` — the id because the assignment payload needs it and the `201` was its only source, and `mutation_seq` because the live-value door asserts it back as `expectedSeq` and the `PATCH` response was *its* only source. **Retired nodes are listed with `state: retired`**, both name-uniqueness indexes being state-agnostic (§4.1) so a tombstone still holds its name against `DUPLICATE_CATEGORY_NAME`. The answer is a **flat page, not a nested tree** — a nested body cannot be keyset-paged — narrowed and ordered through the platform's `OData` surface and paged with `PageInfo` (**P-D-165**), `parent_id` filterable **by value** and **not** orderable because it is nullable. **The roots are not addressable by filter** — `parent_id eq null` parses and then fails the platform's type check (`FieldKind` has nine scalar variants and no nullable one), measured 2026-09-22 — and the door does not work around it: a `roots=true` operand would put back the per-door parameter **P-D-165** took away. A caller selects `parent_id == null` over the page, which the flat shape makes possible. **Paged, unlike `03`'s vocabularies** (**P-D-170** refused paging for a closed set of tens): the shipped ceilings are depth 8 and fan-out 1,000, so this is an open-ended surface. `path` is rendered server-side with the browse projection's own renderer, a caller holding one page having no ancestor chain to join. **No `ETag`** — no door on this surface takes a set-level precondition, P-D-170's rule kept - `inst-tx-read`
...
4. [ ] - `p1` - The registered `→ published` validator requires a **value at the global coordinate — all three of locale, region and brand absent** (`("", "", "")`, P-D-88 arm 2's spelling) — for every localized definition the entity carries values for (rejected at publish, not at draft save); per-brand and per-locale values are optional overrides — the global one is what makes the fallback chain total for **every** brand (M5 fix). **P-D-102** struck two things from this clause: the phrase *"default-locale value"*, which named a coordinate that carries no locale, and the parenthetical *"(brand-less)"*, which read `global` as absent on the brand axis alone. A brand-less value **at the tenant default locale** would carry the locale that was default *when it was written*, so a later config change makes step 3 miss it — the un-totalling this row's own step 5 refuses. A locale-less value cannot go stale that way, which is why the global coordinate is absent on all three axes - `inst-av-default-locale`
5. [ ] - `p1` - Read-side resolution (consumed by slice 08): `LocaleResolver` walks `(locale, region, brand) → (locale, brand) → (default-locale, brand) → global`; the default-locale is the **tenant** default (**P-D-101** struck *"resolves per brand, falling back to"*: the per-brand default had no store, and a second config value under a step that cannot change whether resolution succeeds doubles the very exposure this row's own next sentence argues against) — the chain is total for every brand by step 4's **global** default-locale guarantee. **Totality is anchored on the resolution path, not on the config value** (item 37 of the review): the tenant default locale is ungoverned config with no re-validation, so anchoring on it would un-total the chain for every already-published entity the moment it changed. So the final step is the **global** fallback and the tenant default is only a *preference* consulted before it; a tenant-default change is therefore non-retroactive by construction — the same posture `inst-ti-limits` states for depth limits - `inst-av-resolve`
