...
- **What writes the clone's category assignments, attribute values and metadata map?** All three live
...
- **How are `-copy-N` and `-revived` generated?** `N` is never defined (per-source counter, global,
