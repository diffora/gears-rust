...
2. [ ] - `p1` - **AC #38 map**: every enumerated failure row **that a registry door can refuse** maps to exactly one error code with exactly one raising door. **Three of the fifteen rows are explicitly outside the universe, named here so the lint is buildable rather than perpetually red** (item 32 of the 2026-08-26 review): the retention-orphan row is deliberately an **alarm**, not an API error (10 `inst-rt-order`), and "adopting a `compositionPending` bundle" is a **consumer duty with no registry door** (§2.2). The lint asserts the exclusion list is exactly those two — an unexplained third exclusion fails it - `inst-cc-errors`
