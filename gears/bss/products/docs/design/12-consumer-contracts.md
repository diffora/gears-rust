...
| Enforce `sellable = false` as sellability-gate predicate 6 over **every SKU a new sale includes** — the plan's own, each row's component, each bundle member and its components; no component exemption (**P-D-176**) | pricing | D-46 as amended by P-D-176 / pricing D-376; pricing **D-167 records predicate 6 as needing this registry gear** | `sellable` | **owed, not assertable yet** (item 35 of the review corrected M7's reading: the pricing side is designed but its operand is this gear's `sellable`, so the joint assertion cannot precede the seam it asserts) |
...
4. [ ] - `p2` - **Every list surface answers the platform's query contract** (**P-D-165**): the `OData` family (`$filter`/`$orderby`/`$top`/`$skiptoken`, with `limit`/`cursor` as the platform's own aliases) over a **declared** filter vocabulary, `toolkit_odata::PageInfo` on the response, a keyset walk under a unique tiebreaker, and an **undeclared query key refused rather than dropped**. The assertion this owes is the last one: a door that ignores an unrecognized parameter answers `200` with the *unfiltered* set, which no consumer-side check can distinguish from a correct answer, so the suite has to send one and read the `400`. The vocabularies themselves are impl-crate types rather than SDK types — products has no in-process consumer that builds a filter in Rust (contrast `account-management`, whose `IdP` plugins consume `FilterNode<IdpUserFilterField>`), and promoting them is additive if one appears - `inst-sdk-query-surface`
