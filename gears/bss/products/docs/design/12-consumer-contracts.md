...
1. [ ] - `p1` - Every event schema is a versioned artifact in `products-sdk`; the CI compatibility test deserializes every `vN` fixture against the `vN+1` schema on every schema change (C2) — the 04 EOL field (`mustMigrateBy` present-but-unpopulated) is the standing example; **the 09 export-artifact schema joins the same corpus** (L3 — the discipline it cites is now exercised, not borrowed) - `inst-rc-compat`
