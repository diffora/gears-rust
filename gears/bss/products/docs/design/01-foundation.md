...
- [`../DECISIONS.md`](../DECISIONS.md) P-D-01 (envelope), P-D-02 (mechanical increments),
  ...
  P-D-23 (the 2026-08-27 owner round, recorded inline in the rules they change; the register
  carries the authoritative table), P-D-14 (`system_signal` subjects), and the same day's later
  ...
  here), P-D-32 (the second lens wave's six calls)
...
**200**; `POST /bss-products/v1/{products|skus}/{id}/publish` (**`… × publish`**) → **200**;
`POST /bss-products/v1/{products|skus}/{id}/discard` (**`… × discard`**, **`If-Match` required**) → **200**. Every
mutating door accepts **`Idempotency-Key`** (§3.2), and every one of them is an ordinary `Gate`-mode
...
`PARENT_NOT_PUBLISHED` (registered by slice 04 on the `→ published` **target state**, not the
edge — P-D-32; named here so AC #38's
...
the `If-Match` verb and the publish pin, the idempotency resolver raises `IDEMPOTENCY_CONFLICT` and
...
rule-named codes wherever it runs, including the publish re-run;
...
`inst-av-pii-reason` hook, which is not a pipeline phase at all — every door carrying a free-text `reason` invokes it, and slice 02 holds its single
...
from the same section's pagination rule, whose subject there is an undecodable cursor —
...
1. [ ] - `p1` - `skuCode`/`productCode` race: decided by the `ReservationIndex` (§4.2) and §4.1's `product_code` partial unique index under the insert, not by a read (two concurrent creates: exactly one admitted) - `inst-fd-code-race`
2. [ ] - `p1` - Name race: decided by §4.1's partial unique index under the insert, not by the step-3 read (two concurrent creates of one normalized name: exactly one admitted; the loser fails `DUPLICATE_NAME`) - `(cont. inst-fd-name-unique)`
...
which of the two governs is §6's open item)): `name` and `name_normalized` are **bucket-iii** — a published Product
can be renamed,
...
`fr-parent-child-integrity`'s fail-closed check; `sku_code`, `product_code` and **`brand_id`** are **bucket-i** — the second following AC #1's
"same rules as `skuCode`", and `brand_id` because re-branding moves the row into a different
...
column names a SKU, and 04 `inst-rt-initiate` requires `replacedBy` to name a `published` SKU;
04's own column list contradicts itself on this and the correction is owed there) only through
slice 04's acts** — door
...
review), and **`composition_pending` (`products_sku` only — `bundle` is a value of the SKU-only
...
first publish** (owner's call, 2026-08-27, P-D-28: the whitelist named a prohibition where every
...
admitted UPDATE, without which the guard would refuse every write it otherwise allows.
...
`deprecation_provenance` and `replaced_by_sku_id`** (owner's call, 2026-08-27: those three move on
...
with a **`digest_version`** column beside it (owner's call, 2026-08-27, P-D-29: `sha2` is already
...
2026-08-27, P-D-28) — which is what lets §3.2 hash a *parsed request* under this same rendering
and the gear pin one canonicalization rule rather than two. The digest's input is the version's
own column set, so
...
- `products_idempotency`: `(tenant_id, endpoint, client_key)` PK · `state` (`claimed | answered`)
  · `payload_hash` · `response_status` · `response_body` · nullable `in_flight_until` ·
...
- A test asserting the `BucketRegistry`'s tag map and §4.2's trigger column classes name the same
  ...
  whitelist admits them together, and **bucket-ii asserted only once 07 supplies its row-image
  predicate**; the mechanical columns the whitelist names by hand (`lifecycle_state`,
  `published_version`, `internal_revision`, the provenance pair, `composition_pending`,
  `cloned_from`, the timestamp) carry no bucket tag and are outside the comparison (P-D-32 — the registry is advisory for the physical layer, so
...
- **FRs this slice carries a named half of**: `cpt-cf-bss-products-fr-lifecycle-transitions`
  ...
  row is 10's), `cpt-cf-bss-products-fr-create-product` (uniqueness),
...
**Risks & open items**: the thirty items three review passes had accumulated were decided, merged,
or filed with their owners in `PRD` §15 by the 2026-08-27 owner round. What the fourth pass left
open is below — one standing **risk**, which is a hazard rather than a question:
...
*The fifth pass (three lenses, 2026-08-27) added the items below.*

- *Filed elsewhere by this pass, pointer only:* the instruction row registering the create-door
  live-retire-intent validator is **owed by 04** (P-D-30 settled that it is that slice's); and
  what pins the partition count `N` and the hash behind §4.4's ordering claim is the
  transport-contract row in `PRD` §15.
- *Filed elsewhere by the sixth pass, pointer only:* the `product|sku × discard` grant §2's
  discard door names is absent from 05's RBAC catalog (registered in 05); both phase carve-out members,
  `CONTENT_PII_BLOCKED` and `AUDIT_UNAVAILABLE`, are owed a mirror in 12 `inst-cc-errors`, which
  names neither (registered in 12); and AC #26's literal "returns the entity to `draft`" against the head-row
...
none is answered here. Its **third lens wave** added the six that follow them, and filed
`BREAKGLASS_WRITE_FORBIDDEN`'s raising site with 05 and `ENTITY_TERMINAL`'s widened gloss with 12.
It also corrected a self-contradicting column list in 04, in place.*

*The fourth lens wave (2026-08-27) added the five above and filed elsewhere, pointer only:* the
**envelope slot for `partition_id`/`seq`** — the broker's own schema marks `partition`/`sequence`
`readOnly` and rejects them on publish, so §4.4's "on the envelope" placement,
12 `inst-rc-dedup` and **P-D-27** all rest on a slot a producer cannot write (`PRD` §15, with the event-broker owner);
the `internal:cascade-leg` lane, which no slice's runner writes (04); and whether the
composition-clear re-publish emits `SkuPublished` beside `SkuCompositionCleared` (06).

- **Does the approval-invalidation hook fire on a gated transition that consumes an approval in
  the same transaction?** §2's transition row excepts only `draft→published` from the hook
  ("**except `draft→published`, which the publish door owns and which bumps once with no hook**"),
  but P-D-30 gave the gate phase to transition doors, so `deprecated→published` now consumes a
  record *and* fires the hook in one transaction — the collision 05 **C3** names as its
  ...
  record. **And a re-publish reaches neither rule:** it is by this file's own model neither a save
  nor a transition, so no row says whether it invalidates open approvals — while 05 C3 states the
  rule over "any frozen-content write", a class a re-publish plainly joins. **And the prior half,
  raised by the sixth pass's second lens wave:** no row performs the
...
- **What does the first migration's trigger admit for the columns whose row-image predicate is
  ...
  while C1 requires the guard in the first migration and §4.2 names both failure modes of guessing
  — refusing the writes 04 specifies, or dropping the columns to bucket iv where an operator save
  re-stamps the provenance operand. Owner: this slice with 04 and 07.
- **Three operands the idempotency store leaves unpinned.** (a) `in_flight_until` has only "short
  by comparison" — no value and no config key, where `expires_at` has both; a deadline too short
  admits the duplicate the claim exists to refuse and one too long wedges a crashed door's key.
  (b) §2's doors "accept" `Idempotency-Key` and the PRD makes it conditional, yet §3.1 makes
  ...
  stored outcome forever. (d) **Does the hashed field set include the precondition?** `If-Match` is
  a header, not a body field, and "parsed request" defines neither in nor out: excluded, a client
  that got `STALE_REVISION`, re-read the head and retried under the same key replays the stale
  refusal forever; included, that same retry is a different payload and gets
  `IDEMPOTENCY_CONFLICT`. (e) **What do the three `internal:` lanes store in `response_status`/`response_body`?** Both are
  NOT NULL once answered and exist to reproduce a *wire* response, which a lane with no wire
  surface has none of — while 04's crash replay and 09's per-row publish both ride this store.
  (f) **Who writes `expires_at`, and what does a `claimed` row carry in it?** The column is
  non-nullable and defined only for the answered state, yet the row is inserted `claimed`.
  (g) **Which transaction commits `state = answered`** is unstated and
...
- **Which surface returns the head row and its ETag to an author who did not just write it?**
  ...
  and the set's only entity reads are 08's projections, which serve frozen version content and
  expose no head revision. Owner: this slice with 08 and 12.
- **Does a rejection short-circuit at the first failing phase?** `inst-fd-pipeline` collects
  "violations … per-field into one audited rejection", which across phases yields more than one
  code (shape's `VALIDATION` beside identity's `DUPLICATE_NAME`), while §4.4's audit row carries a
  single `error_code` that §3.1 makes the attribution channel and AC #38 maps by. Owner: this
  slice with 12.
- **What bucket does `created_by` carry?** §4.1's bucket paragraph names nine columns and not this
...
- **How does the canonical rendering represent an absent field in a *parsed request*?** §4.3 pins
  ...
  sending it `null` mean different things at the head door and render identically, so the second
  replays the first's stored outcome. What a partial update's "named field set" is — the request's
  present fields or the entity's full column set — is unstated. Owner: this slice.
- **Do `inst-fd-idem-replay`, `inst-fd-mint-id`, `inst-fd-reserve-code`,
  `inst-fd-publish-freeze`, `inst-fd-publish-reserve-permanent`, `inst-fd-publish-bump`,
  `inst-fd-transition-bump`, `inst-fd-idem-claim-refusal` and `inst-fd-idem-claim-inflight-until` owe an event
  declaration?** §4.5's rule binds "every slice, **this one included**": each state-changing
  instruction names its event or records "no event". Their neighbours in the same subsections do;
  these nine carry neither, and nothing defines whether a step inside a transaction whose event a
  sibling row names is itself a state-changing instruction for 12's completeness check. Owner:
  this slice.
- **Which phase raises `SCOPE_NOT_CONTAINED` once 04's final rule replaces the interim check?**
  §3.1 puts containment in the **identity** phase, and §3.3 rests the one-phase-per-code rule on
  04 C5 being "the final form of 01's interim check" rather than a second raiser. But 04's
  `inst-pc-containment` is a slice-04 rule on slice-04 doors — if it runs as a `RegisteredValidator`
  the code has two raising phases and needs a carve-out this taxonomy says it does not. 04's
  neighbouring row is explicit about registration and this one is silent, so the silence settles
  nothing. Owner: whoever took P-D-24, with 04.
- **Is *one phase, one status class* a rule or only a rationale?** P-D-32 moved two codes to 409
  partly because leaving them left the `state` phase straddling two status classes — but the
  **identity** phase straddles them too (`DUPLICATE_NAME`/`DUPLICATE_CODE` 409,
  `SCOPE_NOT_CONTAINED` 422), and moving containment to 409 would contradict §3.3's own "422 for
  content the door cannot process", a subset check being a payload judgment. Owner: the
  error-contract owner.
- **Does the save door run the governance-gate phase?** §1.7 binds all seven phases to "every
  mutating door"; §2's save row runs six of them — idempotency resolution at step 1, the
  precondition at step 2, and shape/state/identity/registered-validators at step 3 — leaving only
  the gate unaccounted for. §3.1 scopes the gate to "any gated act" without
  saying whether a head save is one, and `inst-fd-approval-hook` has a save *invalidate* an
  approval rather than consume one. §2's door preamble compounds it by calling **every** mutating
  door "an ordinary `Gate`-mode caller", where `Gate` is defined only as the publish mode "which
  needs a `satisfied` record". Owner: this slice with the governance owner.
- **How does a wire caller supply the publish door's expected revision?** `PublishDoor` accepts
  `(entity, expected internal revision)`, but §2's REST publish door carries no `If-Match` and no
  body, and no document names a header, path or body field carrying the pin. Under `PreAuthorized`
  the `approvalId` names the record whose revision is checked; the ordinary interactive publish has
  no such carrier. Owner: the interface owner of
  `cpt-cf-bss-products-interface-authoring-publish`, with 12.
- **Which door writes the other two bucket-i columns while `published_version = 0`?** §4.2 admits
  bucket-i writes on an unpublished head and names `inst-fd-save-txn` as the admitting door, but
  that row carries only the `skuCode`/`productCode` change. `brand_id` and a SKU's `product_id` are
  bucket-i too, and §4.1 says "nothing in the gear re-parents one today" — so the trigger admits a
  write class no door claims. Owner: this slice.
- **Where does `brand_id` come from at create?** It is an operand of §4.1's uniqueness index and
  bucket-i, but no flow step assigns it: §1.3 has the IdP supply brand claims and §2 authorizes in
  "the caller's tenant/brand scope", while a principal holding more than one brand has no rule
  selecting which the row lands in. Owner: this slice with the authorization owner.
- **What value does `digest_version` take on the first frozen row, and where does the current
  version live?** The column is non-null by construction and the "digest-version bump, not a silent
  change" rule is normative, but no initial value, roster or store for the current version is named
  anywhere in the set. Owner: this slice.
- **No table in this slice admits a DELETE, and 10's retention GC must delete from two of them.**
  C5 exempts only the slice-08 projections and the idempotency sweep; §4.3 says frozen rows "admit
  no UPDATE/DELETE ever" and §4.4's audit trigger admits one UPDATE and no DELETE.
  10 `inst-rt-gc` deletes whole audit rows and entity versions in reference-topology order. A door-aware guard is
  already ruled out (no session variable on SQLite), so either the first migration carries a
  row-image-predicated DELETE arm — on what predicate? — or retention runs under a privileged role
  outside the trigger, or these classes are never deleted. Owner: this slice with 10 and the
  retention-duration owner.
- **Does the freeze capture the head row before the publish's own UPDATE, or the values it
  writes?** `composition_pending` is frozen content (§4.3 excludes only three columns, and it is
  not one of them) and is written by the very UPDATE that follows the freeze, while the version
  row's key already carries the post-act `published_version`. Nothing states the instant. It
  decides what 08 projects, what the digest covers and what 10's byte-for-byte restore drill
  compares. Owner: this slice with 06 and 08.
- **Does the `AUDIT_UNAVAILABLE` refusal itself owe an audit row?** §4.4's class is now *every*
  refusal a door raises, and this 503 is one — but its row is by construction the one that could
  not be written, so the class has a member it can never satisfy: a failure path with no
  terminator. §3.3's carve-out list is about *phases*, not about the audit class. Owner: this
  slice with the `nfr-availability-audit` owner.
- **What is the target state of a re-publish of a `deprecated` head?** P-D-32 keyed publish
  validators on the target state rather than the edge, but the door accepts a `deprecated` head for
  version N+1 and "a re-publish changes the version, never the state" — so that act's target state
  is `deprecated`, while the door is specified to run the validators registered for `→ published`.
  Neither §1.7's key nor §3.1's says how the target state of a state-preserving act is computed.
  Owner: this slice, whose own P-D-32 introduced the key.
- **Is a request that parses but omits `If-Match` "malformed"?** `inst-fd-etag` says an absent
  precondition is a malformed request, which §3.3 reserves for a bare 400 with no code; but
  `inst-fd-mint-id`'s criterion is that a parsed request is not malformed, and a `PATCH` without
  the header parses. Under one reading the caller gets no code to match on, and §3.3 makes the code
  the discriminator. Owner: the error-contract owner with the interface owner.
- **Who holds the single declaration of `RETIREMENT_PENDING`?** §3.3 declares it ("declared here
  2026-08-26") now that P-D-30 has moved **both** arms to slice-04 validators, and 04 declares it
  too. §3.3's carve-out for codes listed here but declared elsewhere names exactly one member,
  `CONTENT_PII_BLOCKED`, so the one-declaration rule is either broken or owes a second exemption
  class — and `PARENT_NOT_PUBLISHED`'s reciprocal wording has no counterpart here. **The same shape
  holds for `SCOPE_NOT_CONTAINED`**: 04 declares it in its own owned-code contract *and* lists it
  as declared elsewhere, without the reciprocal qualifier `PARENT_NOT_PUBLISHED` carries. Owner:
  the error-contract owner (P-D-25/P-D-30) with 04.
