...
3. [ ] - `p1` - Validate shape; normalize `name`; enforce **absolute** uniqueness on `(tenant_id, brand_id, name_normalized)` via the partial unique index (§4.1) — collision fails `DUPLICATE_NAME` naming the holder; **a Product naming no brand carries the nil `brand_id` (P-D-178), so across brand-less Products the same index reads `(tenant_id, name_normalized)` and naming a brand re-opens the name**; P-D-04: region scope plays no part; this door's shape, identity and mint steps run in the phases §3.1 assigns them, not in the order numbered here - `inst-fd-name-unique`
4. [ ] - `p1` - `brand_id` is an **optional payload field** whose absence — or an explicitly nil value, the same statement spelled differently — means the Product names no brand (**P-D-178**, superseding P-D-33's *required payload field*: the requirement rested on a claims check `SecurityContext` cannot supply, and an invented brand per Product disables the very index the field is an operand of). It stays an operand of §4.1's uniqueness index and stays bucket-i. Where a brand **is** named it is **still not validated against the caller's brand claims** and refusal `VALIDATION` for a brand the caller does not hold stays **owed** to whoever adds a brand claim to `SecurityContext` (**P-D-33**'s open half, unchanged; deriving it silently from claims would still break on a principal holding more than one brand). Mint `productId` (UUID, server-side, never caller-supplied — a stray id in the payload is a shape-phase
...
7. [ ] - `p1` - The Product create door admits an **optional `primary_category_id`** and writes that assignment inside the same transaction; **absent, the tenant's default category is resolved and assigned instead** (**P-D-182**, `02` `inst-tx-default`), and a tenant that has none leaves the Product unassigned rather than failing the create — `inst-tx-primary-at-publish` stays the guard. `02`'s assignment validators run on either path: the target must resolve in the tenant and must not be retired (`CATEGORY_RETIRED`). Only the **primary** is settable here; secondary assignments are ordinary draft content on the save door - `(cont. inst-tx-default)`
