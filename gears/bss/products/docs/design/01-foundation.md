...
1. [ ] - `p1` - Authorize; idempotency as above - `inst-fd-idempotency`
2. [ ] - `p1` - Parent must exist in the tenant and not be `retired`/`discarded`; the SKU's brand/region scope must pass the **interim containment check**: scope sets are flat value lists, containment = subset, anything not provably a subset fails `SCOPE_NOT_CONTAINED` (conservative until slice 04 pins the final rule) - `inst-fd-containment`
...
4. [ ] - `p1` - Mint `skuId`; persist as `draft` with the slice-03-owned columns present but unjudged (typing/classification rules run when slice 03 registers them); audit + `SkuCreated` outbox row in the same transaction - `inst-fd-create-txn`
...
2. [ ] - `p1` - Draft race: decided by `If-Match` (two editors: second fails `STALE_REVISION`) - `inst-fd-etag`
3. [ ] - `p1` - Publish-vs-edit race: the door's pinned-revision check makes "approve rev N, publish rev N+1" impossible by construction - `inst-fd-publish-pin`
...
`PublishDoor`; bucket-iii/iv columns + `internal_revision` via the save door in any non-terminal
state; bucket-ii columns only via the slice-07 correction door; bucket-i identity columns never
after first publish. **`normalized(name)`** (the uniqueness and promotion-identity operand,
...
**Traces to (PRD)**: `fr-identifier-contract`, `fr-create-product` (uniqueness carrier),
