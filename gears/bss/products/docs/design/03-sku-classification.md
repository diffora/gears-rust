...
1. [ ] - `p1` - One generic shape: `(tenant_id, set_kind, member_code, display_label?, state ∈ {active, deprecated, removed}, seeded_by?)`; mutations ride `GovernedLiveOp` (02 §3.1) **through `POST /bss-products/v1/config/vocabularies/{class}/values` and `…/members/{memberCode}/transitions` — and `POST /bss-products/v1/config/vocabularies/{class}/values/{value}/label` for the display label alone (P-D-146; declared at P-D-159) — the grant chosen by `setKind` — the tier set spending `plan_tier × write` and the unit set `recognized_set × write` (**P-D-90**)**, and every admitted mutation emits the set's event in the same transaction (§4); membership checks are the classification validators' single lookup — **the set is the `active` and `deprecated` rows; a `removed` row is a tombstone outside it** (**P-D-47**: a removal is a state flip, never a DELETE — the donor's taxonomy values are `Active|Retired` and its repository refuses to delete one for the reason that holds here, that a value a published row names has to keep existing; the PK therefore never frees, which makes C3 a schema property rather than a convention); **each of the three doors has a declared op token** — `recognized_set.add`, `recognized_set.transition`, `recognized_set.label` — which a `GovernedLiveOp` submission carries in its `content_snapshot` the way `09`'s lifecycle rows already carry `{"op": …}`, and which is what `05` §4's `min(N, 1)` display-label exception reads (**P-D-171**) **on a `recognized_set/{set_kind}/{member_code}` subject and no other**, the subject naming the member and not the act; **a record that declares one authorizes that change and no other** — the door compares the record's `content_snapshot` with the change it is about to make, `07`'s correction-door comparison at `05`'s live-op gate, and refuses `APPROVAL_REQUIRED` on a mismatch (**P-D-172**), while a record declaring no op of this slice's authorizes every op as it did before; **and the door opens the unit it needs** — with none standing for the change it submits one over its own rendering of that change, answers `202` naming it and writes nothing, the identical request re-sent after approval answering the door's ordinary `201`/`200` (**P-D-173**, pricing's two-arm shape at `D-355`); a unit already open for a *different* change on the member is named in the refusal rather than superseded, since `05` §4 admits one open unit per subject; **the transitions and label doors require `If-Match`** on the member's own tag, compared under the write and refused `STALE_LIVE_OP` when stale (**P-D-174**) — the tag pins the whole row where `expected_state` pins one column and the label door had no pin at all, and both are kept because the state is what the approval unit was agreed against; **the add door takes none**, a create naming no member to have read and the wildcard being refused throughout this gear, its guard being the `DUPLICATE_CODE` refusal under a primary key that never frees. **The route family is `/bss-products/v1/config/vocabularies/{class}/values…` (P-D-175, 2026-09-14)** — the same templates as before under the word both gears now share, `{setKind}` spelled `{class}` and `members` spelled `values`; the grant is still chosen by the class token and is still `plan_tier`/`recognized_set`, never `config` - `inst-rs-shape`
2. [ ] - `p1` - The set is **readable**: `GET /bss-products/v1/config/vocabularies/{class}` answers every member in `member_code` order and `GET /bss-products/v1/config/vocabularies/{class}/values/{value}` answers one, each gating on the kind's **`read`** grant — `plan_tier × read` for the tier set, `recognized_set × read` for the unit set, the same split P-D-90 arm 2 drew for the write half. **Tombstones are listed with `state: removed`**, because the add door refuses a removed code `DUPLICATE_CODE` and a list that hid them would contradict it; the platform baseline is seeded on the first read (P-D-104), so a set is never answered emptier than the next write would find it; a miss on the by-code door is a bare `404`, absent and out-of-scope indistinguishable. **No paging** (**P-D-170**) — these are closed vocabularies of tens. The by-code door answers an **`ETag`** (**P-D-174**): the `SHA-256` of the member's canonical rendering, a live row having no revision to name, and the two per-member write doors assert it back as `If-Match`. **The list answers none**, which is P-D-170's own rule kept rather than dropped — no door takes a set-level precondition, so a list tag would be the header with nobody to assert it that entry refused - `inst-rs-read`
3. [ ] - `p1` - Removal operand is uniform **across this slice's four sets**: **non-terminal published heads** (**P-D-89** confirms the reading and settles that a `draft` head does not block) — a member is removable — flipped to `removed` — when no non-terminal published head references it; frozen versions are self-contained copies, neither blocking removal nor touched by it (M2 fix); the transitions are `active → deprecated → removed`, and `removed → active` (as `deprecated → active`) re-lists the same identity through the same `GovernedLiveOp`, which is safe because the identity never changed (**P-D-47**, the donor's re-add re-activating a retired value); the pre-publish lint (P-D-02: informational) surfaces `deprecated`-member usage so operators see debt before refusal teaches them - `inst-rs-removal-operand`
...
- ~~**Which door writes `products_recognized_set`, at what path and under what grant?**~~ **Answered (P-D-90, 2026-09-01): `POST` on `/bss-products/v1/recognized-sets/{setKind}/members` and `…/members/{memberCode}/transitions`**, one route family under `recognized_set × write`. *P-D-175 moved that family to `/bss-products/v1/config/vocabularies/{class}/values` on 2026-09-14; P-D-90's own wording is kept above because a struck item records what was decided then, and the verb is written outside the span so a dead address is not harvested as a live declaration.* *The item's text stood as:* The only stated
...
- ~~**The scheduled publish lane never resolves `usageTypeRef`**~~ **Answered (P-D-157; struck at P-D-162, 2026-09-05): the lane resolves** — the runner carries the resolver, judges the ref under the gear's system principal, defers on `Unavailable` and releases its lane claim (`e51e989a9`). *The item's text stood as:* (measured 2026-09-05, P-D-146): `04`'s runner enters the publish transaction under `PreAuthorized` without the pre-transaction resolve, which lives in the REST door and needs the caller's `SecurityContext`. A scheduled publish of a metered SKU therefore skips P-D-131's fail-closed check, freezes no binding snapshot, and cannot reach the `deferred` arm §3.2's error taxonomy asks for. Owner `04`/`07`; the fix shape is the runner carrying the resolver and a service context, resolving before the transaction, and mapping `Unavailable` to a deferral. Carried in the feature's §7 as row 22.