...
2. [ ] - `p1` - On confirmation: the SKU is **forced `deprecated` immediately** — emitting `SkuDeprecated` (pricing's AC #82 trigger) with provenance `direct` for an operator-initiated retirement or `cascaded` when driven by a `CascadePlan` (the plan passes provenance through — H1 fix, 2026-08-25 review); adoption block starts now. `RetirementScheduled` is recorded with `effectiveAt` honoring the **configured** lead-time policy (§17.1, interim ≥ 30 days; `RETIREMENT_LEAD_TIME` otherwise), and `SkuRetired` is emitted **at initiation** with the full v1 payload `{skuId, fromVersion, reason, replacedBy?, effectiveAt}` — **`fromVersion` is the entity's `published_version` at the initiation instant** (defined here; it was defined nowhere in the set), and it stays truthful because **the head is closed to publishes for the whole lead window**: while a live retire intent exists the `PublishDoor` refuses version N+1 `RETIREMENT_PENDING` (item 16 of the 2026-08-26 review — only un-deprecation was guarded, so an operator could publish versions 8, 9, 10 after telling consumers the SKU retires from version 7). **Saves stay legal** — they touch the head, which no consumer reads (01 `inst-fd-save-txn`) — and an operator who genuinely needs to publish cancels the retirement first through the governed cancel that already exists (`mustMigrateBy` exists in the schema, never populated in v1 — C3) — the lead-time window IS the consumer handoff - `inst-rt-initiate`
