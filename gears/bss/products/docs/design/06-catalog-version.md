...
| `FreezeLedger` | Per-version ack state: participant → `acked \| pending \| released \| released(forced) \| not_frozen(forced)`. The five values of `products_freeze_ack.state` (§4) and nothing else — `released(forced)` is a **retention** marker and is not an ack (see `inst-rv-intent`) |
...
- **`products_freeze_participant`** — the governed registered set (live);
  **`products_freeze_ack`** — `(tenant_id, catalog_version_id, participant)` → `state ∈
  {pending, acked, released, released(forced), not_frozen(forced)}` with `acked_at` /
  **`released_at`** /
...
`cpt-cf-bss-products-fr-freeze-atomicity`, `cpt-cf-bss-products-fr-freeze-recovery`, `cpt-cf-bss-products-fr-freeze-participant-governance`,
...
**NFRs by id** — #3 `cpt-cf-bss-products-nfr-publication-propagation` and #4 `cpt-cf-bss-products-nfr-posting-safe-budget` (the two
