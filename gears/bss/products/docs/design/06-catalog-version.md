...
| C5 | Resolution for posted/contractual use is refused until `freezeComplete` — and, at `complete(forced)`, refused further until every forced participant has since frozen or released through its own door, or an operator auto-fallback opt-in is recorded (P-D-19; added to this row, because `complete(forced)` **is** a value of `freeze_state` (§4), so the row as written permitted exactly the posted resolution `inst-rv-intent` refuses); the timeout fails closed; browse may proceed; the caller **declares intent** | PRD `fr-freeze-atomicity` |
...
- **Where is P-D-19's per-version auto-fallback opt-in recorded, and through which door?** Three
  ...
  the action. *(All three lenses raised it independently — slice-06 first lens pass.)*
