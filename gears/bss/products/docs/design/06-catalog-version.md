...
| Name | Meaning |
|------|---------|
| `IncrementRequest` | A queued demand for addressability: `(source, lane ∈ {interactive, bulk}, requested_at)` — pricing's pending-ref request or an operator publish |
...
**Risks & open items**:
- **The composition-clear gate exemption is a design statement, flagged**: a system re-publish
  without approvals is P-D-02-consistent (the governed act happened pricing-side) but is the
  first approval-exempt publish in the gear — the slice review should judge it explicitly.
