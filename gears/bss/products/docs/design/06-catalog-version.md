...
Declared by [`../features/catalog-version.md`](../features/catalog-version.md) §2 as `cpt-cf-bss-products-flow-increment`.
...
4. [ ] - `p1` - SLO instrumentation: `requested_at → published_at` p95 ≤ 60 s / max 5 min **for the interactive lane; the bulk lane is measured from its window close under the same pair** (**P-D-67** — a batch whose window closes at the five-minute hard max cannot also publish within five minutes of its earliest request, so the unscoped clause was unsatisfiable for every bulk batch that ran to its bound); a pending request past the lane deadline raises `catalog_version_overdue` (the registry-side mirror of pricing's `commit_overdue`) - `inst-cv-slo`

### Build the snapshot

Declared by [`../features/catalog-version.md`](../features/catalog-version.md) §2 as `cpt-cf-bss-products-flow-snapshot`.
The steps below are this slice's and are the normative ones; the FEATURE carries the
actor, the scenarios and the boundary.
...
3. [ ] - `p1` - Canonical serialization + checksum over the manifest (the 01 engine-canonical discipline, which **P-D-28** states over **any named field set** — "sorted lexicographically by field name" — which orders **fields, not rows** — P-D-28 states in terms that it does not settle row-collection ordering, and **P-D-29**'s row rule names only the category-assignment and attribute-value sets — **P-D-80** generalizes it: a keyed collection sorts by its own key rendering, entry rows by `(entity_kind, entity_id)`, capture rows by `capture_kind`, the manifest rendering complete-set against a roster pinned beside the builder); references to sibling-gear content only (C4). Byte-identity probe: a re-resolution renders from the stored manifest, never re-collects - `inst-sn-checksum`
...
Declared by [`../features/catalog-version.md`](../features/catalog-version.md) §2 as `cpt-cf-bss-products-flow-resolve`.
The steps below are this slice's and are the normative ones; the FEATURE carries the
actor, the scenarios and the boundary.
...
Declared by [`../features/catalog-version.md`](../features/catalog-version.md) §2 as `cpt-cf-bss-products-flow-freeze`.
The steps below are this slice's and are the normative ones; the FEATURE carries the
actor, the scenarios and the boundary.

1. [ ] - `p1` - On `CatalogVersionPublished`, every participant in the **version's snapshotted set** owes an ack; `products_freeze_ack` records them; `freezeComplete` = all acked (AC #21). The ack door (`catalog_version × ack`, **`POST /bss-products/v1/catalog-versions/{catalogVersionId}/acks`** — routes declared by **P-D-67** on the increment door's pattern, S2S under the participant's identity; the release door is **`…/releases`**, P-D-18's own door) accepts a participant's ack **only from that participant's own service identity** (S2S claims — the registration check `PARTICIPANT_UNKNOWN` is membership, not authn); idempotent per `(version, participant)` - `inst-fz-ack`
...
Declared by [`../features/catalog-version.md`](../features/catalog-version.md) §3 as `cpt-cf-bss-products-algo-catalog-version-errors`.
...
Declared by [`../features/catalog-version.md`](../features/catalog-version.md) §3 as `cpt-cf-bss-products-algo-posting-safe`.
The steps below are this slice's and are the normative ones; the FEATURE carries the
Input, the Output and the boundary.
...
- **`products_catalog_version_counter`** — `(tenant_id)` → next id (the gapless allocator), **starting at `1`** (**P-D-67** — pinned so the dev-space ordering argument has a stated premise; pricing's dev-minted ids sit at ~10¹² and the sweep over `pricing_plan_revision.pending_version_ref` is **pricing's**, its module's own doc saying nothing there should outlive a real registry).
...
- **`products_freeze_participant`** — the governed registered set (live);
  ...
  **Six transitions and no others** (**P-D-60**): `pending → acked` (the ack door);
  ...
  `released_at` is **write-once** per registration. **The entry point is the increment transaction** (**P-D-67**): it seeds one row per `participant_set_snapshot` member at `state = pending`, so the ack door is an UPDATE whose row-existence is the membership check, and an empty ledger can no longer satisfy the summary. P-D-49's snapshot rule stays as the defensive belt.
...
- ~~**Nothing creates the ledger rows, and an empty ledger satisfies "all acked".**~~
  **Answered (owner call, 2026-08-31 — P-D-67): the increment transaction seeds them** — one row per
  `participant_set_snapshot` member at `state = pending` — so the ack door is an UPDATE whose
  row-existence is the membership check and the empty-ledger hazard dies; P-D-49 stays as the
  defensive belt. Original text: The only stated
...
- ~~**`participant_set_snapshot` is stored twice and only one copy is inside the checksum.**~~
  **Answered (owner call, 2026-08-31 — P-D-67): the capture-store copy is authoritative and inside
  the checksum; the version-row copy is a derived cache**, annotated exactly as `freeze_state` on the
  same row already is — one byte-identity, one convention. Original text: §4 puts
...
- ~~**What is `products_freeze_ack.state`'s transition table?**~~
  **Answered (owner call, 2026-08-31 — P-D-60): six edges, stated in §4, and one sub-question was
  ...
  acked, so **`pending → released` is admitted**. `released` is terminal; the entry
  point — the increment transaction's seeding — was settled the same day by **P-D-67**. Original text: Unstated: whether `pending` may go
...
- ~~**The manifest's row collections have no named sort key.**~~
  **Answered (owner call, 2026-09-01 — P-D-80): a keyed collection sorts by its own key
  rendering** — P-D-29's *"by the collection's own identifier"* generalized: entry rows by
  `(entity_kind, entity_id)`, capture rows by `capture_kind`, their stores' primary keys under the
  fixed tenant and version; and the manifest renders under `Absence::Null` against a roster pinned
  as a `const` beside the builder, `DIGEST_VERSION` governing changes. Original text: `inst-sn-checksum` rested on
  **P-D-28**, which states in terms that it orders fields and *not* rows; **P-D-29** supplies a row
  ...
  snapshot differently — against C4, AC #20 and §5's byte-identity flagship. Owner: was whoever owns
  01 §4.3's canonicalization pin (P-D-29), with this slice; **closed**. *(Raised by the slice-06 first lens pass.)*
- ~~**`staged_at` has no admitted writer.**~~
  **Answered (owner call, 2026-08-31 — P-D-67): the column is struck** — no admitted writer and no
  reader, the SLO measuring from `requested_at`; the `superseded` pattern again. Original text: A `staged_at` column implies the version row exists
