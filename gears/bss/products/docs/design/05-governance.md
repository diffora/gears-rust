...
1. [ ] - `p1` - Submission runs `MaterialityEvaluator` over the change set (head-row pending edits vs last published version; for a `GovernedLiveOp`, the op payload): **material** ⇒ quorum descriptor per C1 (`required = N`); **non-material** ⇒ `min(N, 1)` — so a tenant at `N = 0` publishes approver-less by policy, which is the point of P-D-11 and replaces the earlier "nothing publishes approver-less" interim. **Since P-D-180 the record does not say so, because at `N = 0` there is no record**: the act is authorized with nothing to consume and a `NULL` `approval_ref`, so nothing on the success path states *authorized at zero* until the platform audit capability lands (P-D-08 S1-S9, PRD §15 — the debt is P-D-180's and is filed there). The quorum descriptor and the materiality verdict are still formed and still recorded **on a submission wherever one is made**, and a submission stays legal at zero — which is what keeps the uncomposed-bundle acknowledgment reachable (`inst-cl-bundle-override`); first publish and every lifecycle transition **to `published`/`deprecated`/`retired`** are material at their **initiating human act** — the mechanical stages of an approved scheduled act (the `effectiveAt` flip, cascade legs) re-enter the gate only in 01's `PreAuthorized(approvalId)` mode, consuming nothing further (H-2, see `inst-gv-one-shot`); a bucket-iv-only re-publish is the non-material case (a re-publish is not a transition, 01's head-row model), and `draft→discarded` is ungated beyond authz (M-1) - `inst-gv-materiality`
...
1. [ ] - `p1` - **The Foundation's gate phase runs on any gated act, not on publish alone** (**P-D-30**) — which is what hosts this slice's gate on 04's un-deprecation edge: a transition door consumes the `satisfied` record exactly as the publish door does, and `inst-gv-one-shot`'s same-transaction flip binds identically there. Separately, **authorization is a pre-pipeline gate rather than a phase** (same call), run before 01's pipeline opens so a denied caller neither consumes an idempotency key nor writes a claim row; the denial's code and status are this slice's to declare, and remain open below. The Foundation gate (01 `inst-fd-governance-gate`) asks this slice: an `ApprovalRecord` in state `satisfied` (`superseded` being a value of the same column, §4) whose pinned revision equals the door's expected revision ⇒ yes — **matched first, so an act that was submitted still spends its record**; **no such record, an **`entity_publish`** subject and an effective quorum of zero ⇒ yes with nothing to consume and a `NULL` `approval_ref` (P-D-180) — the other five kinds keep demanding a record, `system_signal` because step 3 below puts its authority on the signal and `materiality_policy` because it is the subject that governs the count, which is neither a `Gate` consume nor a `PreAuthorized` verify but a third disposition, and carries no override acknowledgment** (so an uncomposed bundle is refused here and reaches publish only through a submission); a stage entering in 01's `PreAuthorized(approvalId)` mode naming a **`consumed`** record that
  ...
  **`APPROVER_ROLE_REQUIRED`** (§3.3); anything else ⇒ `APPROVAL_REQUIRED` (or `STALE_REVISION`
  ...
  operand. **The acknowledgment rides the record, so the record-free arm carries none** (P-D-180): an
  uncomposed `bundle` is the one act at `N = 0` that still requires a submission, its author
  supplying `author_override_ack` (P-D-68 arm 1, P-D-13), and the `satisfied` match being first is
  what makes that submission still spendable. The gate never re-evaluates materiality at publish — the verdict was fixed at submission (a policy change between submit and publish neither re-judges nor voids a pending approval; the pricing evaluated-once rule, adopted) - `inst-gv-gate`
...
| `catalog_version × ack`, `× release` | `POST /bss-products/v1/catalog-versions/{catalogVersionId}/acks` and `POST /bss-products/v1/catalog-versions/{catalogVersionId}/releases` (**P-D-67** — S2S under the participant's identity; `release` is **P-D-18**'s door) | 06 |
| `catalog_version × read`, `× force_complete` | `GET /bss-products/v1/catalog-versions/{catalogVersionId}` (06's resolver, **`× read`**; paired here on lint 3's first run, P-D-151), `GET /bss-products/v1/catalog-versions/{catalogVersionId}/diff/{otherCatalogVersionId}` (06's diff, **`× read`**) and `GET /bss-products/v1/read/freeze-status` (08's dashboard, **`× read`**) — both paired at P-D-159, `GET /bss-products/v1/bulk/exports?catalogVersionId=` (09's export door) spends **`× read`** (**P-D-50**); **`× force_complete`** is `POST /bss-products/v1/catalog-versions/{catalogVersionId}/force-completions` (**P-D-67** — the operator ceremony's door) | 06 |
...
| `category × read\|write` | **`× read`** is `GET /bss-products/v1/categories` (**P-D-181** — 02's read door and the grant's only spender). *Withdrawn there*: P-D-50's cell named `GET /bss-products/v1/browse…` for this action, and the browse handler asks for `product|sku × read` alone — the facet half is a count over rows the caller may already read, and it hands out rendered paths, never a `categoryId`. **`× write`** is doored by **P-D-106** — `POST /bss-products/v1/categories`, `POST /bss-products/v1/categories/{categoryId}/operations` (the taxonomy ops) and `PATCH /bss-products/v1/categories/{categoryId}/attribute-values` (the live-value door) | 02 |
...
| `recognized_set × write`, `plan_tier × write` | `POST /bss-products/v1/config/vocabularies/{class}/values` and `POST /bss-products/v1/config/vocabularies/{class}/values/{value}/transitions`, `POST /bss-products/v1/config/vocabularies/{class}/values/{value}/label` (P-D-146; paired at P-D-159) (**P-D-90** — one route family, the grant chosen by the class token; both spelled in full because an elided span is invisible to a route census; **P-D-175**, 2026-09-14, moved all three off `/recognized-sets/{setKind}/members…` with their path parameters, and left this grant column alone: the two gears converged on the door's address, not on who may open it) | 03 |
| `recognized_set × read`, `plan_tier × read` | `GET /bss-products/v1/config/vocabularies/{class}` and `GET /bss-products/v1/config/vocabularies/{class}/values/{value}` (**P-D-170**). Split by kind exactly as the write half is: a tenant that lets an operator see the tier ladder has not thereby let them see the metering-unit set, and a roster readable at a finer grain than it is written at would make the write grant the weaker of the two | 03 |
| `approval × submit\|read\|decide` | `GET /bss-products/v1/approvals?state=pending` (this slice's own pending-queue door) spends **`× read`** (**P-D-50**); `POST /bss-products/v1/approvals` spends **`× submit`** and `POST /bss-products/v1/approvals/{approvalId}/decisions` spends **`× decide`** (**P-D-120**, strand B, `c1b86fcbb`; named here by **P-D-137**) **Eligibility is this pair's alone since P-D-177 (2026-09-21):** the decide door no longer also demands a `catalog_admin` / `finance_reviewer` token claim, because P-D-134 row 25 routed that claim to the platform-identity owner and it has not arrived — so the check refused every principal on every deployment. The quorum settles under `BaseRoleSet::AnyDecider`; the finance predicate is unchanged and still refuses a finance-material record that is met numerically and not on the lens. | 05 |
...
| `breakglass × elevate` | `POST /bss-products/v1/breakglass-sessions` (**P-D-120**, strand B, `c1b86fcbb`; named here by **P-D-137**; the pre-pipeline elevation gate reads the session id from a header — P-D-133) | 05 |
| `scheduled_transition × write\|cancel\|read` | **P-D-134**: `GET /bss-products/v1/scheduled-transitions` (`× read`), `GET /bss-products/v1/read/deferred-intents` (08's dashboard, `× read`; paired at P-D-159) and `POST /bss-products/v1/scheduled-transitions/{id}/operations` with `op: cancel` (`× cancel`) — 04's doors, strand C's build, **built** (`5da022f6f`); `× write` is not minted, the retire doors writing the rows under `sku × write` / `product × write` (P-D-135) | 04 |
| `product × write`, `sku × write` (04's lifecycle doors) | `POST /bss-products/v1/{products\|skus}/{id}/deprecate`, `POST /bss-products/v1/{products\|skus}/{id}/undeprecate`, `POST /bss-products/v1/{products\|skus}/{id}/retire`, `POST /bss-products/v1/{products\|skus}/{id}/retire/cancel`, `POST /bss-products/v1/products/{id}/retire/resume` (P-D-114 row 11) — the lifecycle acts spend the authoring pair (**P-D-135**); paired here on lint 3's first run (P-D-151), the resume at P-D-159 | 04 |
| `freeze_participant × write` | `POST /bss-products/v1/freeze-participants` (**P-D-67** — the governed set write) | 06 |
| `reference_signal × post`, `reference_producer × write`, `sku × correct` | `POST /bss-products/v1/reference-watermarks` (**`× post`**); `POST /bss-products/v1/reference-producers` and `POST /bss-products/v1/reference-producers/{producer}/retirements` (**`reference_producer × write`**); `POST /bss-products/v1/skus/{skuId}/corrections` (**`sku × correct`**) — 07's doors, declared there since P-D-147 and paired here on lint 3's first run (P-D-151) | 07 |
...
| `audit × read\|export` | `GET /bss-products/v1/read/delivery-state` (08's delivery-state dashboard, `× read`; paired at P-D-159); `× export` has **no route declared** (M-4 fix) | 05 |
...
- **`products_approval`** — `approval_id` (PK) · `tenant_id` · subject `(kind, ref)` — `kind ∈ {entity_publish, governed_live_op, system_signal (P-D-14), sku_correction (07), bulk_batch (09)}` · pinned
  ...
  subject; a new submission explicitly supersedes the open one (L-4). **A door that opens a unit on its caller's behalf therefore opens one only where the subject holds no open record** (**P-D-173**, `03`'s three set doors today, which answer `202` with the unit and write nothing): submitting on every unauthorized call would discard the approvals already cast on a pending unit. Such a door spends its own write grant, not `approval × submit`, and opens the unit under the act its route names rather than one read off a payload. **A live-op door may carry a second, stronger staleness operand beside the envelope's expected state** (**P-D-174**: `03`'s two per-member doors assert the member's own `ETag` as `If-Match`); the envelope's state pin stays, because after P-D-173 it is part of what the approval unit was agreed against and cannot be re-derived at apply time, and a stale tag raises the same `STALE_LIVE_OP` rather than a new code. Append-only after
  finalization.
