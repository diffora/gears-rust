...
| `approval × submit\|read\|decide` | `GET /bss-products/v1/approvals?state=pending` (this slice's own pending-queue door) spends **`× read`** (**P-D-50**); `POST /bss-products/v1/approvals` spends **`× submit`** and `POST /bss-products/v1/approvals/{approvalId}/decisions` spends **`× decide`** (**P-D-120**, strand B, `c1b86fcbb`; named here by **P-D-137**) | 05 |
