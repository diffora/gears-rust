...
| `catalog_version × read`, `× force_complete` | `GET /bss-products/v1/catalog-versions/{catalogVersionId}` (06's resolver, **`× read`**; paired here on lint 3's first run, P-D-151), `GET /bss-products/v1/bulk/exports?catalogVersionId=` (09's export door) spends **`× read`** (**P-D-50**); **`× force_complete`** is `POST /bss-products/v1/catalog-versions/{catalogVersionId}/force-completions` (**P-D-67** — the operator ceremony's door) | 06 |
...
| `product × write`, `sku × write` (04's lifecycle doors) | `POST /bss-products/v1/{products\|skus}/{id}/deprecate`, `POST /bss-products/v1/{products\|skus}/{id}/undeprecate`, `POST /bss-products/v1/{products\|skus}/{id}/retire`, `POST /bss-products/v1/{products\|skus}/{id}/retire/cancel` — the lifecycle acts spend the authoring pair (**P-D-135**); paired here on lint 3's first run (P-D-151) | 04 |
