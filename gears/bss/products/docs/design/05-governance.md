...
| `recognized_set × write`, `plan_tier × write` | `POST /bss-products/v1/recognized-sets/{setKind}/members` and `POST /bss-products/v1/recognized-sets/{setKind}/members/{memberCode}/transitions`, `POST /bss-products/v1/recognized-sets/{setKind}/members/{memberCode}/label` (P-D-146; paired at P-D-159) (**P-D-90** — one route family, the grant chosen by `setKind`; both spelled in full because an elided span is invisible to a route census) | 03 |
| `recognized_set × read`, `plan_tier × read` | `GET /bss-products/v1/recognized-sets/{setKind}` and `GET /bss-products/v1/recognized-sets/{setKind}/members/{memberCode}` (**P-D-170**). Split by kind exactly as the write half is: a tenant that lets an operator see the tier ladder has not thereby let them see the metering-unit set, and a roster readable at a finer grain than it is written at would make the write grant the weaker of the two | 03 |
