...
| **Cross-tenant DSAR reach: a platform grant, or one request per tenant?** | Design 10 `inst-er-export`'s DSAR clause enumerates a principal's rows **across tenants** "under the platform DSAR grant". No such grant exists in 05's RBAC catalog, which asserts every door names its pair; and a tombstone is a write, while 05 C5 refuses any write under elevation and `DESIGN.md`'s `constraint-tenant-isolation` limits v1 break-glass to read/audit-export. Two non-equivalent ways out: a platform-plane `compliance × erase` grant outside the tenant elevation path, or v1 erasure per tenant with a principal spanning tenants needing one request each. Until it is decided, `inst-er-erase`'s "erasure completes" claim holds only within one tenant. *(Owner: Architecture + Legal. Filed from design 10 §6.)* | TBD |
