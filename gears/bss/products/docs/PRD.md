...
| v1 launch criterion: a registered reference producer | D-47's flip guard defers every SKU retirement as `no_producers` while the producer registry is empty (C6, fail-closed), and the `retirement_held` alert names the hold after `retirement_held_alert_hours`; pricing — or whichever producer owns the watermark — MUST be registered as a reference producer before the first retirement is scheduled. **P-D-138 (2026-09-04, the owner)**; no code change. | binding |
