...
> **No goal above carries a measurable target yet**: the metric, baseline and target for each business outcome are deferred on the same footing as the numeric NFR values (§7.1, §14) — the goals are binding **design intent** until they are set. *(Owner: Product, with the BSS Program Lead.)*
...
- **Plan, Price, PriceWindow, PriceList, Bundle composition, add-on rules, billing descriptors (invoice line template / tax category / GL code — **the codes too**, and not only the descriptor: **P-D-169**, 2026-09-14, withdrawing P-D-131's carve-out of 2026-09-03), plan lifecycle/migration/grandfathering, plan publish validation & approval, price history** → `PRD-plan-price-modeling-202605281200`.
...
**Role**: Reviews and approves finance-material catalog changes and tax-category dictionary definitions; second approver under the two-person rule for finance-bearing changes. **This registry computes no finance-material operand of its own** (P-D-169 took both accounting codes out of it, and `PlanTier` is Product's): a change is finance-material here only when its submitter declares it so.
...
| Catalog approval & publishing workflow | `p1` | Approval-gated publish with the **configured approver quorum** (default 1, floor 0 — P-D-11, P-D-177) above a typed materiality policy; approvals pinned to the approved revision; idempotent mutation boundary. Categories & attribute definitions are governed live entities. |
...
- **Tax determination/statutory invoicing and revenue recognition** — Tax Engine / Billing / Finance. Catalog supplies **neither code** (P-D-169): tax-category assignment is a price row's and the GL code a plan descriptor's. What Product Catalog owns is the tax-category **dictionary**.
...
#### ~~Stable accounting codes on SKU~~ — withdrawn (P-D-169)

**Withdrawn 2026-09-14 (P-D-169). Neither code is a SKU field, and this registry holds neither vocabulary.** §2.1's *"owned elsewhere and **MUST NOT** be re-specified here"* governs both again, and each has a named home on the other side of the seam:

- **Tax category** — assignment *"belongs solely to Pricing's `price.tax_category_ref`, not a SKU field or region default"* (the tax-ownership amendment at the head of this document, 2026-09-10; the merge that carried it in, and the six sentences of this PRD it had not reached, are **P-D-168**'s record). What Product Catalog owns is the **dictionary** — stable code and display name — which this registry does not yet expose; the read `ProductCatalogClientV1::list_tax_categories` asks for is open work, not a SKU field.
- **GL code** — the tenant's GL vocabulary is `pricing_gl_code_taxonomy`, declared at `PUT /bss-pricing/v1/config/gl-codes` and checked against a **plan revision's** descriptor at plan publish (pricing D-356, 2026-09-09). Nothing downstream ever read a GL code off a SKU: `CatalogSku` carries none and the `SchemaPin` (§14) pins none.

*Stood as:* "Setting tax-category and GL codes **MUST** persist them as stable codes and **validate each against a configured recognized set** (owner = Finance, deprecate-then-remove governance). The codes are **required at publish for `product`/`service`-type SKUs**; a SKU published without a required code is unpostable and **MUST** be rejected. The system **MUST NOT** compute tax or post to GL (codes only)." Its rationale — *"validating codes at authoring prevents unpostable SKUs surfacing weeks later at ERP export"* — is unchanged in substance and is now discharged where the code is authored.

**A consequence to state rather than leave implicit:** a GL code is now one value per **plan revision**, not one per SKU. A tenant that must post two SKUs of one plan to different revenue accounts cannot express that today; the place to fix it would be a price-row field in pricing, never a return of this requirement.
...
A definition carrying a value on any non-terminal head **MUST NOT** be removed; the path is deprecate-then-remove (**P-D-131**, 2026-09-03 — the design's operand, adopted here).
...
- [ ] `p2` - **ID**: `cpt-cf-bss-products-fr-clone`
...
- **Authentication** — the platform identity provider (design 01 §1.3's `cpt-cf-bss-products-actor-oss-ams-idp`, §1.8's consumed dependency); the registry consumes the resulting principal and tenant claims and specifies no mechanism of its own. Authorization *is* in scope (§6.7, §6.8).
- **Documentation requirements** — project-level baseline; this PRD states no doc-coverage or doc-freshness target of its own.
- **Support requirements** — project-level baseline; no gear-specific support tier, on-call rota or response target is set here.
- **Deployment requirements** — project-level baseline; this PRD pins no release cadence, rollout topology or environment matrix of its own.
- **User-experience goals** — project-level baseline; the registry is a backend service (§9) and every consuming surface owns its own UX.
...
| **Security** | Complete tenant/brand/region isolation (deny-by-default), RBAC, the configured approver quorum for material changes (default 1, floor 0 — its predicates fixed), time-boxed break-glass for cross-tenant access, retention/erasure reconciled with immutable audit, minimal PII in events. | Cross-tenant leakage is a commercial risk; unauthorized publish is a fraud risk; standing super-access and unbounded retention are compliance risks. |
...
**Alternative Flows**:
- **Incomplete at publish**: publish rejected fail-closed (missing required fields / `PlanTier`).
...
**Alternative Flows**:
- **Cascade with referenced children** (EOL-requiring being the post-v1 subset — **P-D-132**, 2026-09-03): those children listed and left un-retired; parent stays non-`retired` with deferred-retire intent tracked.
...
- **Then** mutability MUST be classified by lifecycle state: structural identity immutable (remedied only by retire + clone); `type`/metering-unit immutable-but-correctable via the fresh-zero path; material-but-mutable (`PlanTier`/`sellable`) via a new published version under governance; other fields via a new version
...
**11. ~~Stable accounting codes on SKU~~ — withdrawn with `fr-accounting-codes` (P-D-169)**
- Neither code is a SKU field. What stood here is discharged on the other side of the seam: tax-category assignment at a price row (the 2026-09-10 amendment), the GL code at a plan revision's descriptor against `pricing_gl_code_taxonomy` (pricing D-356).
...
- **And** retiring a Product with non-`retired` SKUs MUST require confirmed cascade-retire (partial by design, recording `direct` vs `cascaded`); referenced children (EOL-requiring being the post-v1 subset, **P-D-132**) are listed and left un-retired; never-published children auto-`discarded`
...
- **When** the change is material (touches `PlanTier`/metering-unit, a lifecycle transition, a Category create/rename/re-parent/retire/delete/set-default (**P-D-182**), a material attribute-definition change, or exceeds the configured affected-entity count)
- **Then** the system MUST enforce the tenant's configured approver quorum `N` (typed policy, default 1, floor 0 — P-D-11, P-D-177): `N` distinct approvers, each distinct from the author and holding CatalogAdmin or FinanceReviewer; a declared finance-material change MUST include ≥ 1 FinanceReviewer among them at every `N ≥ 1`; at `N = 0` the predicate MUST NOT be imposed (no principal could satisfy it, and the gate would then refuse forever) and the record MUST carry an explicit unsatisfiable-predicate marker instead
...
- **And** version-liveness MUST be sourced from per-version freeze-registration records — **acked-and-not-yet-released**, evaluated over the version's **`participant_set_snapshot`** so a snapshot member with no registration row still holds the version (**P-D-49**), the release arriving through the `catalog_version × release` half of the freeze-participant contract (P-D-18, closing the §15 choice in favour of freeze-registration over a `(catalogVersionId, producer)` count) — never the SKU-level count alone; a process that would orphan a live reference MUST fail closed with an alert
...
- Recognized-set owners (metering units → Product + Rating; `PlanTier` → Product) seed and govern their sets; the registry validates against them. The two accounting sets were here until P-D-169 and are not this registry's.
...
| ~~UsageType deletion vs published declarations~~ | **Answered on the registry side (P-D-131, 2026-09-03)**: a deleted type is the correction door's `unresolvable_target` arm. **Owed to the collector's PRD owner**: a `UsageType` with live bindings is never hard-deleted, only tombstoned with a signal. *Stood as:* The collector's delete-RESTRICT counts only its own usage records — a UsageType referenced by a **published** metering-unit declaration can be deleted, leaving a sold-but-unrateable meter (rating quarantines, correctly but indefinitely). Negotiate with usage-collector: (a) extend the delete guard to registry-published declarations (needs a reference signal from us), or (b) a deletion event consumed here + by pricing's `meter_binding_divergent` remediation path. Until decided, quarantine is the fail-safe and the pre-publish lint warns on `deprecated`/dangling units. *(Owner: Product + usage-collector + pricing; raised by PR #14 review.)* | **answered / collector obligation open** |
...
| ~~Cross-tenant DSAR reach: a platform grant, or one request per tenant?~~ | **Answered by P-D-50: per tenant, and no platform grant is minted.** A DSAR erasure enumerates and tombstones the principal's rows in the **requesting tenant only**, audited in-tenant; a principal appearing in several tenants needs one request per tenant. The platform-plane `compliance × erase` alternative was refused because it creates a write path outside tenant elevation, which `DESIGN.md`'s `constraint-tenant-isolation` and 05 C5 both forbid, and the gear will not build one on an assumption about what Legal requires. **The contingency is recorded, not closed**: should Legal rule per-tenant erasure incomplete, the platform grant becomes mandatory and is a post-v1 change rather than a gap in the rule. Design 10 `inst-er-export`'s L5 clause and `inst-er-erase` now state the bound. *(Owner: Architecture + Legal.)* | **** |
| **Is `products_pii_allowlist` itself a PII store?** | By construction it holds person-named strings and its entries are exportable for the Legal review, yet only the identity map carries the posture "excluded from every export except the compliance surface, encrypted at rest". The same answer decides whether the allow-list's justification and sign-off fields belong inside the content-PII write block (design 10 §3.1 deliberately did not add them). *(Owner: Legal with the data-protection owner. Filed from design 10 §6.)* | TBD |
| **Is `{source}-copy-N` a legal `skuCode`, and what is the code's format?** | `fr-identifier-contract` requires the code to be "short, fixed-format" and no document pins the format, so neither adjective has an operand — while design 11 `inst-cn-identity` derives a clone's suggestion as `{source}-copy-N`, and a clone of a clone compounds the suffix. Needs the format pinned, after which the suggestion either conforms or is replaced. *(Owner: the PRD owner, with 01's `inst-fd-mint-id`. Filed from design 11 §6.)* | TBD |
...
| ~~Recognized tax-category / GL-code sets: owner + add / de-list workflow~~ | **Closed by P-D-169, 2026-09-14**: neither set is this registry's. The GL vocabulary is `pricing_gl_code_taxonomy`; the tax-category dictionary is Product Catalog's to expose, and is not a recognized set of SKUs. | **closed** |
...
| Event-bus transport contract owner + home (ordering, at-least-once, DLQ retention) | Registry states its requirements; the contract is owned by Common Core / Events & Audit. **The two ordering specifics the registry could not pin (raised by the slice-01 review) are the broker's since P-D-47:** the gear derives no partition of its own and sets no `partition_key`, so the broker's own default governs, and the consequence this PRD requires is that no partition-count change can re-route a tenant mid-stream — the reordering AC #28 forbids. The partition-selection mechanics are the broker's contract and are carried in design 01 §4.4, not here. What stays open here is what the row always named: the transport contract's owner and home. *(Owner: Architecture/Eng — Common Core.)* | TBD |
| Platform audit **sealing** capability: owner + home + delivery | **Decided here (P-D-08): tamper-evidence is not built per gear.** The registry ships the complete append-only trail plus a reserved, unwritten sealing seam (`seal_state`/`chain_id`/`seq`/`prev_hash`/`row_hash`) and states the requirements the platform capability MUST satisfy — P-D-08 **S1–S9** (construction, segmentation, never-on-the-mutation-path, verification cadence, WORM anchoring, residency, erasure compatibility, retention, coverage). No such platform gear exists today, and the ledger and pricing each built their own chain; that replication stops here. Until the capability ships, audit immutability is the trigger whitelist on both engines and nothing cryptographic (**P-D-46**) — an edited audit row is undetectable (see §16). **The capability owes a second thing since P-D-180 (2026-09-22): the record of a record-free authorization.** A governed act at effective quorum zero now writes **no** approval row, so the sealed trail MUST carry `required: 0`, `quorum_reduced: true`, the gate's own reason string, the actor and the subject at its pinned revision — otherwise a quorum-zero publish is traceable only as a publish event beside a `NULL` `approval_ref`, which cannot be told apart from an older row or from a defect. That is the interim state today and it is **insufficient**, accepted by the owner as a debt rather than as a design. *(Owner: Architecture / Common Core — Events & Audit.)* | TBD |
...
| NFR workshop: named DRI, held within 2 weeks of approval, SLO table ratified | Targets are binding design targets until then. *(Owner: BSS Program Lead.)* **P-D-132 (2026-09-03): the Program Lead names the DRI and the date; every interim number in the design set (P-D-107, 113, 118, 121, 127, 132) is a binding design target until the table is ratified.** | TBD |
| v1 launch criterion: pricing's freeze ack | Pricing's `CatalogVersionPublished` ack MUST be registered and observed on one real catalog version before the first posted use; until then every version is posting-unsafe by construction, which is the freeze protocol's intended loud state. *(Owner: the product owner — **P-D-132**, 2026-09-03.)* | decided |
| v1 launch criterion: a registered reference producer (P-D-138) | D-47's flip guard defers every SKU retirement as `no_producers` while the producer registry is empty (C6, fail-closed), and the `retirement_held` alert names the hold after `retirement_held_alert_hours`; pricing — or whichever producer owns the watermark — MUST be registered as a reference producer before the first retirement is scheduled. **P-D-138 (2026-09-04, the owner)**; no code change. | binding |
...
| ~~**Where does the audit row's correlation id come from?**~~ | **Answered (P-D-118, 2026-09-03; landed 2026-09-04): the W3C trace id.** The gear reads it off the ambient trace context (`infra::events::correlation_id`) and stores it as `text`; a request carrying none — and every background act — stores `NULL` by design. No request header is read and nothing is gear-minted. *Stood as:* Design 01 §4.4 lists a correlation id on every audit row without the `nullable` marker its optional columns carry, and §6 repeats it as an envelope operand, while 01 §2's door contract names only `If-Match` and `Idempotency-Key` and no rule mints or reads one. §6 above requires it for break-glass audit. Needs: request header, W3C trace context, or gear-minted — and what a request carrying none stores. *(Owner: Platform, with Design. Filed from design 01 §6.)* | **answered** |
...
|--------|-----------------------------|-------------|
| Materiality threshold | Any material-field change requires the tenant's configured approver quorum `N` (**default 1**, **floor 0** — P-D-11, P-D-177); affected-entity-count trigger ≥ 10; a single-entity non-material change requires `min(N, 1)`. `N` is reachable only by explicit configuration (absent ⇒ default, so `0` is never reached by omission), its initial value comes from tenant provisioning, and every later change to it is material under the then-current quorum. The FinanceReviewer predicate is **not** part of this policy — it governs who, not how many | Finance |
| ~~Recognized tax-category & GL-code sets~~ | **Withdrawn (P-D-169)**: no interim policy is owed here — neither set is this registry's. | — |
| PlanTier taxonomy seed | Seeded with **`standard`** (**P-D-131**, 2026-09-03) + operator-defined tiers; tier identity is a stable code, rename = display-only | Product |
...
| Break-glass post-hoc review SLA | **24 hours** after session expiry, interim (`breakglass_review_sla_hours`); the obligation alert fires when it lapses — **P-D-133**, 2026-09-04 | Security/Architecture |
| Held-retirement alert threshold | `retirement_held` fires for a deferral older than **72 hours**, interim (`retirement_held_alert_hours`) — **P-D-133**, 2026-09-04 | Ops |
