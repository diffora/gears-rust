...
- **Then** the system MUST enforce the tenant's configured approver quorum `N` (typed policy, default 2, floor 0 — P-D-11): `N` distinct approvers, each distinct from the author and holding CatalogAdmin or FinanceReviewer; a finance-material field MUST include ≥ 1 FinanceReviewer among them, and where the configured `N` makes that predicate unsatisfiable the record MUST state it rather than pass silently
