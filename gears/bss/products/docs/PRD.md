...
- **And** a publish whose sole content is a system-owned flag cleared by an inbound governed signal MUST be recorded under subject kind `system_signal` — auto-satisfied against the signal reference, refused on a dirty head, never exempt from the record (P-D-14)
