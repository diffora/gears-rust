...
been a stale attribution in the crate that the same wave corrected. The predicates it describes
...
for the parent and every child leg the reversal touches.
...
The table has **36 rows**, and the arithmetic is stated rather than left to a reader: §6's eighteen
are rows **1–18**, in the slice's own order; rows **19–36**, marked `**`, were raised by this
document's own reading of the crate — seven before the review, ten by it, and one by the
implementation of `inst-lc-deprecate` — and are new.
...
| 36** | **No document declares the door the deprecation act arrives through.** `design/04` §3 `inst-lc-deprecate` is a `p1` instruction — an operator act stamping provenance `direct`, cascading onto the children — and `09 inst-bl-lifecycle` calls its per-row caller *"the ordinary 04 policy doors"* and *"each row's 04 transition door"*. But the set declares exactly **three** entity-scoped act spans, all in `01` and `11`: `.../{id}/publish`, `.../{id}/discard` and `.../{id}/clone`. There is no `deprecate` span, no grant named for one, and no §9 interface entry. The crate now performs the act and registers `POST /bss-products/v1/products/{id}/deprecate` under `product × write` on the discard door's own reasoning, because a normative act with no carrier is inert — but that path and that grant are **the crate's, not the set's**, which is why `dod-deprecation-provenance` and `dod-deprecation-cascade` carry bare markers rather than ticks. Either the span is declared (and the route census moves from seventeen to eighteen), or the act rides a door that already exists | `dod-deprecation-provenance`, `dod-deprecation-cascade` | the design-set owner with 01 — the span roster and the grant are both 01's and 05's | 
