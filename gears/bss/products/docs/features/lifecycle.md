...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-activation-runner`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-runner-failure-posture`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-retirement-initiation`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-replaced-by`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-cascade-parent-path`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-deferred-intent`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-no-orphan`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-scope-containment-final`
...
**Ticked 2026-09-04 under P-D-109, clause by clause (P-D-134, corrected).** *Flat value-set subset of the
parent's, over restrictions*: `domain::containment::contains` — an unrestricted parent contains
every child, an unrestricted child needs an unrestricted parent, non-empty sets by subset.
*Evaluated on save*: `skus::run_save` calls `recheck_parent_containment` and `products::run_save`
calls `check_children_stay_contained` — `a_sku_save_widening_out_of_the_parents_scope_is_refused`.
*Re-evaluated on publish*: `skus::run_publish` calls `recheck_parent_containment` —
`a_publish_whose_parent_narrowed_out_of_band_is_refused_scope_not_contained`. *Refusing
`SCOPE_NOT_CONTAINED`*: `scope_not_contained_domain_err`, one translation for both doors, probed
at create by three cases. **P-D-134's first text said the save clause had no call site; that was
a grep of the primitive (`check_containment`) and not of the wrapper's callers, and this DoD's own
body had already named the four sites.** §7 row 21 defeats no clause: nothing is left to replace.
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-scope-narrowing`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-lifecycle-errors`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-lifecycle-events`
...
- [x] `p1` - **ID**: `cpt-cf-bss-products-dod-lifecycle-audit`
...
|---|---|---|---|
| ~~1~~ | **Deferred flips can hold indefinitely** while a producer watermark stays stale — correct by constraint but operationally invisible without `08-read-models`' surfacing and the `retirement_held` alert; the fail-safe tripwire bounds the corrections debt, nothing yet bounds held retirements. Candidate for an operator report, not a new mechanism *(P-D-109: a live question, not a blocker — this row asks how a behaviour that is *correct by constraint* is surfaced, which `08` part-owns.)* **Answered (P-D-133, 2026-09-04, the product owner): P-D-126's deferred-intent dashboard plus `retirement_held_alert_hours`, 72 interim; ops owns the runbook.** | ~~`dod-deferred-intent`~~ | **struck** |
...
| ~~3~~ | **Owed: the instruction row registering the create-door live-retire-intent validator.** Whose it is has been settled — this feature's, its operand being the live retire intent in `products_scheduled_transition`, which this feature owns. **No instruction row registers it yet, and until one does nobody builds the guard** — leaving the hole open: a draft SKU created under a Product with a live retire intent defers that retirement indefinitely *(P-D-109: a live question, not a blocker — this row asks for an **owed instruction row**, not for a store clause.)* **Answered (P-D-134, 2026-09-04): written** — `inst-rt-create-guard` in `design/04` §3; measured that no create or save door reads `find_live_retire_intents` today, so the guard is C's build. | ~~`dod-cascade-plan`~~ | **struck** |
~~| 4~~ | **struck**
...
| 6 | **EOL (post-v1) will need** the subscriptions-lifecycle AC by number, the consumer-ack contract, and a suspension event — the schema field is already vN-compatible *(P-D-132, 2026-09-03: **post-v1, explicitly** — nothing decided until Subscriptions' AC exists; `dod-eol-lockout` re-priced p3.)* | `dod-eol-lockout` | the PRD owner with Subscriptions |
| ~~7~~ | **Does the create-door retire-intent validator also register on the save door?** `01-foundation`'s save instruction is the only door that may change a SKU's `product_id`, so a draft SKU can be **re-parented** under a retire-pending Product by a door neither arm covers — the hazard `inst-fd-containment-retire-intent` itself describes *(P-D-109: a live question, not a blocker — this row asks whether one further door also registers the validator — a coverage gap, not a defect in the host.)* **Answered (P-D-134, 2026-09-04): yes** — the save door may set `product_id` before first publish, so the guard registers on both doors. | ~~`dod-cascade-plan`~~ | **struck** |
...
| ~~16~~ | **Does `leave-and-list` cover referenced children or only EOL-requiring ones?** The plan instruction scopes the arm to "children whose flip guard cannot clear — referenced SKUs"; the PRD and its AC both scope it to "EOL-requiring children left un-retired", and EOL is disabled in v1 — so **on the PRD's wording the arm has no v1 population at all** **Answered (P-D-132, 2026-09-03, the product owner): referenced children** — the design's operand; EOL-requiring is the post-v1 subset, and the PRD's two sentences are widened. | ~~`dod-cascade-plan`~~ | **struck** |
...
| ~~21~~** | **What operand does this feature replace in the containment check?** The slice describes `SCOPE_NOT_CONTAINED`'s "final semantics" as registered here, replacing the operand inside `01-foundation`'s identity phase. But `domain::containment` **already implements the final restriction-based rule verbatim** — unrestricted parent contains every child, unrestricted child needs an unrestricted parent, ordinary subset between non-empty sets — wired at three call sites. If nothing is left to replace, the obligation is a no-op and the slice's C5 wording is stale **Answered (P-D-134, 2026-09-04): nothing is replaced** — `domain::containment` ships the final rule at three sites and C5's wording is corrected; **and the DoD ticks** — corrected 2026-09-04: the save clause *is* wired (`run_save` → `recheck_parent_containment`; the Product `run_save` → `check_children_stay_contained`), the first text having grepped the primitive rather than the wrapper's callers. | ~~`dod-scope-containment-final`~~ | **struck** |
...
| ~~25~~** | **Seven broker events against eight shipped payload types, two body shapes and a roster test that is exact in both directions.** Measured 2026-09-04 after 2.4: this feature now declares **eight** tokens (`THE_LIFECYCLE_PAIR` 2 + `THE_LIFECYCLE_REST` 6, including `ProductRetirementEffective`). `SCHEMA_REFS` carries **30** tokens after D's merge (foundation 8 + lifecycle 8 + set 3 + bulk 1 + taxonomy 8 + retention 2). The lifecycle subset is still the eight declared tokens. The lifecycle subset of `SCHEMA_REFS` equals the eight declared tokens — no delete to reconcile. `THE_EIGHT` is still the Foundation eight and was **not** reddened: the roster test unions named lists. Body shapes are four — `EventBodyCore`, `PublishedEventBody`, `DeprecatedEventBody`, `RetiredEventBody`. The row's "seven vs eight" and "first add reddens THE_EIGHT" no longer hold. | ~~`dod-lifecycle-events`~~ | **struck** |
