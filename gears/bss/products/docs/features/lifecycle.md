...
| 18 | **Pointer**: which slice declares `PARENT_NOT_PUBLISHED` is open in `01-foundation` §6. This feature asserts one arm — "named in 01, registered here" — and **the answer is not this feature's to give** *(P-D-109: a live question, not a blocker — this row asks **where** a code is declared, which is a pointer.)* | `dod-lifecycle-errors` | the owner of that open item |
