...
/// The three configured ceilings `METADATA_LIMIT` names (**P-D-107** arm 1).
///
/// Unlike [`TaxonomyLimits`] these are **not** `Option`: `dod-metadata-door`
/// requires *"configured caps on key count, key byte length and value byte
/// length"* enforced at the door, and `ProductsConfig` refuses a zero at boot,
/// so a caller always has three numbers. An absent cap would be a fourth
/// state the requirement does not have.
#[domain_model]
#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub struct MetadataLimits {
    /// Maximum keys the **resulting** map may hold.
    pub keys: u32,
    /// Maximum bytes in one key.
    pub key_bytes: u32,
    /// Maximum bytes in one value.
    pub value_bytes: u32,
}

/// Which metadata ceiling a write would exceed, and by what.
///
/// The same shape as [`TaxonomyLimitExceeded`] and for its reason: the refusal
/// must **name** the limit, and a caller holding only a message would have to
/// parse it back out.
#[domain_model]
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct MetadataLimitExceeded {
    /// Which ceiling, **in `ProductsConfig`'s spelling** — `max_keys`,
    /// `max_key_bytes`, `max_value_bytes`. Deliberately not
    /// [`MetadataLimits`]' own field names, which drop the prefix to satisfy
    /// `clippy::struct_field_names`: an operator reading this refusal is
    /// looking for the setting to change, and the setting is
    /// `metadata_max_keys`.
    pub limit: &'static str,
    /// The configured ceiling.
    pub allowed: u32,
    /// What the write would make it.
    pub measured: u32,
    /// The offending key, or `None` for `max_keys`, which is about the map.
    pub key: Option<String>,
}

impl MetadataLimitExceeded {
    /// The refusal code (`design/02` §3.3).
    pub const CODE: &'static str = "METADATA_LIMIT";
}

/// Judge a metadata write against its ceilings.
///
/// # The count is on the **resulting** map, and that is the whole point
///
/// `dod-metadata-door` requires *"a test **MUST** prove a map standing at the
/// cap can be reduced"*, and the door is a **per-key merge**: an absent key is
/// left alone and a `null` removes one. So a payload of ten keys against a map
/// of fifty is not eleven writes over the cap — it is whatever the merge
/// leaves. Counting the payload, or the pre-merge map, would refuse exactly
/// the reduction the `DoD` obliges.
///
/// `resulting_keys` is therefore the caller's post-merge count, and the byte
/// checks run over `written` — the entries this act actually sets, since a key
/// already stored under an older, larger ceiling is not this write's to refuse.
///
/// # Order is stated, and it is not a precedence claim
///
/// Count, then key length, then value length; and `written` is walked in
/// **sorted key order** so two writes of the same payload refuse identically.
/// `design/02` states no precedence among this feature's sixteen codes and its
/// own §7 records that, so this is reproducibility rather than ranking.
///
/// # Built and unreachable, and the reason has changed
///
/// [`TaxonomyLimitExceeded`]'s doc calls that *"the honest shape for a rule
/// whose operand is owed"* — no caller could supply its thresholds. **This
/// rule's operands are not owed**: `ProductsConfig` carries all three ceilings
/// since **P-D-107** arm 1. What it waits on is its **door**.
/// `dod-metadata-door` enforces `METADATA_LIMIT` at
/// `PATCH …/{products|skus}/{id}/metadata`, which **P-D-106** doored and
/// nothing has built, and the code's `DomainError` variant lands **with** that
/// door rather than before it: a variant with no raiser is a dead `match` arm,
/// which is what `infra::error_mapping`'s own paragraph refuses. Its grant is
/// held on the same terms by `authz_tests`' census.
///
/// So this is the door's judge, complete and tested, waiting for the door.
///
/// # Errors
///
/// [`MetadataLimitExceeded`] naming the first ceiling exceeded.
pub fn metadata_limit_verdict(
    resulting_keys: u32,
    written: &[(String, String)],
    limits: MetadataLimits,
) -> Result<(), MetadataLimitExceeded> {
    if resulting_keys > limits.keys {
        return Err(MetadataLimitExceeded {
            limit: "max_keys",
            allowed: limits.keys,
            measured: resulting_keys,
            key: None,
        });
    }
    let mut sorted: Vec<&(String, String)> = written.iter().collect();
    sorted.sort_by(|left, right| left.0.cmp(&right.0));
    for (key, _) in &sorted {
        let measured = u32::try_from(key.len()).unwrap_or(u32::MAX);
        if measured > limits.key_bytes {
            return Err(MetadataLimitExceeded {
                limit: "max_key_bytes",
                allowed: limits.key_bytes,
                measured,
                key: Some((*key).clone()),
            });
        }
    ...
    for (key, value) in &sorted {
        let measured = u32::try_from(value.len()).unwrap_or(u32::MAX);
        if measured > limits.value_bytes {
            return Err(MetadataLimitExceeded {
                limit: "max_value_bytes",
                allowed: limits.value_bytes,
                measured,
                key: Some((*key).clone()),
            });
        }
