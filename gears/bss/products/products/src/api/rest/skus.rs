...
fn register_list_door(router: Router, openapi: &dyn OpenApiRegistry) -> Router {
    OperationBuilder::get("/bss-products/v1/skus")
        .operation_id("bss_products.list_skus")
        .summary("List and search current Sku heads")
        .description(
            "Returns current authoring heads in every lifecycle state, including draft, \
             ...
             `sku x read`, scoped to the caller's tenant and PDP constraints. \
             Each item has the same shape as GET /skus/{id}, including internal_revision. \
             Supports OData $filter and $orderby; $select is refused. Default order: sku_code asc, sku_id asc. \
             This is a live keyset walk, not a snapshot across concurrent edits.",
        )
        .tag(TAG)
        .authenticated()
        .no_license_required()
        .with_odata_filter::<repo::SkuHeadFilterField>()
        .query_param("$orderby", false, "Orderable fields: sku_id, product_id, sku_code, lifecycle_state, sellable, internal_revision, published_version. Nullable fields cannot be order keys.")
        ...
        .handler(list_skus)
        .json_response_with_schema::<toolkit_odata::Page<SkuView>>(openapi, StatusCode::OK, "One page of current heads.")
        .error_400(openapi)
        .error_401(openapi)
        .error_403(openapi)
        .error_500(openapi)
...
/// `GET /skus`: current heads, including drafts and terminal states.
async fn list_skus(
...
) -> Result<axum::Json<toolkit_odata::Page<SkuView>>, CanonicalError> {
    ...
    .await
    .map_err(|e| query::head_list_error_to_canonical("skus", &e))?;
    Ok(axum::Json(page.map_items(SkuView::from)))
}
...
pub(crate) const SKU_VERSION_CONTENT_ROSTER: [&str; 17] = [
...
pub(crate) fn sku_version_content(
    ...
    // 03's classification (P-D-145, narrowed by P-D-169): the flag is a JSON
    // boolean like `composition_pending`; the type and the tier ride as
    // strings when the head carries them and are omitted otherwise
    // (`Absence::Omit`), the roster turning the omission into `null` on the
    // frozen row. The two accounting codes rode here too until P-D-169.
...
/// that is slice 05's: [`NoMaterialityPolicyGate`](crate::domain::governance::NoMaterialityPolicyGate) answers
...
/// this door uses.
///
/// The handler [`publish_sku`] passes [`NoMaterialityPolicyGate`](crate::domain::governance::NoMaterialityPolicyGate), which is
...
///   has nothing to consume: [`NoMaterialityPolicyGate`](crate::domain::governance::NoMaterialityPolicyGate) answers
...
/// Build the host a SKU door runs under (P-D-142): `GateHost::Real` becomes
/// the stored host for the act, a `Given` one passes through. A refusal before
/// the host exists — a bucket-ii touch outside the correction door, an
/// unregistered column — is the act's own, answered through
/// [`answer_head_act`] like every other; a storage failure is the door's `500`.
async fn resolve_sku_host(
    state: &ApiState,
    act: &ActContext,
    ...
    crate::api::rest::resolve_host(&conn, &act.scope, act.tenant_id, host, for_act)
        .await
        .map_err(|e| match e {
...
/// P-D-139/P-D-142): the record the host named is spent in the act's own
/// transaction, and a record spent under the act refuses it.
async fn settle_sku_authorization(
    runner: &(impl toolkit_db::secure::DBRunner + Sync),
    inputs: &HeadActInputs,
...
async fn publish_sku_gated(
    ...
    let outcome = match resolve_sku_host(state, &act, host, for_act).await {
        Ok(gate) => publish_in_one_transaction(state, ctx, &inputs, &gate, mode).await,
        Err(refused) => Err(refused),
    };
    answer_head_act(state, &act, sku_id, head.internal_revision, outcome).await
}
...
/// and they are decided the same way: [`NoMaterialityPolicyGate`](crate::domain::governance::NoMaterialityPolicyGate) and
...
/// decides it: [`NoMaterialityPolicyGate`](crate::domain::governance::NoMaterialityPolicyGate) as a literal, so no wire input
/// chooses one.
///
...
async fn undeprecate_sku_gated(
    ...
    let outcome = match resolve_sku_host(state, &act, host, for_act).await {
        Ok(gate) => undeprecate_in_one_transaction(state, &inputs, &gate).await,
        Err(refused) => Err(refused),
    };
    answer_head_act(state, &act, sku_id, head.internal_revision, outcome).await
}
...
async fn deprecate_sku_gated(
    ...
    let outcome = match resolve_sku_host(state, &act, host, for_act).await {
        Ok(gate) => deprecate_in_one_transaction(state, &inputs, &gate).await,
        Err(refused) => Err(refused),
    };
    answer_head_act(state, &act, sku_id, head.internal_revision, outcome).await
}
...
async fn retire_sku_gated(
    ...
    let outcome = match resolve_sku_host(state, &act, host, for_act).await {
        Ok(gate) => retire_in_one_transaction(state, &inputs, &gate, &detector, request).await,
        Err(refused) => Err(refused),
    };
    answer_head_act(state, &act, sku_id, head.internal_revision, outcome).await
}
...
async fn cancel_sku_retirement_gated(
    ...
    let outcome = match resolve_sku_host(state, &act, host, for_act).await {
        Ok(gate) => cancel_retire_in_one_transaction(state, &inputs, &gate, request).await,
        Err(refused) => Err(refused),
    };
...
fn parse_sku_value(
    ...
    match field {
        ...
        SkuSaveField::SkuType | SkuSaveField::PlanTier => {
            let raw = expect_sku_string(wire, value)?;
            let trimmed = raw.trim();
            if trimmed.is_empty() {
                return Err((
            ...
            Ok(match field {
                ...
                _ => SkuSaveValue::PlanTier(trimmed.to_owned()),
            })
        }
...
async fn recheck_classification(
    ...
    // The profile is judged for its own sake: it decided which accounting
    // codes a publish required until P-D-169 took both out of the gear, and
    // what remains is the closed-set refusal on `sku_type` itself.
    if publishing || image.sku_type != head.sku_type {
...
async fn validate_classification_on_create(
    ...
    match classify_on_create(state, scope, tenant_id, sku_type, plan_tier, now).await {
        Ok(()) => Ok(()),
...
/// [`discard_sku`] decide it: the [`NoMaterialityPolicyGate`](crate::domain::governance::NoMaterialityPolicyGate) literal, so no
