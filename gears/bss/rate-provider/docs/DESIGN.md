...
- **Scope today:** not exploitable in v1 as shipped. The only configured source is ECB, a
  ...
  disclose. Even with a credential-bearing feed configured, the exposure is conditional on
  operator error — the documented `${VAR}` path (above) keeps the raw key out of the file
  the dump serializes — so dump-side redaction is defense-in-depth for that
  misconfiguration, not a precondition for onboarding an authenticated feed (PRD §12 states
  the same).
...
|-------|-----------|-------|-----------|
| `FxNoFetchAttempted` | `rate(fx_provider_fetch_duration_seconds_count[3 × fx.rate_sync_tick_secs]) == 0 and rate(fx_provider_fetch_errors_total[3 × fx.rate_sync_tick_secs]) == 0` | Platform on-call | "No attempt completed at all", which is what separates a dead feed from a slow one. Read **together with** the ledger's tick heartbeat: with the heartbeat rising, this narrows to "the job is alive but discovery yields no sources". |
...
**This gear sees fetch attempts, not ticks — which is why the pair is read together.** The
