...
//! argument.
//!
//! `cargo test` runs test *binaries* sequentially, so this gear's suites do not
//! normally contend for the shared server; two concurrent `cargo test`
//! invocations would, and the arrangement holds under that — the reuse path
//! takes no lock, the start path retries a name conflict, and the prune skips
//! every database whose run is still alive.
//!
...
async fn resolve_server() -> (u16, Option<ContainerAsync<Postgres>>) {
    ...
    // Either nothing is there, or what is there is not answering: a container a
    // killed run left half-started. Removing it by name is safe — the name is
    // this harness's own, and a live one would have answered above.
    ...
    // A sibling binary won the race and started it under our name.
    let port =
        published_port(HARNESS_CONTAINER).expect("the sibling's container must publish a port");
    (port, None)
}
...
async fn start_named() -> Option<ContainerAsync<Postgres>> {
    ...
    for _ in 0..5_u32 {
        ...
        // A name conflict means somebody else got there; if theirs answers, use
        // it rather than fighting for the name.
        if let Some(port) = published_port(HARNESS_CONTAINER)
            && answers(port).await
        {
            return None;
    ...
    panic!("postgres never started under the name {HARNESS_CONTAINER}: {last}");
}
...
async fn prune_stale_databases(port: u16) {
    // Calibrate before dropping anything: a `ps` that cannot see *this* process
    // cannot be trusted to report another one gone, and the answer to a question
    // that cannot be asked is to leave every database alone.
