...
/// **The precondition token is obtainable from the list** (review R-1).
///
/// `PATCH /price-overlays/{id}` requires `If-Match: "<revision>-<version>"`, and
/// that tag was emitted at exactly two places: the create's `201` and the
/// `PATCH`'s own `200`. The create's *replay* deliberately omits it, this list
/// emitted no `ETag`, and there is no `GET …/price-overlays/{id}` — the
/// `Location` the create sets answers `405`. So a caller that lost the one-shot
/// tag, and whose overlay was then moved by anyone else, could construct no
/// `If-Match` that was not `409 STALE_VERSION`, and no request existed that would
/// tell it the current version. The door was closed to that caller permanently.
///
/// **Armed as the caller's predicament, not as the happy path.** Every existing
/// case takes the tag from the create's response, which is exactly the thing the
/// stranded caller does not have. This one edits once to move the version, then
/// throws that response away and rebuilds the tag from **the list alone**.
#[tokio::test]
async fn the_if_match_tag_can_be_rebuilt_from_the_list_after_a_concurrent_edit() {
    let harness = Harness::new().await;
    let overlay = seed_overlay(&harness, 10).await;

    // Somebody else edits, taking the row past the version the create handed out.
    let moved = harness
        .allowed()
        .send(with_headers(
            "PATCH",
        ...
        .await;
    assert_eq!(moved.status(), StatusCode::OK, "the first edit lands");

    // The stranded caller holds nothing but what the list returns.
    let listed = harness
        .allowed()
        .send(request("GET", PRICE_OVERLAYS, None))
        .await;
    assert_eq!(listed.status(), StatusCode::OK);
    let page = body_json(listed).await;
    let item = page["overlays"]
        .as_array()
        .expect("the page carries the overlays")
        .iter()
        .find(|row| row["price_overlay_id"] == serde_json::json!(overlay))
        .expect("the overlay is on the page")
        .clone();

    let revision = item["revision"]
        .as_u64()
        .expect("the page carries a revision");
    let version = item["row_version"]
        .as_u64()
        .expect("the page has to carry the version half of the tag, or the door is unreachable");
    let tag = format!("\"{revision}-{version}\"");
    ...
    assert_eq!(
        second.status(),
        StatusCode::OK,
        "a tag built from the list alone must satisfy the precondition"
    );
}
