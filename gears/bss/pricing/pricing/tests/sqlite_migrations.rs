...
const EXPECTED_CHECKS: &[&str] = &[
    ...
    // `m20260802_000066` added `unit_rate_nano` and gave it this CHECK on
    // Postgres only, stating that rebuilding `pricing_price` for one clause
    // "would be a large edit". Ten migrations later `m20260802_000076` performed
    // exactly that rebuild - restating twenty of the table's twenty-one CHECKs
    // and omitting this one - so the reason had stopped being true and half of
    // `amount_minor`'s non-negativity rule was left behind on the mirror.
    ...
    // D-334's declared rounding vocabulary — `m20260802_000080`, the taxonomy
...
const EXPECTED_PRIMARY_KEYS: &[(&str, &str)] = &[
    ...
    // D-334 (`m20260802_000080`): the taxonomies' key, on a table of their
    // shape.
...
const EXPECTED_TRIGGER_BODIES: &[(&str, u64)] = &[
    ...
    // that moved.
    //
    // Moved again 2026-08-19 by `m20260802_000090`, and this time by a
    // whole-function restatement rather than a rename: the guard gained a clause
    // for `resolved_rounding_policy`, the column `m20260802_000089` added so the
    // publish stops writing its rounding resolution over the *authored*
    // `rounding_policy_ref`. One digest moved and the name census is unchanged,
    // which is the same evidence as before.
    (
        "trg_pricing_price_frozen_columns",
...
#[tokio::test]
async fn a_partial_rollback_past_the_rescale_keeps_the_append_only_guard() {
    // `m082`'s `SQLite` `down` drops `trg_pricing_price_frozen_columns` so the
    // rescale `UPDATE` can run - the guard would refuse it - and then never puts
    // it back. Every version from `m081` down to `m070` therefore has a
    // `pricing_price` with **no append-only guard at all**: a published row's
    // price, scope and entity-tag columns become mutable by any statement.
    //
    // `down_then_up_round_trips` cannot see this. It walks the chain all the way
    // to the bottom, where `m002` drops the table outright, so the window where
    // the guard is missing closes before anything looks. This stops halfway,
    // which is where an operator rolling back a bad release actually stands.
    let conn = Database::connect("sqlite::memory:")
        .await
        .expect("connect in-memory sqlite");
    let manager = SchemaManager::new(&conn);
    ...
    assert!(
        ...
        "positive control: the guard is there at the head of the chain"
    );

    // Down to and including `m082`, and no further.
    let rescale = "m20260802_000082_reserved_rate_is_a_rate_column";
    for migration in chain.iter().rev() {
        migration
            .down(&manager)
            .await
            .unwrap_or_else(|e| panic!("down {} must succeed: {e}", migration.name()));
        if migration.name() == rescale {
            break;
        }
    }
    ...
    assert!(
        objects_of(&conn, "trigger")
            .await
            .contains(&"trg_pricing_price_frozen_columns".to_owned()),
        "a rollback that stops at `{rescale}` must leave the append-only guard armed; without \
         it every published price row on this version is silently mutable"
    );
}
