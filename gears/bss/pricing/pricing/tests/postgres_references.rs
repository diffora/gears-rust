//! The reference state machine end to end on native Postgres: the prices door and the ticker
//! over pricing's real chain, against the scripted Products registry double of run 3 (see
//! `price_support::Script`). Products' own Postgres half of the barrier (reserve against the
...
use bss_pricing::{
    ...
    infra::{
        ...
        storage::repo::{price_repo, reference_op_repo as ops, row_repo},
    },
};
...
impl Door {
    ...
    async fn reference_state(&self, price: &Value) -> String {
        price_repo::find(
            &self.db.conn().unwrap(),
            &self.scope(),
            self.tenant(),
            price.as_str().unwrap().parse().unwrap(),
        )
        .await
        .unwrap()
...
async fn postgres_create_reserves_writes_and_confirms_and_a_delete_releases() {
    ...
    let one = format!("/prices/{}", created.1["id"].as_str().unwrap());
...
#[tokio::test]
#[ignore = "needs the Postgres harness"]
async fn postgres_forced_release_reconciles_both_ways_and_announces_a_lost_price() {
    for fenced in [false, true] {
        ...
        if fenced {
            ...
            assert_eq!(lost[0]["data"]["priceId"], created.1["id"]);
            ...
            .unwrap()
            .unwrap();
            let refused = row_repo::insert(&conn, &p.scope(), price_support::row(&price))
                .await
                .unwrap_err();
            assert!(refused.to_string().contains("PRICE_REFERENCE_LOST"));
        } else {
