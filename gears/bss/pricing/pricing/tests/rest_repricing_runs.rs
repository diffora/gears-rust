...
async fn a_run_opens_over_the_published_rows_and_freezes_them_pending() {
    ...
    // The report is what `repricing_run_content_hash` pins, so its bytes are
    // main's: chrono's `to_rfc3339()`, a `+00:00` designator, not the `Z` the
    // wire uses. A unit opened before the type change must still digest to its
    // `content_hash` after it.
    assert_eq!(
        run["report"]["changeover"],
        serde_json::json!("2099-08-20T00:00:00+00:00"),
        "{run}"
    );
}
