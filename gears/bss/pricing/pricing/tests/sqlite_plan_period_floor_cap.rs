...
//! market key, the four `CHECK`s that make a bound usable, the append-only
...
//!
//! **Why the four `CHECK`s are here and the publish rules are not.** Each of
...
/// a composite foreign key, four `CHECK`s and three append-only triggers, so a
...
/// and with four `CHECK`s on one row it is not hypothetical: a `0` floor and an
...
// ---------------------------------------------------------------------------
// The four CHECKs — each is a bound that admits no bill.
// ---------------------------------------------------------------------------
