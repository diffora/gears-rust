...
//!
//! `m20260802_000024`…`…000027` had never run on the backend they target.
...
//! `cargo test -p bss-pricing --test postgres_schema_bundle -- --ignored`.
...
async fn a_party_may_not_be_named_platform() {
    ...
    must_be_rejected(
        &conn,
        &party_sql(BUNDLE_A, 0, VENDOR_1, "platform", 9000),
