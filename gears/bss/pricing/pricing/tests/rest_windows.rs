...
use bss_pricing::domain::instant::{format_rfc3339, format_rfc3339_offset};
...
async fn a_scheduled_window_with_no_threshold_policy_opens_a_unit_and_writes_nothing() {
    ...
    assert_eq!(
        ...
        format!(
            ...
            seeded.price_id,
            format_rfc3339_offset(common_to())
        ),
...
async fn a_window_units_detail_renders_the_act_being_signed_for() {
    ...
    assert_eq!(
        ...
        // The literal main asserted: the view echoes the act token's own spelling
        // (`interval_end`), and a token must read alike before and after this
        // migration or a retry never finds its unit.
...
async fn a_cancel_opens_a_unit_and_the_window_is_still_scheduled() {
    ...
    assert!(
        unit.subject_ref.ends_with(&format!(
            ...
            bound = format_rfc3339_offset(common_to())
        )),
