...
async fn an_absent_plan_and_a_foreign_one_read_alike() {
    ...
    rest_support::seed_foreign_plan(&h, foreign).await;
...
///
/// The path parameter is a `Uuid` axum rejects before the handler, so the only
/// malformed request reaching this surface is one that never reaches it. The 422
/// half is `module_test.rs`'s property for every route; this asserts the shape of
...
async fn a_malformed_plan_id_never_reaches_the_handler() {
    ...
    // axum's own path rejection, before the handler and before the gate. The body
    // carries **no RFC 9457 problem document at all** — not even an empty one —
    // which is what says this gear never answered: every refusal it produces goes
    // through the canonical ladder and arrives as one.
    ...
    assert!(
        ...
        "a gear refusal would be a problem document; got {:?}",
...
/// `published_price_ids` empty) rather than assumed, because the whole argument rests
/// on it.
///
...
async fn a_fully_covered_plan_answers_not_evaluable_rather_than_sellable() {
    ...
    assert_ne!(document["verdict"], serde_json::json!("sellable"));
}
