...
fn every_mutating_router_applies_the_correlation_edge() {
    ...
    assert!(
        mutating.len() >= 6,
        "the scan sees fewer mutating routers than this gear mounts - four single-router \
         modules plus the two that export a governance router of their own: {mutating:?}"
    );
    assert!(
        offenders.is_empty(),
...
async fn a_foreign_tenants_object_reads_like_an_absent_one_on_every_by_id_read() {
        ...
        .filter(|route| !route.mutating && route.path.contains('{'))
    {
        ...
        // whether the two refusals below are distinguishable, so it is skipped
        // rather than asserted - several by-id reads in the census address rows
        // this fixture deliberately does not seed, and their own doc says why.
