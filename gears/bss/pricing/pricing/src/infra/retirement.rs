...
//!
//! A third thing refuses and is **not** a reference:
//! [`ensure_no_live_migration_targets`] reads the migration plane directly and
//! raises `RETIRE_TARGET_OF_MIGRATION`. It is kept out of [`ReferenceReport`]
//! because the two errors are typed apart in the ladder and their remedies are
//! different acts on different objects. The consequence worth stating: the
//! **dry-run does not report it**, so a preview can call a plan retirable that
//! the commit then refuses. Closing that means deciding whether a live schedule
//! is a third `BlockingReferenceKind` — which would fold it into
//! `RETIRE_PLAN_REFERENCED`, the thing the ladder deliberately avoids — or
//! whether the preview grows a channel of its own. Not invented here.
...
use crate::infra::storage::repo::{
    ...
    migration_repo, outbox_repo, plan_repo, price_repo, window_repo,
};
...
///   `approved → unwound` transition — all three since **D-333** (2026-08-16). What
...
///   already taken.
///
///   **This bullet said the opposite until 2026-08-18** (review Z21-1): that the
///   design set does not declare the token and that D-204 clause (2) *refuses*
///   minting it. Both halves were false, and the shape of the error is worth more
///   than the correction — a mirroring obligation with no target reads to the next
///   author as a blocked external dependency, which is D-333's own account of why it
///   corrected the same sentence in the register. `infra::cutover::record_cutover`
///   and `infra::grandfather` describe this world correctly and are the wording to
///   follow.
///
