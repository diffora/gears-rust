...
use crate::domain::instant::{format_rfc3339_offset, format_rfc3339_serde};
...
pub fn horizon_unit_ref(
    ...
    format!(
        ...
        prior.map_or_else(|| "none".to_owned(), format_rfc3339_offset),
        format_rfc3339_offset(proposed)
    )
}
...
fn horizon_state_value(record: &PriceRecord) -> serde_json::Value {
    serde_json::json!({
        ...
        "grandfatherUntil": record.grandfather_until.map(format_rfc3339_serde),
