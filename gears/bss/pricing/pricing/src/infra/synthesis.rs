...
fn row_value(
    ...
    json!({
        ...
        //
        // **Projected, not raw** (review M-4). This rendered `stored.billing_timing`
        // until 2026-08-19, and that column is `NULL` on a usage line by intent —
        // `inst-bt-required` requires it only on `recurring`, and
        // `check_setup_fields` forbids it on setup rows. So every usage and
        // one-time line of a legacy subscriber's frozen record carried `null`
        // where the read model for the identical row renders `arrears` /
        // `advance`. `published_billing_timing`'s own doc says why the column must
        ...
        // **This payload is uncompiled, and now says so** (review M-5).
        //
        ...
        // and what the publish resolved it to. `roundingPolicyRef` carried the
        // resolution under an authored name until 2026-08-19, because the publish
        // wrote it over the authored column - so a consumer could not tell a row
        // that named a policy from one that fell back to the tenant default, and
        // this comment already called it "the resolved rounding policy" while the
        // key said otherwise.
