...
pub async fn publish_rows(
    ...
    //
    // `roundingPolicyRef` was not resolved at all until 2026-08-19 (review M-2).
    // `RoundingPolicyResolved` returns at its first line the moment a tenant
    // default exists, and its doc says *"the resolved id then freezes into the
    // read model and the snapshot"* — which nothing performed. Both renderers read
    // the raw nullable column, so a tenant with a default and forty rows carrying
    // none froze `"roundingPolicyRef": null` on every one of them. Two costs, and
    // the second is why this is a freeze rather than a lookup: a consumer computing
    // a charge from the pinned version had no rounding mode, **and** the tenant
    // could later flip the default and silently re-round every already-frozen
    // version. Freezing is what makes that impossible, which is the whole reason
    // `resolved_tax_category` beside it is frozen the same way.
    ...
    for ((resolved, rounding), price_ids) in by_resolution {
        ...
        let outcome = price::Entity::update_many()
            ...
            // **Beside the authored column, never over it.** This wrote
            // `RoundingPolicyRef` itself until 2026-08-19, so a row that carried
            // no policy of its own came back from publish naming one - the tenant
            // default stopped reaching it, `authored_content` and
            // `infra::supersession` carried the resolution into every successor
            // draft as authored, and `RoundingPolicyDeclared` had nothing left to
            // judge. `ResolvedTaxCategory` one clause up never had that shape.
            .col_expr(
...
fn insert_model(tenant_id: Uuid, record: &PriceRecord) -> Result<price::ActiveModel, RepoError> {
    ...
    let price::ActiveModel {
        ...
        // reason (`m20260802_000089`). A creation says nothing about it either.
...
fn content_assignments(model: price::ActiveModel) -> Vec<(price::Column, Value)> {
    let price::ActiveModel {
        ...
        // `m20260802_000089` the resolution went into `rounding_policy_ref` -
