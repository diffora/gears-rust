...
impl OverlayRepo {
    ...
    pub async fn list(
        ...
        // both columns of the order** (review R-3). Resuming on the id alone
        // against an `(id, revision)` sort drops every remaining revision of the
        // overlay a page boundary falls inside. Spelled as a disjunction rather
        // than a row-value comparison for `group_membership_repo`'s reason: both
        // engines take `(a, b) > (c, d)`, sea-orm has no builder for it, and a raw
        // fragment would put a dialect-shaped string into a repository that holds
        // none.
        ...
        // it, and that order cannot carry a keyset walk: a precedence-first order
        // would resume in the wrong place and the walk would skip rows -- exactly
        // what D-125 forbids. The cursor is `(id, revision)`, the same pair this
        // orders by (`api::rest::cursor::encode_id_and_revision`, 24 bytes); it
        // was a single 16-byte id until 2026-08-18, which is one column short of
        // the order and dropped rows at a boundary inside an overlay.
        //
