...
//!
//! This is the third of the three sites review finding Z8-14 named; the two in
//! `overlay_repo` were corrected in `21633827f` and `overlay_repo_tests.rs` is the
//! file these cases follow.
...
/// Unique but not named: a neighbouring table whose column is spelled the same.
///
/// `pricing_bundle_component.plan_id` exists, which is why the predicate matches
/// the **qualified** name rather than the bare column.
#[test]
