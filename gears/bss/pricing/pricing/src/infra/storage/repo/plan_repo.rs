...
async fn list_authoring_odata_page(
    ...
    if let Some(cursor) = &query.cursor {
        // `validate_cursor_against` compares hashes only when the request and
        // the cursor both carry one, so a page-2 request that keeps the cursor
        // and drops `$filter` passes it. This walk re-derives its predicate from
        // the current request, so that request would silently page a different
        // result set under page 1's cursor — the authoring-default lifecycle set
        // instead of the filter the walk began under. Disagreeing about whether
        // a filter exists at all is the same mismatch as disagreeing about which.
        if cursor.f.is_some() != query.filter_hash.is_some() {
            return Err(OdataPageError::Odata(ODataError::FilterMismatch));
        }
    ...
    loop {
        ...
        for row in rows {
            ...
            out.push(to_domain(row).map_err(OdataPageError::Repo)?);
