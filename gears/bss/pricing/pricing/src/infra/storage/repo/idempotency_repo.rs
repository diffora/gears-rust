...
impl IdempotencyGate {
    ...
    pub async fn claim(
        ...
        let held = read_held(txn, scope, tenant_id, operation, client_key).await?;
...
/// Read back the row a claim just collided with, scoped.
///
...
/// itself inside one transaction, which is why it surfaces as a storage failure
/// rather than as an absent claim.
async fn read_held(
    txn: &DbTx<'_>,
    scope: &AccessScope,
    tenant_id: Uuid,
    operation: &str,
    client_key: &str,
) -> Result<idempotency_dedup::Model, RepoError> {
