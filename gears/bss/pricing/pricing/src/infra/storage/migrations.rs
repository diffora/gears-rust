...
//!
//! **What a migration in this chain owes its own verification step, learned twice in
//! one day (2026-08-18, review P-1/Z14-1/Z14-4).** A change to DDL, to a trigger body
//! or to a column *name* is narrow in the diff and narrow on **neither engine**:
//! `RENAME COLUMN` rewrites `SQLite` trigger bodies (since 3.25) and does not rewrite
//! a PL/pgSQL body, which Postgres keeps as opaque text in `pg_proc.prosrc`. So
//! `m20260802_000082` renamed a column, argued in its own prose that no guard needed
//! restating, verified itself against three `SQLite` targets — and left the whole
//! `pricing_price` append-only guard raising `42703` on every non-draft UPDATE, price
//! supersession included. The repair, `m20260802_000086`, was verified on Postgres and
//! broke the `SQLite` reverse walk by restoring the pre-fix body verbatim. One
//! asymmetry, two defects, the same column, the same day, each shipped by a run that
//! was green.
//!
...
//! other engine's tier.
//!
//! **And a `DOWN` restores the state the chain below it leaves**, which is not always
//! the previous `UP`'s text. The convention in this crate — *"the `DOWN` is the
//! previous migration's `UP`, verbatim"* — assumes the previous state was correct;
//! where a migration repairs a defect, restoring that text literally reintroduces it,
//! and the reverse walk reaches the repair before it reaches whatever renamed the
//! column underneath. `m20260802_000086`'s `DOWN` is a semantic no-op for exactly this
//! reason, and `m20260802_000087`'s is deliberately asymmetric between the engines.
//!
...
pub mod m20260802_000086_guard_pricing_price_reserved_rate_nano;
pub mod m20260802_000087_create_source_plan_indexes_on_postgres;
mod m20260802_000088_create_read_shape_indexes;
mod m20260802_000089_add_pricing_price_resolved_rounding_policy;
mod m20260802_000090_guard_pricing_price_resolved_rounding_policy;
...
impl MigratorTrait for Migrator {
    fn migrations() -> Vec<Box<dyn MigrationTrait>> {
        vec![
            ...
            Box::new(m20260802_000086_guard_pricing_price_reserved_rate_nano::Migration),
            Box::new(m20260802_000087_create_source_plan_indexes_on_postgres::Migration),
            Box::new(m20260802_000088_create_read_shape_indexes::Migration),
            // The resolved rounding policy gets a column of its own, and the
            // frozen-column guard is restated to name it. Two migrations because
            // that is the shape this table's five earlier column additions took:
            // the `ADD COLUMN` and the whole-function guard restatement are
            // separate acts, and running them apart is what makes the second one
            // visible when it is forgotten.
            Box::new(m20260802_000089_add_pricing_price_resolved_rounding_policy::Migration),
            Box::new(m20260802_000090_guard_pricing_price_resolved_rounding_policy::Migration),
