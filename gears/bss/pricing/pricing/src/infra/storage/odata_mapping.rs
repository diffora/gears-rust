...
fn lifecycle_token(value: &ODataValue) -> Result<ODataValue, String> {
    match value {
...
impl FieldToColumn<PlanFilterField> for PlanODataMapper {
    ...
    // As for migrations, plus one extra reason here: an unknown token also
    // reads as "the caller named a lifecycle filter" to `filter_mentions_field`,
    // which suppresses the authoring default — so a typo would silently widen
    // the walk instead of narrowing it.
    fn map_value(
        field: PlanFilterField,
        _op: FilterOp,
        value: &ODataValue,
    ) -> Result<ODataValue, String> {
        match field {
            PlanFilterField::LifecycleState => lifecycle_token(value),
            _ => Ok(value.clone()),
        }
    }
}
...
mod tests {
    ...
    fn an_unknown_enum_token_is_refused_by_the_filter() {
        ...
        assert!(
            PlanODataMapper::map_value(PlanFilterField::LifecycleState, FilterOp::Eq, &bogus)
                .is_err(),
        ...
        for state in LifecycleState::ALL {
            let good = ODataValue::String(state.as_str().to_owned());
            assert!(
                PlanODataMapper::map_value(PlanFilterField::LifecycleState, FilterOp::Eq, &good)
                    .is_ok(),
