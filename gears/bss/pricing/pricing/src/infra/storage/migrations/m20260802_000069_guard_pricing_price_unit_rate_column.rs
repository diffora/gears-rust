...
//!
//! The band rates need no line here, and the reason is **not** the one this
//! header gave until 2026-08-18 (review P-7). It claimed
//! `pricing_price_tier_band`'s guard freezes `unit_price_nano` by name, carried
//! through a rename. No trigger body on either engine names that column at all:
//! `unit_price_minor` / `unit_price_nano` appear in exactly four places in the
//! whole chain, and all four are `m20260802_000011`'s two `CREATE TABLE`s and
//! their `CHECK`.
//!
//! What actually protects a band rate is **stronger** than a column freeze. The
//! tier-band guards (`m20260802_000011` on Postgres, `m20260802_000076` on
//! `SQLite`) read the **parent row's** `lifecycle_state` and refuse any `UPDATE`
//! of a band under a non-draft parent, wholesale. So there is nothing here to
//! restate on a rename -- which is the answer the next reader was going to have to
//! derive after failing to find the trigger reference this sentence promised.
//!
