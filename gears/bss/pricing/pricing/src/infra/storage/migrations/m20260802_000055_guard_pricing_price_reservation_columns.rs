//! `pricing_price`'s frozen-column guard gains Slice 10's two reservation
//! columns.
//!
//! `m20260802_000054` added `reserved_rate_minor` and `reservation_flavor` and
//! put neither in either engine's guard. This is the same gap
//! `m20260802_000039` declared and `m20260802_000040` paid for the tax columns,
//! and `m20260802_000050`/`m20260802_000051` paid for the proration ones -- and
//! the argument is the sharpest the table has had. `reserved_rate_minor` is a
//! **rate**: it is authored draft content a `PATCH` moves, it is inside the
//! approval content pin, and under D-139 it is money *per covered granule*. A
//! published row whose reserved rate a writer outside this crate changed would
//! diverge from the very pin that approved it, silently, and what diverges is an
//! amount.
//!
//! `reservation_flavor` travels with it because the pair is one fact
//! (`inst-rv-attrs`) and because the flavor is what decides whether the reserved
//! quantity leaves the on-demand tier counter at all (`inst-rv-tier-q`,
//! `inst-rv-level`): a writer that flipped `capacity` to `consumption` on a
//! published row would move the charge without moving the rate.
//!
...
//! rule and is followed here rather than referenced. The two statement blocks
//! were read out of `m20260802_000051`'s own `UP` text; two lines were inserted
//! after `included_allowance` in each; and the generator asserted, before
//! writing anything, that it had found the expected number of frozen-column arms
//! (one on each engine -- the guess that `SQLite` carried two is what the
//! assertion caught), that each block came out **exactly two lines longer per
//! arm**, that each new column appears exactly once per arm,
//! and that **every original line survives**. This program has twice had a
//! scripted replacement do something other than intended, once deleting whole
//! function bodies; those assertions are the cheapest check that catches the
//! class.
//!
//! The `DOWN` is `m20260802_000051`'s `UP` **verbatim** -- the state this
//! migration moves away from -- for the same reason that one's `DOWN` was
//! `m20260802_000040`'s text.
//!
//! What watches the restatement is unchanged and is why taking it is safe:
//! `sqlite_migrations` pins trigger **bodies** by digest, not merely their
//! names, and `postgres_schema_price` executes every arm of this function. A
//! restatement that lost an arm reddens there rather than in production.
//!
//! # Where the two sit in the list
//!
//! Directly after `included_allowance`, the other Slice-10 column on this table
//! and the one they are authored beside. The list is not sorted by anything, so
//! "beside the fact it belongs to" is the only ordering rule it has --
//! `m20260802_000040`'s words for the same decision, kept.
