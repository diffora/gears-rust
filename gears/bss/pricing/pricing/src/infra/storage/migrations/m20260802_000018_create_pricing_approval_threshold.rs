...
//!
//! **Not `pricing_policy_object`'s pair.** `m20260802_000006` carries
//! `approval_threshold_minor bigint` and `approval_threshold_currency
...
//! Nothing reads those two columns today, so the pair is **dropped by this same
//! migration** rather than left beside its replacement: leaving it would give the
//! gear two places to read a threshold from, which is the shape this program has
//! twice found to be how a stale claim survives.
//!
...
//! (`m20260802_000001…rs:46-49`), and `SQLite` has no `GRANT`/`REVOKE` at all, so
...
//! `now()` -> `(CURRENT_TIMESTAMP)`, the `bss.` prefix dropped):
//!
...
//! * **the column drop is a table rebuild.** `SQLite`'s `ALTER TABLE … DROP
//!   COLUMN` refuses a column named in a CHECK constraint, and both dropped
//!   columns are named in two, so the portable form is the create-copy-drop-rename
//!   dance. `pricing_policy_object` carries no index, no trigger and no inbound
//!   foreign key, which is what makes the rebuild safe to write out in four
//!   statements.
//!
//! The `down` is symmetric on both backends: the new table goes, and the two
//! columns come back with the two CHECKs that were dropped with them.
...
const PG_UP_STATEMENTS: &[&str] = &[
    "CREATE TABLE bss.pricing_approval_threshold (
        ...
        CONSTRAINT chk_pricing_approval_threshold_currency CHECK (
            length(currency) = 3),
        -- Exactly one basis. See the module doc: both is a choice the evaluator
        -- would have to make, neither is an entry that thresholds nothing.
        CONSTRAINT chk_pricing_approval_threshold_basis CHECK (
            (absolute_minor IS NULL) <> (percent_bp IS NULL)),
        CONSTRAINT chk_pricing_approval_threshold_absolute_non_negative CHECK (
            absolute_minor IS NULL OR absolute_minor >= 0),
        CONSTRAINT chk_pricing_approval_threshold_percent_positive CHECK (
            percent_bp IS NULL OR percent_bp > 0),
        CONSTRAINT chk_pricing_approval_threshold_version CHECK (
            version >= 0)
    )",
    "CREATE OR REPLACE FUNCTION bss.pricing_approval_threshold_no_delete() RETURNS trigger AS $$
        ...
        END;
     $$ LANGUAGE plpgsql",
    "CREATE TRIGGER trg_pricing_approval_threshold_no_delete
        BEFORE DELETE ON bss.pricing_approval_threshold
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_approval_threshold_no_delete()",
    "CREATE OR REPLACE FUNCTION bss.pricing_approval_threshold_no_update() RETURNS trigger AS $$
        ...
        END;
     $$ LANGUAGE plpgsql",
    "CREATE TRIGGER trg_pricing_approval_threshold_no_update
        BEFORE UPDATE ON bss.pricing_approval_threshold
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_approval_threshold_no_update()",
    // The Foundation-era pair this table replaces. Dropping the columns drops the
    // two CHECKs that name them; Postgres does that itself, which is why they are
    // not dropped by name here.
    "ALTER TABLE bss.pricing_policy_object
        DROP COLUMN approval_threshold_minor,
        DROP COLUMN approval_threshold_currency",
];
...
const PG_DOWN_STATEMENTS: &[&str] = &[
    ...
    "ALTER TABLE bss.pricing_policy_object
        ADD COLUMN approval_threshold_minor bigint,
        ADD COLUMN approval_threshold_currency varchar(3)",
    "ALTER TABLE bss.pricing_policy_object
        ADD CONSTRAINT chk_pricing_policy_object_threshold CHECK (
            (approval_threshold_minor IS NULL) = (approval_threshold_currency IS NULL))",
    "ALTER TABLE bss.pricing_policy_object
        ADD CONSTRAINT chk_pricing_policy_object_threshold_non_negative CHECK (
...
const SQLITE_UP_STATEMENTS: &[&str] = &[
    ...
    "CREATE TRIGGER trg_pricing_approval_threshold_no_delete
        BEFORE DELETE ON pricing_approval_threshold
        FOR EACH ROW
        BEGIN
          SELECT RAISE(ABORT,
            'pricing_approval_threshold: DELETE of a threshold version is not permitted; a threshold policy is append-only history');
        END",
    "CREATE TRIGGER trg_pricing_approval_threshold_no_update
        BEFORE UPDATE ON pricing_approval_threshold
        FOR EACH ROW
        BEGIN
          SELECT RAISE(ABORT,
            'pricing_approval_threshold: a threshold version is immutable; a correction is a new version, because an earlier version is what an approval pin covers');
        END",
    // The column drop, as a rebuild. See the module doc: SQLite refuses to drop a
    // column a CHECK names, and both of these are named in two.
    ...
    "INSERT INTO pricing_policy_object_rebuilt (
        tenant_id, tax_display_mode, default_rounding_policy_ref,
...
const SQLITE_DOWN_STATEMENTS: &[&str] = &[
    "DROP TABLE IF EXISTS pricing_approval_threshold",
    "CREATE TABLE pricing_policy_object_rebuilt (
        tenant_id                       text    NOT NULL PRIMARY KEY,
