...
//! fail-safe, and every tenant starts there. Until this table it was also a state
//! no tenant could **return** to, and the reason was the shape of
//! `m20260802_000018`'s table rather than anybody's decision: the policy is
...
//! `m20260802_000018`'s convention and it is kept for its stated reason: one
...
//! `now()` -> `(CURRENT_TIMESTAMP)`, the `bss.` prefix dropped), plus the two
...
const PG_UP_STATEMENTS: &[&str] = &[
    "CREATE TABLE bss.pricing_approval_threshold_tombstone (
        ...
        -- One version number sequence across both threshold tables, so one rule.
        CONSTRAINT chk_pricing_approval_threshold_tombstone_version CHECK (
            version >= 0)
    )",
    "CREATE OR REPLACE FUNCTION bss.pricing_approval_threshold_tombstone_no_delete() RETURNS trigger AS $$
        ...
        END;
     $$ LANGUAGE plpgsql",
    "CREATE TRIGGER trg_pricing_approval_threshold_tombstone_no_delete
        BEFORE DELETE ON bss.pricing_approval_threshold_tombstone
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_approval_threshold_tombstone_no_delete()",
    "CREATE OR REPLACE FUNCTION bss.pricing_approval_threshold_tombstone_no_update() RETURNS trigger AS $$
        ...
        END;
     $$ LANGUAGE plpgsql",
    "CREATE TRIGGER trg_pricing_approval_threshold_tombstone_no_update
        BEFORE UPDATE ON bss.pricing_approval_threshold_tombstone
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_approval_threshold_tombstone_no_update()",
];
...
const SQLITE_UP_STATEMENTS: &[&str] = &[
    ...
    "CREATE TRIGGER trg_pricing_approval_threshold_tombstone_no_delete
        BEFORE DELETE ON pricing_approval_threshold_tombstone
        FOR EACH ROW
        BEGIN
          SELECT RAISE(ABORT,
            'pricing_approval_threshold_tombstone: DELETE of a threshold version is not permitted; a threshold policy is append-only history');
        END",
    "CREATE TRIGGER trg_pricing_approval_threshold_tombstone_no_update
        BEFORE UPDATE ON pricing_approval_threshold_tombstone
        FOR EACH ROW
        BEGIN
          SELECT RAISE(ABORT,
            'pricing_approval_threshold_tombstone: a threshold version is immutable; a correction is a new version, because an earlier version is what an approval pin covers');
        END",
];
