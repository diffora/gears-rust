...
//! moved and the frozen-column arm said nothing.
//!
//! **The vehicle is a sanctioned flip, and that matters.** A content-only
//! `UPDATE` on a non-draft row is already refused — by
//! `trg_pricing_plan_flip_whitelist` and by Postgres's arm 4, both of which
//! refuse *every* non-draft `UPDATE` that is not `published -> superseded |
//! retired`. What was unrefused is content **riding** one of those two flips:
//! `SET lifecycle_state = 'retired', entitlement_grants = ...` moved the grant
//! set, which is also the realistic shape, since supersession and retirement are
//! real writes the gear performs. Corrected on review: this paragraph first
//! claimed the bare `UPDATE` succeeded, which reproduces as a refusal — the
//! defect was real, the stated reproduction was not, and the tests had the right
//! vehicle all along.
//!
...
//! waves followed the discipline on `pricing_price` -- `m20260802_000040` paid
//! for two of them at once -- and the two `pricing_plan` waves did not. Nothing
//! noticed because the census that guards this class pins an array copied from
//! the trigger and is blind to a column the trigger omits.
//!
