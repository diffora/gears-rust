...
const PG_UP_STATEMENTS: &[&str] = &[
    "CREATE TABLE bss.pricing_price_tier_band (
        ...
        CONSTRAINT chk_pricing_price_tier_band_unit_price CHECK (unit_price_minor >= 0),
        -- No zero-width band. NULL `to_qty` is the open top, not a missing value.
        CONSTRAINT chk_pricing_price_tier_band_width CHECK (
            to_qty IS NULL OR to_qty > from_qty),
        -- A band's identity is where it starts; there is no ordinal column.
    ...
    "CREATE INDEX idx_pricing_price_tier_band_price
        ON bss.pricing_price_tier_band (tenant_id, price_id)",
    "CREATE OR REPLACE FUNCTION bss.pricing_price_tier_band_kind() RETURNS trigger AS $$
        ...
        END;
     $$ LANGUAGE plpgsql",
    "CREATE TRIGGER trg_pricing_price_tier_band_kind
        BEFORE INSERT OR UPDATE ON bss.pricing_price_tier_band
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_price_tier_band_kind()",
    // The same rule read from the parent end: a row that still has bands may
    // not become a kind that carries none. Without it the forbidden pair is
    // reachable by moving the parent instead of the band.
    ...
    "CREATE TRIGGER trg_pricing_price_tier_band_parent_kind
        BEFORE UPDATE ON bss.pricing_price
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_price_tier_band_parent_kind()",
    // The parent's `lifecycle_state` is the band's. UPDATE and DELETE consult
    // OLD.price_id — the parent the band is bound to now, whose freeze governs
    // it; INSERT and UPDATE consult NEW.price_id — the parent the band would
    // land under, whose freeze forbids the append.
    "CREATE OR REPLACE FUNCTION bss.pricing_price_tier_band_append_only() RETURNS trigger AS $$
        ...
        END;
     $$ LANGUAGE plpgsql",
    "CREATE TRIGGER trg_pricing_price_tier_band_append_only
        BEFORE INSERT OR UPDATE OR DELETE ON bss.pricing_price_tier_band
        FOR EACH ROW EXECUTE FUNCTION bss.pricing_price_tier_band_append_only()",
];

// The parent-side trigger sits on `pricing_price`, which this migration does
// not own and therefore does not drop; it has to be dropped by name or it
// outlives its own table.
const PG_DOWN_STATEMENTS: &[&str] = &[
    "DROP TRIGGER IF EXISTS trg_pricing_price_tier_band_parent_kind ON bss.pricing_price",
...
// ---------------------------------------------------------------------------
//
// Systematic transforms: `bss.` dropped, `uuid` -> `text`, and each PL/pgSQL
// trigger function split into fixed-message `RAISE(ABORT, ...)` triggers, one
// per DML verb. The parent lookup is a `WHERE NOT EXISTS` subquery in the
// trigger *body*: a `SQLite` `WHEN` clause may not contain a subquery, so the
// body is the only place the cross-table read can happen. Every CHECK, the
// UNIQUE, the FK and the index are preserved.
...
const SQLITE_UP_STATEMENTS: &[&str] = &[
    ...
    "CREATE INDEX idx_pricing_price_tier_band_price
        ON pricing_price_tier_band (tenant_id, price_id)",
        ...
        BEGIN
          ...
          WHERE NOT EXISTS (
            SELECT 1 FROM pricing_price
             WHERE price_id = NEW.price_id AND model_kind IN ('graduated','volume'));
        END",
    // The parent end of the same rule. The `WHEN` clause carries the kind test
    // — a `SQLite` `WHEN` may not contain a subquery — and the body carries the
    // cross-table read, as everywhere else in this file.
    "CREATE TRIGGER trg_pricing_price_tier_band_parent_kind
    ...
    "CREATE TRIGGER trg_pricing_price_tier_band_no_insert
        ...
        END",
    // Both ends: the parent the band leaves and the parent it lands under.
...
// Dropping the band table takes its own triggers with it, but not the one this
// migration put on `pricing_price`.
