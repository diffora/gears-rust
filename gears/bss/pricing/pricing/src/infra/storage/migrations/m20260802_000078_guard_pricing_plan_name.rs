//! `pricing_plan`'s frozen-column guard gains `plan_name`.
//!
//! `m20260802_000077` added the column; this freezes it, in the same wave rather
//! than in someone else's. A plan's name is content: it is what the catalog
//! called this plan while that revision was live, and a writer moving it on a
//! published revision rewrites what an operator reading history would see. The
//! projector re-materializes truth rows on a warm re-drive (§4.4), so a name
//! moved under a frozen `CatalogVersion` comes back as though it had always been
//! that way.
//!
//! # Written because the census demanded it
//!
//! Since **D-263** both engines' frozen-whitelist censuses read the column list
//! off the **table** and the predicate off the **trigger**
//! (`the_frozen_whitelist_names_every_content_column_the_table_holds` and its
//! Postgres twin), so a column added and left unguarded reddens two suites. This
//! migration was not written from diligence; it was written because a test would
//! not let the previous one land alone.
//!
//! # Produced the same way as `m20260802_000058` and `m20260802_000062`
//!
//! `m20260802_000051`'s rule: **not by hand and not by a free-form script.** Both
//! blocks were read out of `m20260802_000062`'s own `UP` text — the guard as it
//! now stands, not as `m20260802_000001` wrote it — with one line inserted
//! before the `row_version` line in each. The generator asserted before writing
//! that it had found **exactly one** frozen-column arm per engine, that each
//! block came out **exactly one line longer**, that the column appears **exactly
//! once** per arm, and that **every original line survives** verbatim. The
//! alignment was read off the anchor line rather than chosen.
//!
//! `DOWN` restores `m20260802_000062`'s text verbatim, which is the guard as it
//! stood when the column did not exist.
...
// ---------------------------------------------------------------------------
// SQLite - the trigger dropped and recreated; it has no `CREATE OR REPLACE`.
// ---------------------------------------------------------------------------
    ...
    "CREATE TRIGGER trg_pricing_plan_frozen_columns
        ...
        FOR EACH ROW WHEN OLD.lifecycle_state <> 'draft'
          AND (NEW.plan_id              IS NOT OLD.plan_id
            ...
            OR NEW.plan_name            IS NOT OLD.plan_name
