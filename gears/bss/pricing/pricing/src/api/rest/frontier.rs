...
/// and this paragraph said it disagreed until 2026-08-18 (review R-6). It claimed
/// the trait returns a `FailedPrecondition` here and that "its signature promises
/// a `PinFrontier`". Both halves are false: the signature is
...
///
/// Worth the correction because the trait has **no implementor** (review R-5):
/// whoever writes the first one reads this file for the contract, and folding
/// "nothing to pin yet" into a refusal would meet every consumer as an outage at
/// tenant onboarding — the exact confusion both docs otherwise spend paragraphs
