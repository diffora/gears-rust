...
//! predecessor no longer the key's current row, and is refused — `LIFECYCLE_FORBIDDEN`
//! from `compose_windows`, because the successor's own window now begins exactly at the
//! changeover. That is a legible refusal and a safe direction (nothing is written), but
    ...
    OperationBuilder::post("/bss-pricing/v1/plans/{planId}/supersessions")
        ...
        .description(
            "Composes the **supersession unit** (D-88) over one canonical scope key: a successor \
             ...
             (`LIFECYCLE_FORBIDDEN`); an `existing_grandfathered` generation may never be \
