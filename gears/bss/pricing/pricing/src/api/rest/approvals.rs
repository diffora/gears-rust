...
/// `CONTENT_PIN_DOMAIN_SEP` does not move, whatever generation it stands at. (This
/// named `v14` until 2026-08-18, when the constant was already at `v15` — review
/// Z14-3. The invariant is what the argument needs; the literal is only what goes
/// stale, and `content_pin.rs:534` records that exact failure mode about itself a
/// dozen lines from the constant.) `approval_repo::re_derive` re-derives
...
async fn withdraw_approval(
    ...
    //
    // **`require_constraints: true`, and the two halves are unrelated.** This read
    // `false` until 2026-08-18, which did not merely accept an allow carrying no
    // constraints: `access_scope`'s tenant-membership assertion is conjoined with
    // that same flag, so the `Some(subject_tenant)` passed one line down did not
    // constrain the answer either. A principal whose `plan × publish` compiled to a
    // *different* tenant was handed `CatalogAuthority` here — and that is the
    // authority `authorize_decision` accepts in place of being the submitter, so an
    // authority held only in B closed a unit in A and released the canonical scope
    // keys it held. `authz`'s own rule is unconditional: `require_constraints` must
    // be `true` on every authorizing path. Being tenant-wide in *plan* is not being
