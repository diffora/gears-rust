...
//!
//! # It is a `ref`, and this surface does not validate it
//!
//! The stored value is a free-form reference to a policy this gear does not own
//! and has no vocabulary for; presence is all that is checked, here and at
//! publish. That is a **known hole** carried by the 2026-08-10 review as
//! *"`rounding_policy_ref` is an unvalidated free-form id"*, and it is not closed
//! here: a vocabulary has to come from whoever owns rounding, and inventing one
//! at this keyboard would refuse refs that are correct today. What this surface
//! does refuse is the empty string, for the reason `plan_name` does (D-318) — a
//! cleared default is spelled `null`, and two spellings of one state is a defect
//! this gear has paid for.
//!
    ...
    let router = OperationBuilder::put("/bss-pricing/v1/config/rounding-policy")
        ...
        .description(
             ...
             **refused** rather than stored, so that unset has one spelling. \
             The value is a free-form reference this gear does not validate beyond presence; it \
             names a rounding policy owned elsewhere. Unlike the approval-threshold policy this \
