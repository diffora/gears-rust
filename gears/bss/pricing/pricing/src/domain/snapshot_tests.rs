...
#[test]
fn the_composite_carries_all_three_parts() {
    // The snapshot is the aligned catalog-side entry of the Tariffs
    // composition: dropping a part here would leave the two views disagreeing
    // about what a subscriber was priced from.
    let snapshot = stamped();

    assert_eq!(snapshot.price_ids().len(), 2);
    assert_eq!(snapshot.evaluation_policy_version(), "policy-v3");
    assert!(snapshot.version_ref().pending_ref().is_some());
}
