...
#[test]
fn only_the_custom_frequency_carries_an_interval() {
    // The payload rides the variant, so the two states the three-column
    // decomposition admits — a `monthly` frequency carrying an interval, a
    // custom frequency carrying none — are unrepresentable and need no rule.
    // This test is the statement that they stay so.
    let custom = Frequency::CustomEveryN {
        n: 45,
        unit: CustomIntervalUnit::Days,
    };
    let Frequency::CustomEveryN { n, unit } = custom else {
        panic!("the custom variant is the only one carrying an interval");
    };

    assert_eq!(n, 45);
    assert_eq!(unit, CustomIntervalUnit::Days);
}
...
#[test]
fn the_shape_holds_every_slice_two_authored_value() {
    let mut subject = shape();
    subject.billing_cycle = Some(BillingCycle::Hybrid);
    subject.frequency = Some(Frequency::CustomEveryN {
        n: 90,
        unit: CustomIntervalUnit::Days,
    });
    subject.plan_tier = Some("gold".to_owned());
    subject.plan_tier_override = true;
    subject.available_from = Some(now());
    subject.purchase_min_qty = Some(1);
    subject.purchase_max_qty = Some(10);
    subject.invoice_grouping_key = Some("emea-consolidated".to_owned());
    subject.phases = PhaseGraph::new(vec![phase(1, PhaseKind::Evergreen, 0, None)]);
    subject.addon_rules = vec![AddonRule {
        addon_sku_id: Uuid::from_u128(0xadd0),
        required: true,
        min_qty: Some(1),
        max_qty: Some(5),
        step_qty: Some(1),
        price_override_ref: Some(Uuid::from_u128(0x0ffe)),
        depends_on: vec![Uuid::from_u128(0xadd1)],
        conflicts_with: vec![Uuid::from_u128(0xadd2)],
    }];
    subject.descriptor_set = Some(DescriptorSet {
        invoice_line_template: Some("{plan} {period}".to_owned()),
        gl_code: Some("4000".to_owned()),
        itemization_rule: Some("per_charge".to_owned()),
        additional: [("cost_centre".to_owned(), "emea".to_owned())]
            .into_iter()
            .collect(),
    });
    subject.baseline = Some(PublishedBaseline {
        terminal_phase_id: phase_id(1),
        phase_ids_in_use: [phase_id(1)].into(),
        available_from: Some(now()),
        available_to: None,
    });

    assert_eq!(subject.billing_cycle, Some(BillingCycle::Hybrid));
    assert_eq!(subject.addon_rules.len(), 1);
    assert_eq!(
        subject
            .descriptor_set
            .as_ref()
            .and_then(|set| set.additional.get("cost_centre").cloned()),
        Some("emea".to_owned())
    );
    // The baseline is the referent the three comparison rules need: without it
    // they either pass vacuously or block every re-publish.
    assert_eq!(
        subject.baseline.as_ref().map(|base| base.terminal_phase_id),
        Some(phase_id(1))
    );
    assert_eq!(
        subject
            .baseline
            .as_ref()
            .map(|base| base.phase_ids_in_use.len()),
        Some(1)
    );
}

#[test]
fn an_empty_descriptor_set_is_authorable() {
    // `DESCRIPTOR_INCOMPLETE` is what rejects it; the type has to hold it, or
    // the rule has nothing to report on.
    let empty = DescriptorSet::default();

    assert!(empty.invoice_line_template.is_none());
    assert!(empty.gl_code.is_none());
    assert!(empty.itemization_rule.is_none());
    assert!(empty.additional.is_empty());
}
