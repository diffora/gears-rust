...
fn foundation_plan_rules(params: &PublishRuleParams) -> ValidationPipeline<PlanShape> {
    ValidationPipeline::new()
        ...
        // it.
        //
        // This comment used to say `rounding_policy_ref` "is on every price row,
        // so no publish escapes it". It is not: the column is nullable and a
        // `None` row resolves to the tenant default, which the rule skipped and
        // `RoundingPolicyResolved` short-circuits on. That is why the rule is
        // handed the default as well — see its type doc.
