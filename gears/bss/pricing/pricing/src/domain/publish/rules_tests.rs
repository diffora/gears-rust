...
fn the_row_type_gained_no_field_and_the_generation_moved_only_for_a_rostered_one() {
    ...
    assert_eq!(
        ...
        "the generation moved to `ep-2` for `usage_counter_on_plan_change` (D-113, Slice 6), a \
         ...
         outside, so it moved no generation -- this said \"is money\" until 2026-08-18 and \
         `m20260802_000082` is the whole argument that it is a **rate** rather than a whole-minor \
         amount, which is why the reason is now stated as `unit_rate`'s, the class it shares. \
