...
///
/// This rule was the one member of that five-rule family without one (review
/// B3-1), and `let Some(kind) = subject.model_kind else { return };` was the
/// first statement of `evaluate`. `ModelKind::Package` is legal only on a usage
/// row (`inst-mk-chargekind`), so `package_size` / `package_price_minor` beside a
