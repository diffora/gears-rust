...
//!
//! That is what makes the compile re-entrant. The pre-D-130 in-place rewrite
//! destroyed its own input — the authored bounds were unrecoverable after the
//! first publish — so a supersession, repricing successor or clone built from the
//! published row carried already-offset bands *plus* the declaration and tripped
//! `ALLOWANCE_DOUBLE_FREE`, blocking the reprice of every allowance-carrying row.
//!
...
/// `amount_minor`"* into the top band, which was true until **D-311** (2026-08-11)
