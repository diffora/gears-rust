...
#[test]
fn a_voided_record_is_as_final_as_a_decided_one() {
    // The withdraw path (`inst-as-void`) frees the pinned scope key and "a fresh
    // submit opens a **new** record" — so a voided record is never re-opened,
    // and re-deciding one is the same refusal as re-deciding an approved one.
    let refusal = refused(ApprovalState::Voided, ApprovalState::Approved);
    assert_eq!(
        refusal,
        TransitionRefusal::NotPending {
            from: ApprovalState::Voided
        }
    );
}
...
fn no_state_transitions_to_itself() {
    // Re-submitting a submitted record is not a no-op; §4 opens a **new** record
    // for a re-submit, so treating the self-edge as harmless would let one
    // approval unit be re-pinned in place.
    for state in ApprovalState::ALL {
        assert!(
            state.transition(*state).is_err(),
            "{state} -> {state} must be refused"
        );
    }
}
