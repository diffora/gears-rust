...
//! added the column (`m20260802_000037`), and `contract_change` compares it —
...
//! that case is closed.
//!
//! This paragraph said the opposite until 2026-08-18 (review M-7): it declared
//! `T-14` owed — *"no arm compares it"* — while the comparison sat two hundred
//! lines below in the same file. **The cost of leaving it was a duplicate arm**,
//! and the natural place to add one is `quantity_change`, which `row_delta` runs
//! **first**; a tax-category move would then report under the wrong field name in
//! `pricing_approval.materiality`, an INSERT-only column an auditor reads years
//! later. The reported field is the operator's remediation, which is the whole
//! argument this module makes for naming one.
...
fn quantity_change(current: &PriceRow, baseline: &PriceRow) -> Option<&'static str> {
    ...
    // The fourth, added 2026-08-18 (review M-3). `LevelMaxHold` states what it
    // decides: `hold_last` carries the last observed level across a sampling gap
    // for at most this many granules, and **beyond that the level reads 0**. So
    // the field chooses, for every gap, between billing a stale level and billing
