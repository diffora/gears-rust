...
pub enum Trigger {
    ...
    /// `05-governance.md` §4 and §6 and `inst-as-unwind`, by D-333 on 2026-08-16 —
    /// so what is owed is the column and the enum variant, in this crate, on a
    /// decision already taken. (This said §4 does not declare it and that D-204
    /// clause (2) forbids minting it, until 2026-08-18: review Z21-1. Both halves
    /// were false, and together they read as a blocked external dependency where
    /// there is none.) `infra::retirement::retire_in`'s "what this function does not
