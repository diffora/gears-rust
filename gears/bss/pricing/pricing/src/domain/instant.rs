...
/// what the cutover, supersession and membership-publish `subject_ref` keys and
/// error detail already used. `time`'s well-known `Rfc3339` drops the fractional
/// second when it is zero, which would re-key every unit already in the
/// register. Not every key did: the grandfather-until subject and its
/// `pending_ref`, and every window-act subject and request id through
/// `interval_end`, were `to_rfc3339()` — that is [`format_rfc3339_offset`], for
/// the same re-keying reason in the other direction.
///
/// A value chrono's `Serialize` wrote — a frozen payload field, an outbox event
/// — was never this form; that is [`format_rfc3339_serde`].
#[must_use]
// The `.SSS` field is whole milliseconds, so the nanosecond division truncates
// by design.
...
/// RFC 3339 exactly as chrono's `Serialize` for `DateTime<Utc>` wrote it.
///
/// That impl was `to_rfc3339_opts(SecondsFormat::AutoSi, true)`: no fraction
/// on a whole second, else three, six or nine digits — whichever first divides
/// the nanoseconds evenly — and a `Z`. Frozen read-model payloads are pinned per
/// catalog version (D-136), `cohort` is compared for equality across a gear
/// boundary as a string (D-126), and outbox events are read by other gears; a
/// row projected before this migration and one projected after must spell one
/// instant one way. Neither `time`'s `Rfc3339` (minimal digits: `.1`, not
/// `.100`) nor [`format_rfc3339`] (always `.SSS`) reproduces those bytes.
#[allow(clippy::integer_division)] // the 3/6/9 grouping is the whole rule
#[must_use]
pub fn format_rfc3339_serde(at: OffsetDateTime) -> String {
    let at = at.to_offset(time::UtcOffset::UTC);
...
/// RFC 3339 exactly as chrono's `DateTime::to_rfc3339()` wrote it.
///
/// `to_rfc3339_opts(SecondsFormat::AutoSi, false)`: the 3/6/9 grouping of
/// [`format_rfc3339_serde`] with a `+00:00` designator instead of `Z`. The
/// grandfather-until approval subject (D-184) and its `pending_ref`, and every
/// window-act subject and request id built through `interval_end` — the
/// register's primary key and the value `request_id` idempotency is decided on
/// (D-190) — were rendered this way on main, so a unit opened before this
/// migration and the same act retried after it must spell the instant alike or
/// the retry opens a second unit. The `cutover/…` and `supersede/…` act labels
/// on outbox events were this form too, and so was the `changeover` of the
/// frozen repricing-run report — the document `repricing_run_content_hash`
/// pins, so a unit opened before the migration still digests to its hash.
#[allow(clippy::integer_division)] // the 3/6/9 grouping is the whole rule
#[must_use]
pub fn format_rfc3339_offset(at: OffsetDateTime) -> String {
    let at = at.to_offset(time::UtcOffset::UTC);
    let nanos = at.nanosecond();
    let fraction = if nanos == 0 {
        String::new()
...
/// RFC 3339 `serde` for request and response instants: `time::serde::rfc3339`
/// with the UTC normalisation `chrono`'s `DateTime<Utc>` did at the boundary.
///
...
/// Serializing delegates to `time` unchanged, so the wire form is untouched.
pub mod rfc3339 {
    ...
    /// Serialize as `time::serde::rfc3339` does.
    ///
    /// # Errors
    ///
    ...
    pub mod option {
        ...
        /// Serialize as `time::serde::rfc3339::option` does.
        ///
        /// # Errors
        ///
        /// Whatever the serializer reports.
        ...
        pub fn serialize<S: Serializer>(
            ...
            time::serde::rfc3339::option::serialize(at, serializer)
        }
