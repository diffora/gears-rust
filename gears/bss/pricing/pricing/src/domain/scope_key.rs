...
impl ChargeKind {
    ...
    /// — a second reading of a rule that has exactly one (review M-4). Written as
    /// the inverse of the match above so the two cannot drift: a token added to
    /// one arm is a compile error in the other.
    #[must_use]
