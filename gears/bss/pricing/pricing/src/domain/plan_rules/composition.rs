...
//!
//! Five rules over a [`PlanShape`]: the plan's tier is declared, its priced
//! metered lines are unambiguous, and the three properties D-16 asks of the
//! plan-authored add-on graph hold. Each is the half of its instruction that
...
//!
//! `inst-cmp-injective` used to read "each usage plan revision maps **exactly
//! one** `meteringUnit`". That claim was contradicted by three rules of its own
//! slice — D-84's per-market completeness ranges over "every
//! `(meter, dimensionKey)` line the plan prices", the slice's own D-84
//! integration case exercises a plan pricing two meters, and D-43's grant
//! `applicability` scopes to a **set** of published meters — and it was enforced
//! by nothing: the partial `UNIQUE` that does the enforcing carries `meter`
//! **and** `dimension_key`, so it always implemented the per-line reading. The
//! invariant this rule restates is therefore **one priced line per
//! `(meter, dimensionKey)` per scope-key slice**, and a plan pricing cloudlets,
//! storage and egress is one plan rather than three.
//!
...
//!   free, because the pre-D-116 wording required one canonical key to "cover
//!   every market" and a single canonical key fixes one market — the check as
//!   written before D-116 was unsatisfiable, not merely unimplemented.
//!
