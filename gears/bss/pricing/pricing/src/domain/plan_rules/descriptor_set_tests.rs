...
fn the_two_row_borne_elements_are_not_checked_here() {
    ...
    // And a complete descriptor set publishes even though this crate has no
    // `tax_category_ref` column anywhere: the element is unchecked in this gear
    // altogether, which is a gap to report rather than one to close here.
    assert!(
