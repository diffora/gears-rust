...
pub mod actions {
    ...
    /// third of its three halves to say so** (review T-2). `05-governance.md` §5
    ...
    /// schedule, bulk import, repricing run. Until a transport exists the only
    /// falsifiable artifact is this record of the gap.
...
pub async fn access_scope(
    ...
    let scope = enforcer
        ...
        .map_err(|e| match e {
            ...
            | authz_resolver_sdk::EnforcerError::CompileFailed(_) => AuthzError::Denied(Box::new(
                pdp_denial(ctx, rt, action, resource_id, owner_tenant_id, e.to_string()),
            )),
