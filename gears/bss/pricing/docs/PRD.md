...
**Protocol/Format**: model kind, ordered tier bands, `tierAggregationWindow`, `billingGranularity`, `priceEligibility`/`grandfatherUntil`, per-key `PriceWindow` **intervals** + coverage end (D-99 — intervals, never a point-in-time flag, so "active at `t`" is derived at read time), reserved-rate/flavor, derived-meter definition, `customerGroup` `PriceOverlay` + resolved membership; all frozen in `pricingSnapshotRef` (Design). **Second lane — the `migrated-origin` snapshot (D-102, 2026-07-31 review fix):** a `migrated-origin` ref resolves through **no** `CatalogVersion` by construction (D-76/D-87), so it cannot be served off the read model; Rating/Tariffs fetch its self-contained payload + provenance from `GET /v1/pricing/migrated-origin-snapshots/{subscriptionRef}` under the service identity's `plan × read` (Design 11 `inst-sy-surface`). Until this lane the payload D-87 requires them to evaluate from had no reader-facing surface anywhere in the set.

**Compatibility**: No default substitution; absence of a required field must have failed publish; Tariffs never reads draft state; a consumer pins only a **pin-eligible** `CatalogVersion` (D-101 — committed *and* every subject row of that version warm-complete, so one pin never resolves two different contents over time).
...
Normative relationship: **supersession is versioning scoped to one canonical scope key** — it MUST create a new immutable `Price` row (never mutate in place) and MUST open/close the corresponding `PriceWindow` rather than overlap it, executing only as the atomic **supersession unit** (D-88: successor row + window shorten + window schedule, one approval unit, one transaction, changeover instant ≥ approval commit + the max batching-delay SLO). Supersession operates **within one `priceEligibility` class and one `chargeKind`**, and on usage rows it additionally preserves `modelKind` and the unit/counter-determining evaluation-policy fields (D-82 + D-98) — supersession changes the price, never what or how the key meters nor which formula prices it. An `existing_grandfathered` row is **immutable in price** — it MUST NOT be superseded; the **only** permitted mutation is **setting or tightening `grandfatherUntil`** (never loosening, never the price), which is a **material change** (AC #28). No mechanism may produce overlapping active `PriceWindow` rows for **one** canonical scope key (manifest invariant, as extended in §2.2).
