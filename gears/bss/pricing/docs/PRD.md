...
Publish **MUST NOT** proceed without the complete billing descriptor set per manifest §4.1 / D-48 v1 (invoice line template, tax category, GL code, composition/itemization rules — plus `billingTiming`, which **rides each recurring price row** as a `pricing_price` column rather than a descriptor-set field); the validation report **MUST** list any missing descriptor fields. The set **MUST** be sufficient for Billing/ERP to post without re-querying mutable catalog rows; extension is additive-only (D-48).
...
**Direction**: provided by this PRD to Subscriptions — **two-way**: **two** inbound lanes — (1) grandfathering re-bind reporting (recorded 2026-07-28, previously implicit) and (2) the **migration execution handshake** (recorded 2026-07-29: execution-start declaration + processed-scope report)

**Protocol/Format**: eligible plans; migration targets + `PlanMigrationScheduled`; phase map + `convertsToPhaseId` + `displayTrialDays`; proration input contract (`billingAnchorPolicy`, `prorationBasis`, `creditOnDowngrade`); plan-change contract (`allowedChangeTargets`, `comparabilityRank`); entitlement grant set; sellability gate (all six predicates: active window, committed version, availability dates, lifecycle state, GA flags, registry `sellable` flag — standalone lines only, D-46); grandfathering eligibility surface (`priceEligibility`/`cohort`/`grandfatherUntil` + the read-time `EligibilityExpirySignal`) with the **inbound re-bind feedback**: at each renewal of a subscription bound to an expired generation, Subscriptions reports the outcome — the released generation on re-bind, still-bound otherwise — the input the `pricing.grandfather.expiry_signal_backlog` alarm consumes (Design 07). **Migration execution handshake (inbound lane 2, Design 11 §5):** before its first `PlanLink` batch Subscriptions **MUST** call `POST /v1/pricing/migrations/{id}:start`, which flips `scheduled → in_progress`, runs the D-36 execution-time re-validation of contract locks and boundary deltas, and returns the **exclusion set** Subscriptions MUST honour; Subscriptions re-reads the schedule state per processing batch thereafter (so a D-34 in-flight cancel is observed), honours the `(migration_id, subscription)` dedup key (M2), and closes with `POST /v1/pricing/migrations/{id}:complete` carrying the processed/excluded/failed sets. Without this lane the `scheduled → in_progress → completed` transitions have no trigger and D-34/D-36/D-38 rest on an unspecified handshake (Design).
...
**Protocol/Format**: billing descriptor set (invoice line template, tax category, `glCode`, itemization rule) frozen into `CatalogVersion`; plus `billingTiming` per recurring row (riding the price row — the fifth element of the D-48 v1 contract) (Design).

**Compatibility**: Sufficient to render an invoice line and post to GL **without** re-querying mutable catalog rows; the v1 minimum field list is **pinned (D-48, 2026-07-28)**: invoice line template/labels, `taxCategory`, `glCode`, itemization rule, `billingTiming` — additive extension only; Billing countersigns at its gear PRD.
...
- **And** `modelKind=flat` on a usage row MUST fail publish (`MODEL_KIND_CHARGEKIND_MISMATCH`) — `flat` is a non-usage kind (D-18 matrix, S3 `inst-mk-chargekind`)
...
- NFR targets (read-model availability/DR RPO/RTO 99.9%/5m/30m, mass-repricing >= 50 rows/s, plan/tier size caps 100/500 + interval caps 366d/24m, idempotency-key TTL 24h) — **ratified 2026-07-28 by the product owner as launch defaults**; the mass-repricing figure is perf-test-verified against the tenant worst case.
...
| **Usage-based** | Charge by metered consumption | Requires parent SKU `meteringUnit`; tier bands or an untiered `per_unit` rate; MUST set `billingGranularity` on **all** usage rows (untiered or tiered); MUST set `tierAggregationWindow` **when tiered or `package`** |
...
| Billing descriptors | Publish MUST include the complete billing descriptor set per manifest §4.1 / D-48 v1 (four descriptor-set fields; `billingTiming` rides each recurring price row) |
...
| Per-unit quantity source | `modelKind=per_unit` **non-usage** rows MUST persist a `quantitySource` (`subscription_seat_count` \| `manual`); catalog MUST NOT infer or compute the quantity. On a `per_unit` **usage** row the meter supplies `Q`, so `quantitySource` MUST be **absent** — presence MUST fail publish (2026-07-28 review fix; S3 `inst-mk-required`) |
| Evaluation-policy placement | `tierAggregationWindow` and `billingGranularity` are **usage-row only**; on `flat` and on `per_unit` **non-usage** rows they MUST be **absent** — presence MUST fail publish. A `per_unit` **usage** row (the plain untiered metered rate) carries `billingGranularity` like every usage row |
...
| `reservationFlavor` | `consumption` (matched usage at `reservedRate`, remainder on-demand) **or** `capacity` (allocated quantity billed at `reservedRate` regardless of usage — `capacityCharge`); aligned field-for-field with Tariffs `reservationMatch`. On a **non-`sum` row** only `capacity` is authorable — `consumption` fails publish (`LEVEL_RESERVATION_CONSUMPTION_FORBIDDEN`, D-53) |
| Evaluation policy | The row **is** a usage row: `billingGranularity` is REQUIRED, and `tierAggregationWindow` is REQUIRED **when tiered or `package`** — the "forbidden on non-usage rows" rule does **not** apply |
