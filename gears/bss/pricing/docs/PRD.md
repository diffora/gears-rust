...
| **Term** | **Definition** |
|----------|----------------|
| **Plan** | Catalog entity binding a published SKU to a **billing cycle** (one-time, recurring, usage-based, hybrid), add-on rules, mandatory **PlanTier**, and optional phase price schedules (**D-373** removes the grouping field). |
...
| **Billing descriptor contract** | Five elements per published snapshot (**D-48 + D-373**): **four row-borne** — `invoiceLineTemplate`, `glCode`, `taxCategory`, `billingTiming` — and **one derived**, `itemizationRule`, from the bundle's `invoice_itemization` (`itemize` for any other plan). The plan half is `billing { itemizationRule, ext }`, with D-152's additive extension in `pricing_plan.descriptor_ext`; there is no descriptor-set entity. Template and GL are resolved and frozen per row at publish. Authored literals and plan/phase names remain **single-language today**; per-locale **authoring** remains the registry-localization extension question, while **Presentation** owns rendering localization. Billing renders the frozen template with its locale and period; `{sku}` and `{unit}` inherit localized registry values at the pinned `CatalogVersion` (**D-373**). |
...
| **chargeKind** | Price-row axis distinguishing the charge components a plan carries at once: `recurring`, `usage`, or `one_time`. Part of the canonical scope key, so mixed components on one `planId` are **distinct** rows, not duplicates. Plan behaviour is derived from the lines a phase actually carries; there is no live plan-type token. |
| **one_time_setup** | ~~A `chargeKind` for an optional one-time setup/activation charge on a recurring/hybrid plan.~~ — *withdrawn from the live contract 2026-09-19: setup is a `one_time` line beside other kinds; `ChargeKind` has three tokens.* |
...
| **invoiceGroupingKey** | ~~Optional Plan field that groups billable lines onto the same invoice/section; consumed by Billing for line layout. Optional (empty = no grouping). It is a **layout hint only** and MUST NOT override the single-currency-per-invoice invariant: Billing MUST split lines of different currencies onto separate invoices even when their grouping keys match.~~ — *withdrawn by D-373, 2026-09-16* — no consumer reads the layout hint. |
...
| **dimensionKey** | Identifier for a usage dimension on a `(skuId, dimensionKey)` rated line; dimensional pricing by event properties is Future scope. |
...
The single scope key for row-uniqueness, supersession, `PriceWindow` non-overlap, and window coverage is **`(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)`**, extended on `chargeKind = usage` by the pair **`(skuId, dimensionKey)`** — an axis of the key **conditionally** (normative, D-196, product owner 2026-08-06: every usage line of one plan in one `(currency, region, phase, priceEligibility, cohort)` shares `chargeKind = 'usage'`, so under the eight axes alone a plan's second meter was refused `DUPLICATE_SCOPE_KEY` at authoring), with the pair's **absence spelled per surface** on a non-usage row (clarified 2026-08-26 — this sentence said "the `''` sentinel" unqualified, and there are three answers): the `meter` **column** is nullable, the two **unique indexes** key over `COALESCE(meter, '')`, and the **rendered** canonical key carries `none` in both positions, which is `ScopeKey`'s absent-axis token and the form a cross-gear fixture compares. `chargeKind` in `recurring | usage | one_time | one_time_setup` distinguishes the price components a single plan legitimately carries at once; `cohort` (default `none`) is the grandfathering **generation** discriminator — set to the cutover instant on `existing_grandfathered` rows only, so repeated cutovers create coexisting generations — a hybrid plan holds a `recurring` **and** a `usage` row (optionally a `one_time_setup` row) on one `planId`, and a **one-time plan's base row is `one_time`**, so they are **distinct keys**, not duplicates.
...
| Billing descriptor contract per invoice line | `p1` | D-373: invoice template, GL code, tax category and billing timing are row-borne; itemization is derived from bundle `invoice_itemization` (`itemize` for non-bundle plans) |
...
The catalog **MUST** support one-time, recurring, usage-based, and mixed charge lines on one plan, with the catalog requirements in §17.1. A Plan **MUST** reference a **published** SKU and be storable in `draft` before publish — and, because it is storable incomplete, publish **MUST** reject an ordinary phase with no logical charge lines (`PHASE_CHARGE_LINES_EMPTY`) and a revision that carries recurring lines with no `frequency` (`RECURRING_FREQUENCY_REQUIRED`). Publish **MUST** also reject a logical line with no monetary binding in a sold `(currency, region)` (`LINE_MARKET_PRICE_MISSING`; an explicit zero operand counts). Recurring frequency **MUST** support `monthly`, `quarterly`, `semiannual`, `annual`, and `customEveryN{Days|Months}(n)`. **Supersedes** D-149 cycle dispatch (`billing_cycle`, `CYCLE_METADATA_MISSING`, `BASE_MARKET_INCOMPLETE`).
...
A plan **MAY** mix recurring and usage lines on the same `planId`; publish validation **MUST NOT** require both parts. The read model **MUST** expose every line under one `planId` for Tariffs evaluation. Every logical line **MUST** have a monetary binding in every sold `(currency, region)` (`LINE_MARKET_PRICE_MISSING` otherwise) — a plan must never be sellable in a market whose events would fail closed; a market where a line is genuinely free authors an explicit `$0` binding, never an absence. **Supersedes** `fr-hybrid-completeness` / `USAGE_MARKET_INCOMPLETE` / `HYBRID_INCOMPLETE`.
...
#### Optional one-time charge beside other kinds

- [ ] `p2` - **ID**: `cpt-cf-bss-pricing-fr-one-time-setup`

A plan **MAY** declare a `chargeKind=one_time` line beside recurring and/or usage lines on the same `planId`. `one_time_setup` is **not** a live kind. Timing of a one-time charge (activation vs trial conversion vs plan-change) is consumer-contract, not a distinct catalog kind.

**Rationale**: Mixed kinds are derived from actual lines; a fourth kind duplicated `one_time`.
...
Publish **MUST NOT** proceed without the complete billing descriptor **contract** per manifest §4.1 / D-48 v1 (**D-373**): **four row-borne elements** — `invoiceLineTemplate`, `glCode`, `taxCategory`, `billingTiming` — and **one derived element**, `itemizationRule`, read from the bundle's `invoice_itemization` (`itemize` for any other plan). Required-ness belongs to the publish rules: `billingTiming` is required on recurring rows; itemization is derived, never authored on the plan, and needs no publish gate. The v1 contract's five-element **content** is unchanged. The validation report **MUST** list any missing element, naming the row for the row-borne ones. For `taxCategory` the required element is the row's **effective** category — `coalesce(row.tax_category_ref, region readiness default)` — which publish **MUST** resolve and freeze with the row, and a row whose effective category resolves to nothing **MUST** fail publish regardless of the tenant tax-display policy (**D-154**, 2026-08-03: the policy's warn mode governed that arm, so a pinned v1 element could be published absent; and freezing only the raw column left Billing re-resolving the fallback against the tenant-declared region taxonomy, a mutable row, which is exactly what this FR's sufficiency clause forbids). The contract **MUST** be sufficient for Billing/ERP to post without re-querying mutable catalog rows; extension is additive-only (D-48) and the extended required-set is declared **per tenant in `pricing_policy_object`** (**D-152**, 2026-08-03 — "config-extensible without a schema change" had named no surface anywhere in the design set, so the validator could only ever check the pinned three). **D-373 (2026-09-16)** updates the dated D-152 carrier and census: the tenant's `additional_required_descriptors` keys are checked against `pricing_plan.descriptor_ext`, the only authored plan-level descriptor content; the snapshot plan half is exactly `billing { itemizationRule, ext }`. D-110's dated rationale remains: `billingTiming` on each recurring row (2026-07-28) and `taxCategory` as each row's tax-category reference (**D-110**, 2026-07-31 review fix — the category is per row, so a single per-plan descriptor column could not hold it).

**Row resolution and delegation (D-373):** `invoiceLineTemplate` = `coalesce(row.invoice_line_template, tenant.default_line_templates[charge_kind])`; an empty result **MUST** block publish with `DESCRIPTOR_INCOMPLETE` (422), naming `priceId`. `glCode` = `coalesce(row.gl_code_ref, tenant.default_gl_code_ref)`; an empty result **MUST** block publish with `GL_CODE_UNRESOLVED` (422), naming `priceId`, regardless of tenant policy. Publish **MUST** freeze the results as `resolved_invoice_line_template` and `resolved_gl_code`; later policy edits cannot change frozen rows. A declared GL vocabulary checks each row's effective code for active membership (`GL_CODE_UNKNOWN`, one finding per row); an empty vocabulary constrains nothing. Missing plan extension keys use `DESCRIPTOR_INCOMPLETE` without a row. Slice 2 remains this requirement's sole claimant; the row half is delegated — **resolved and frozen per row — see Slice 3 §6, Slice 4** — and recurring timing remains Slice 6's rule.

**Template vocabulary (D-373; Slice 2 §3):** exactly seven placeholders are accepted: `{sku}` (the row SKU's localized `displayName`), `{sku_code}` (its `sku_code`), `{unit}` (its localized, sales-facing `unitDisplayLabel`, never metering-unit identity), `{plan}` (`planName`), `{phase}` (phase `display_name`), `{dimension}` (row `dimension_key`), and `{period}` (the billed service period). Billing reads the first three from the registry read model at the **pinned `CatalogVersion`**, the next three from the pricing snapshot, and supplies its own period. Pricing freezes the **template string**, never the rendered label; pinned registry reads satisfy the no-mutable-requery promise. An unknown placeholder **MUST** fail the row or policy **write** with `LINE_TEMPLATE_INVALID` (422). Authors may use `{dimension}` to distinguish usage dimensions and `{phase}` to distinguish phases, or override any row; the shipped defaults do not include those placeholders.

**Tenant defaults (D-373):** `pricing_policy_object.default_gl_code_ref` is nullable; `default_line_templates` ships with these three ASCII literals, keyed by `charge_kind`:

| `charge_kind` | Shipped default template |
|---|---|
| `recurring` | `{sku} - {period}` |
| `usage` | `{sku}, {unit}` |
| `one_time` | `{sku}` |

Tenants may edit or empty each default through the policy door (**D-373**); an empty default with no row override fails publish under the row resolution rule above. The default GL code belongs to the tenant, with no plan or SKU default.

**Rationale**: Invoice reproducibility requires a complete, frozen descriptor contract at publish (**D-373**).
...
A subscription/purchase **MUST NOT** be created unless **all six sellability predicates** hold at `t` for **every canonical scope key the purchase binds on the bound `(currency, region)`** — the recurring/usage/`one_time` components and every phase of the chain, eligibility-resolved, grandfathered generations excluded; one failing component key blocks the plan-market, never a partial sale (D-94, joint rule with Subscriptions — the bundle conjunction below is the bundle-specific statement of this same rule): (1) an **active** (not merely scheduled) `PriceWindow` covers the key **and the key's active-plus-scheduled coverage extends through `now + the longest billing cycle sold on the key`** (D-80 — an open-ended window satisfies this trivially; a finite-covered key stops selling one full cycle before its coverage ends, so no purchase can land in a trailing void; the read model exposes the per-key coverage end); (2) the plan is addressable in a **committed `CatalogVersion`** (pending fan-out / unwarmed read model is not sellable); (3) plan-level `availableFrom`/`availableTo`, when set, are open at `t`; (4) the plan's lifecycle state is not retired; (5) no GA gate flags the bound market (per-market `not_sellable_ga`, prepaid-execution gate); (6) **every SKU the sale includes** — the plan's own, every charge row's component, every bundle member's and every component of a member plan — is open for new sales in the registry's **current published** state (`published`, not deprecated, `sellable = true`; products gear, D-46 as amended by **D-376** / products P-D-176). There is **no component exemption** and none for a zero price or a phase not yet entered; the set is frozen with the pricing terms, an empty set is `not_evaluable` rather than vacuously true, every closed SKU is named, and an unreadable registry is `not_evaluable`, which a consumer refuses — so a registry outage stops new sales, by intent. The read model **MUST** expose all six predicates; Subscriptions enforces the conjunction. The gate governs the creation of **new** subscriptions only: a renewal of an existing subscription is not a purchase and **MUST NOT** be blocked by it (retirement and passed purchasability dates never kill in-flight renewals; those follow the grandfathering/migration mechanics). For a bundle, the gate evaluates the **conjunction over its components**: the bundle is sellable at `t` only while every referenced component key passes predicates (1)–(5) **and predicate (6) holds over the bundle's whole frozen sale set** — the bundle SKU, every member plan's SKU and every component those plans price (D-376; the exemption D-46 gave components is withdrawn) — plus the bundle's own purchasability dates. Optional plan-level `availableFrom`/`availableTo` (the unified pair for **all** billing cycles), when set, **MUST** be validated against window coverage at publish; deferred publish ("publish at T") is out of launch scope.
...
**Protocol/Format**: **D-373** preserves D-48's five-element contract: four row-borne elements — resolved `invoiceLineTemplate`, resolved `glCode`, effective `taxCategory` (D-110/D-154), and `billingTiming` (required on recurring rows) — frozen with each row; one derived element, `itemizationRule`, from `pricing_bundle.invoice_itemization` for bundle plans and `itemize` otherwise. The plan section is exactly `billing { itemizationRule, ext }`, with `ext` from `pricing_plan.descriptor_ext` (D-152); there is no descriptor-set entity (Design).

**Compatibility**: Sufficient to render an invoice line and post to GL **without** re-querying mutable catalog rows; the v1 minimum field list is **pinned (D-48, 2026-07-28)**: invoice line template/labels, `taxCategory`, `glCode`, itemization rule, `billingTiming` — of these, `billingTiming` and `taxCategory` ride the price row (D-110) while the other three are descriptor-set fields; additive extension only; Billing countersigns at its gear PRD. `glCode` MAY additionally be governed by a **tenant-declared GL-code vocabulary** checked at publish (**D-356**, 2026-09-09): an empty vocabulary constrains nothing, and the v1 field list is unchanged — ledger stores account *class* and takes the concrete code from this snapshot, so the vocabulary is Catalog's to hold. **D-373 (2026-09-16)** supersedes the dated D-48/D-110 shape census above: **four row-borne, one derived**, with template/GL resolution from row overrides or tenant defaults at publish, never at rendering time. GL vocabulary membership is checked per row. Billing renders the frozen template using its locale/period and registry labels at the pinned `CatalogVersion`; those versioned reads satisfy sufficiency. The five-element content, additive-only extension and pending Billing countersign are unchanged.
...
| Billing descriptors panel | As a Finance Manager, I want invoice and GL fields per price row so that Billing renders each line correctly (**D-373**) | 1. Open the plan publish checklist<br>2. Review each row's template and GL tenant defaults; set optional row overrides and resolve its tax category<br>3. Read derived itemization on the plan; edit bundle itemization on the bundle builder<br>4. Resolve validation gaps named per row and any missing plan extension keys | — |
...
- **And** trial **runtime** mechanics (payment-method requirement, auto-convert vs. expire — enforcing the catalog-published `phaseDurationDays`) remain owned by Subscriptions Trial Management

**9. Billing descriptor completeness**
- **Given** a Plan submitted for publish
- **When** a row has no effective template, GL code or tax category after fallback resolution, a recurring row lacks `billingTiming`, or a required plan extension key is missing (**D-373**)
- **Then** publication MUST be blocked
- **And** the validation report MUST list every missing element, naming `priceId` for row-borne elements; an empty template uses `DESCRIPTOR_INCOMPLETE`, an empty GL code uses `GL_CODE_UNRESOLVED`, and missing plan extension keys use `DESCRIPTOR_INCOMPLETE` without a row (**D-373**)
...
- **And** published snapshots MUST include `billing.itemizationRule` derived from the Bundle's `invoice_itemization`; non-bundle plans MUST carry `itemize`, with no authored plan-level itemization field or publish gate (**D-373**)
...
- **Given** a Plan with a published price row on a canonical scope key `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` + on `chargeKind = usage`: `(skuId, dimensionKey)` (conditional, D-196)
...
- **Given** a published billing descriptor contract with four row-borne elements and one derived element (**D-373**)
- **When** Billing/ERP consumes the snapshot to render an invoice line and post to GL
- **Then** each row MUST carry resolved `invoiceLineTemplate`, resolved `glCode`, effective `taxCategory` and `billingTiming` (required on recurring rows), and the plan half MUST be `billing { itemizationRule, ext }`, sufficient to post **without re-querying mutable catalog rows**; Billing renders the frozen template with its locale/period and registry labels at the pinned `CatalogVersion` (**D-373**)
- **And** publish MUST fail if any element of that agreed minimum set is absent (the v1 list is **pinned — D-48, 2026-07-28**: invoice line template/labels, `taxCategory`, `glCode`, itemization rule, `billingTiming`; three are descriptor-set fields while **`billingTiming` and `taxCategory` ride the price row** — D-110, since a tax category is per row and a per-plan descriptor column could not mirror it; additive extension only). **D-373 (2026-09-16)** replaces the dated shape census with **four row-borne elements and one derived element**; publish resolves and freezes row template/GL from row overrides or tenant defaults, resolves tax under D-154, requires recurring timing and checks D-152 plan extension keys. Itemization is derived from the bundle (`itemize` otherwise), so it needs no publish gate.
- **And** when the tenant has declared a GL-code vocabulary, a present `glCode` outside it MUST fail publish with `GL_CODE_UNKNOWN`; an empty vocabulary MUST constrain nothing (**D-356**, 2026-09-09 — an optional membership check on one v1 element; the pinned list is unchanged). **D-373 (2026-09-16)** applies this dated D-356 membership check to each row's effective GL code, with one finding per row naming `priceId`; retirement is refused with `TAXONOMY_VALUE_IN_USE` while a published row's `resolved_gl_code` names the value, excluding drafts.
- **And** row or tenant-policy writes containing an unknown template placeholder MUST fail with `LINE_TEMPLATE_INVALID`; changing tenant defaults after publish MUST leave frozen row template/GL values unchanged (**D-373**)
...
- **When** published
- **Then** the catalog read model MUST publish the trial phase's `kind` and `phaseDurationDays`, from which preview/quoting shows the trial length (e.g. "14-day trial") — there is no separate `displayTrialDays` member (D-377)
- **And** Subscriptions MUST enforce trial runtime **from this same published value** (single source: catalog authors, Subscriptions enforces)
...
- **Given** a **published** price row on the canonical scope key `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` + on `chargeKind = usage`: `(skuId, dimensionKey)` (conditional, D-196) — **whether or not** subscriptions are currently bound to it
...
| Upstream SKU retirement/unpublish joint contract while plans reference it? | **Answered — joint contract closed by cross-reference (D-47)**: the registry never retires a SKU with active plan references (its `SkuReferenceCount` 3-state predicate, fail-closed without a fresh watermark — products PRD `fr-reference-signal`/`fr-retirement-eol` side; the id `fr-sku-lifecycle` cited here until 2026-08-25 never existed); pricing flags referencing plans on the registry's deprecation signal and blocks new adoption (AC #82 side). Both PRDs now cite each other's half. | 2026-07-28 |
| `SkuReferenceCount` watermark — who produces it? | **Answered (product call, 2026-08-25): pricing is the sole v1 producer** of the registry's `SkuReferenceCount` per-producer watermark (products PRD `fr-reference-signal`: "as of `T`, complete live plan→SKU reference set is {…}"), built **jointly with the products gear development** and delivered before products v1 GA; Subscriptions/Contracts register as producers at their own build. Shape owned by the products Design. | 2026-08-25 |
| Minimum Billing/ERP descriptor field set + PSP/ERP posting contract beyond `glCode` + events? | **Answered — catalog-side v1 pinned (D-48)**: the descriptor set = **invoice line template/labels, `taxCategory`, `glCode`, itemization rule, `billingTiming`** — sufficient to render the line and post to GL with no catalog re-query (AC #53); extension is **additive only**. Status: tracked dependency — Billing countersigns when its gear PRD lands. **D-373 (2026-09-16)** updates the shape to four row-borne elements and one derived element; Billing renders the frozen template from the pricing snapshot and registry labels at the pinned `CatalogVersion`, satisfying the no-mutable-requery promise. Countersign remains pending on this shape and on whether line grouping is needed and who owns it. | 2026-07-28 |
...
| **Localization** owner for Plan-owned display fields (extend the registry localization mechanism to Plan/phase/descriptor content; Presentation renders)? (F-37) | **Open** — descriptors are single-language today. Owner: Architecture + Presentation. **D-373 (2026-09-16)** clarifies that authored template literals remain single-language while `{sku}` and `{unit}` inherit registry localization at the pinned `CatalogVersion`; per-locale authoring of plan-owned content remains open. | — |
...
| ~~Upstream SKU retirement joint contract open~~ — **closed by cross-reference (D-47, 2026-07-28)** | Draft/published plans may reference a retired SKU | Closed: the registry never retires a SKU with active plan references (products PRD `fr-reference-signal` `SkuReferenceCount` predicate, fail-closed); pricing flags referencing plans and blocks new adoption (AC #82) |
...
|-------------------|------------|---------------------------|
| **One-time** | Single purchase | A `one_time` charge line **per sold `(currency, region)`** — uniqueness is the canonical scope key (§2.2); absence of a monetary binding fails publish `LINE_MARKET_PRICE_MISSING`; optional quantity min/max; optional availability window; MUST NOT attach a recurring-only add-on — an add-on SKU none of whose published plans carries a `one_time` line, refused `ADDON_INCOMPATIBLE` by the composition step (D-149 placement, still live) |
| **Recurring** | Subscription fee (`monthly`, `quarterly`, `semiannual`, `annual`, or `customEveryN{Days\|Months}(n)`) | Recurring charge lines with a monetary binding **per sold `(currency, region)`** (`LINE_MARKET_PRICE_MISSING` otherwise); links to `PriceWindow`; supports plan phases and `per_unit` (per-seat) pricing; `billingTiming` (`in_advance`\|`in_arrears`) REQUIRED at publish; MAY also carry `one_time` and `usage` lines; `frequency` persisted as metadata — required only when recurring lines exist (`RECURRING_FREQUENCY_REQUIRED`); publish MUST also reject non-positive `n` |
| **Usage-based** | Charge by metered consumption | Requires parent SKU `meteringUnit`; tier bands or an untiered `per_unit` rate; MUST set `billingGranularity` on **all** usage rows (untiered or tiered); MUST set `tierAggregationWindow` **when tiered or `package`** (D-58); consecutive phase-local usage definitions MUST stay counter-compatible (`PHASE_USAGE_INCOMPATIBLE`) |
| **Mixed** | Any combination of the three live kinds | Same `planId` MAY declare recurring, usage and `one_time` lines together; there is no hybrid plan type and no `one_time_setup` kind. An ordinary phase with no logical lines fails `PHASE_CHARGE_LINES_EMPTY`. |
...
| Descriptor completeness | Publish MUST NOT proceed without the complete billing descriptor contract (**D-373**): effective row template, GL and tax, recurring timing, and required plan extension keys; itemization is derived from the bundle (`itemize` otherwise), with no publish gate |
...
**Reserved-capacity price (catalog primitive)** — reservation is modeled as **attributes on the single `usage` price row**, **not** a second row, so it does not price the same `(skuId, dimensionKey)` twice:
