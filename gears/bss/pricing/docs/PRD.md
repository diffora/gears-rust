...
Charge artifacts and `BillableItem`s **MUST** carry catalog refs and `pricingSnapshotRef` per manifest §4.1; posted invoice periods **MUST NOT** re-query mutable catalog rows. Publish **MUST** stamp identifiers sufficient for the manifest `pricingSnapshotRef`. The catalog-side stamp is the **pending `CatalogVersion` ref** and the **resolved price ids**; the third part this reference names, the **evaluation-policy version**, has no producer, format or owner in any document of this gear or of Rating, and publish therefore stamps **no placeholder** for it (**D-161**, 2026-08-03, found while building the publish commit). That is deliberate rather than an omission: Rating's snapshot composer fail-closes on a missing pricing pre-stamp and never fabricates a segment, so an absent value refuses at the boundary that owns the check while an invented one satisfies it, rides posted periods immutably, and can never afterwards be told from a real version. What the segment *is* — a vocabulary generation, a content digest over the frozen evaluation-policy fields, or nothing at all once those fields resolve through the price ids at the pinned version — is an open fork for the product owner jointly with Rating ([`DECISIONS.md`](./DECISIONS.md) §F.1), and it is **launch-blocking**: while nothing is stamped, that composer rates no charge.
