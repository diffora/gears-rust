...
**Struck 2026-08-16 by D-330.** `cpt-cf-bss-pricing-fr-historical-import-governance` **leaves the
...
- **Struck 2026-08-16.** The criterion's requirement, `cpt-cf-bss-pricing-fr-historical-import-governance`, left the requirement set in the same wave, and the flow it tested (Slice 5 §2) left the design set. Nothing it asserted is owed: no restricted role, no mandatory reason, no row-shape parity subset, no disjoint reference store (D-76), no always-material import unit (D-13), no zero-billable-effect predicate
