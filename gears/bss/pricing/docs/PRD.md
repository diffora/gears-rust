...
- **Given** a Plan with a published price row on a canonical scope key `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` + on `chargeKind = usage`: `(meter, dimensionKey)` (conditional, D-196)
...
- **Given** a **published** price row on the canonical scope key `(planId, currency, region, priceOverlay, phase, priceEligibility, chargeKind, cohort)` + on `chargeKind = usage`: `(meter, dimensionKey)` (conditional, D-196) — **whether or not** subscriptions are currently bound to it
