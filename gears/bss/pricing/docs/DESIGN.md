...
| [`design/10-advanced-primitives.md`](./design/10-advanced-primitives.md) | 6.10 | 3 | Reserved capacity (p1), prepaid credit grant (p2), publish-compiled `includedAllowance` (p1 — D-45: $0 band + marker / carry grant), derived (composite) meter formula-as-data (p2), `discountRef` hook, typed `minQtyThreshold`, trailing-tier qualification (D-40). |
