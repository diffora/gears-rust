...
Keys and values are tenant-owned and validated with the four DIM errors. The registry starts seeded with region, declared with no values; the first price naming it stores the seed. A referenced value cannot be removed even when its row is historical, and registry edits require no approval (spec §5, §14).
