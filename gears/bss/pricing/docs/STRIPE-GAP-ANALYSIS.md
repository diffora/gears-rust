...
> **Status: COMPLETE (2026-07-28) — retained as a dated analysis record.** All four gaps are
> dispositioned: G-1 → Promotions PRD (Future owner; `discountRef` is the committed day-1
...
the invoice credit balance). **Actioned → D-48 (2026-07-28)**: the joint-contract line is pinned —
**drawdown applies to the post-discount, pre-tax amount; a credit reduces the charge, it never
pays the invoice** (one doctrine with metered-`creditUnit` grants, which net quantity and are
pre-tax by construction), with an explicit **revisit checkpoint at Tax Engine GA** (MVP sells
tax-exclusive, so the placement is dormant until then — Finance re-confirms before inclusive
tax activates). Carried next to the grant definition in
[10-advanced-primitives.md](./design/10-advanced-primitives.md) and mirrored in the rating
consumer contracts until Billing docs exist; Billing countersigns at its gear PRD.
