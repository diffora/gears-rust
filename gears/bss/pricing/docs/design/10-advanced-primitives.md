...
| `source_scope_key` | — | **D-129:** on `source = compiled_allowance`, the allowance-carrying row's **canonical scope key** (the **ten** axes — the eight plus the conditional usage pair `(meter, dimensionKey)`, D-196; stating it as eight here reproduced the very `DUPLICATE_SCOPE_KEY` defect D-196 removed, one store over: compiled-allowance grants come from usage rows, so a plan with two metered lines each carrying `rollover = carry` renders one key and the `UNIQUE` below refuses the second grant — item 14 of the 2026-08-26 products review) — the issuance binding Billing matches the subscription's resolved key against (`inst-ac-carry`). Stable across supersession by construction; `UNIQUE (plan_id, plan_revision, source_scope_key)` |
