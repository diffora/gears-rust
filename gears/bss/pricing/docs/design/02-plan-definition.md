...
| `cpt-cf-bss-pricing-actor-subscriptions` | Consumes phase map (a trial's length is its `trial` phase's `phaseDurationDays` — D-377), sellability inputs |
...
`convertsToPhaseId`, terminal phase); billing descriptor completeness;
...
add-on rules, `planName` (D-318), and **D-373**
...
2a. [ ] - `p1` - **The plan's own SKU carries the role its plan realizes (normative, D-376, 2026-09-20):** an ordinary plan's SKU MUST be an `offer`; a plan carrying a bundle composition's MUST be a `bundle` — the **context** decides which, from the composition it can see, never from an authored field — else `PLAN_SKU_TYPE_INVALID`; a `component` cannot own a plan. A deprecated SKU may not be **newly** bound (`PLAN_SKU_DEPRECATED`), while a plan already published against one keeps it (D-370's rule for rows, applied here). Judged at create, at a change of binding, at clone, in bulk and at publish. It is judged **independently of the rows**, because the row rule (S3 `inst-pr-sku-role`) exempts the plan's own SKU by identity and therefore never looks at it: a plan with no rows, or whose every row prices a component, would otherwise publish sold as a component or as an uncomposed bundle SKU. **`sellable` is deliberately not read**: an offer closed for new sales is a legitimate binding an author must be able to maintain; whether the plan may be *sold* is S7's predicate (6) - `inst-pl-sku-role`
...
2. [ ] - `p1` - **Every applicable logical line in an ordinary phase owes a monetary binding in every sold market** (`LINE_MARKET_PRICE_MISSING`, 422, phase, line scope and market named). Sold markets follow `currency_binding::sold_markets` (grandfathered-only generations are not required markets), including bundle components on selling generations. An explicit zero operand counts; absent operands or an absent variant do not. Eligibility, cohort and overlay applicability is preserved rather than multiplying incompatible selection classes. Bindings are matched on line scope (eligibility/cohort/overlay). Parent SKU `meteringUnit` and usage `billingGranularity` / `tierAggregationWindow` remain Slice 3's rules - `inst-cs-market-price`
...
**Steps**:
0a. [ ] - `p2` - **Plan name (D-318, 2026-08-15; required by D-382, 2026-09-21):** a plan **MUST** carry a human label, `planName`. Before D-318 it carried none and every surface showing a plan to a person rendered its `PlanTier`, a classification the catalog reasons about, or eight characters of a UUID; D-318 added the column but left it optional, so that state stayed reachable by omitting one member of the create. The column is **`NOT NULL` with no default** and **frozen once published** like every other content column, so a rename is a new revision. **Every door that makes a plan names it**: `POST /plans` carries the member as a required string — absence is refused by the transport, before any rule runs — and `POST …/plans/{planId}/clone` takes a body naming the copy, because a clone still does not carry its source's name. An empty or whitespace-only name, and one longer than **120 characters** (counted in characters, not bytes), are refused at the **write stage** with `PLAN_NAME_INVALID`, and the publish rule judges the same predicate so the two cannot drift. Framed into the approval content pin (`v13`) and shown in the reviewer's pinned document — a name is what a consumer surface calls the plan, so a swap between submit and approve changes what a buyer is told they are buying. **No uniqueness constraint**, deliberately: identity is `planId`, two plans may share a label, and the check would cost a scan per publish to buy a convention an operator can hold - `inst-cmp-planname`
...
4. [ ] - `p1` - ~~A `trial` phase publishes `displayTrialDays` = its `phaseDurationDays`~~ — **withdrawn (D-377, 2026-09-20).** A trial's length **is** `phaseDurationDays` on a phase whose `kind` is `trial`; there is no second name for it, no `display_trial_days` column, and a request that still sends `display_trial_days` is refused **by name** as an unknown field rather than dropped. The shape this rule existed to stop — a phase publishing a trial length it does not have — is unrepresentable once the projection is gone, and the two neighbours that covered its edges stay: a non-terminal phase without a duration is `PHASE_DURATION_INVALID`, a terminal phase of kind `trial` is `TERMINAL_PHASE_KIND_INVALID`. The frozen phase payload carries `kind` and `phaseDurationDays` and **no** `displayTrialDays` key. D-151's diagnosis stands as the reason; the code it minted, `DISPLAY_TRIAL_DAYS_INVALID`, is retired - `inst-ph-trial`
...
2. [ ] - `p1` - **Row template (D-373 R2)**: effective template = `coalesce(row.invoice_line_template, tenant.default_line_templates[charge_kind])`. An empty result fails publish with `DESCRIPTOR_INCOMPLETE` (422), naming `invoiceLineTemplate` and `priceId`; otherwise publish freezes the template string as `resolved_invoice_line_template`. At both the row and policy write doors, a placeholder outside the vocabulary below is refused with `LINE_TEMPLATE_INVALID` (422), so publish never receives a malformed template - `inst-ds-template`
   - [ ] **Write syntax (D-373)**: the row and tenant-policy write doors accept only the seven placeholders below and escaped literal braces `{{` / `}}`; malformed braces and unknown names produce `LINE_TEMPLATE_INVALID` before persistence - `inst-ds-template-syntax`
...
`PHASE_OVERRIDE_UNIT_MISMATCH` (422 — **superseded:** terminal-phase override units; live continuation is `PHASE_USAGE_INCOMPATIBLE` / `unit_determining_mismatch`. Historical: a phase-scoped usage override changing the
...
`PHASE_OVERRIDE_ORPHANED` (422 — **superseded:** terminal-phase usage inheritance; live lookup is phase-local. Historical: a phase-scoped usage row with no phase-invariant
terminal-phase row of its `(skuId, dimensionKey)` line; D-117 — without the base the D-89
...
| ~~`display_trial_days`~~ | — | **removed (D-377)**: it was `phase_duration_days` under a second persisted name. A trial's length is `phase_duration_days` on a `kind = 'trial'` row |
...
(the `display_trial_days` drift `CHECK` that stood here went with its column — **D-377**); the add-on rule's three quantity
...
one priced line per `(skuId, dimensionKey)` **per scope-key slice**
...
the SKU's unless an explicit audited override; **one priced line per `(skuId, dimensionKey)` per
...
**per `(skuId, dimensionKey)` line per scope-key slice** — a plan MAY price several meters
...
non-terminal phase without a duration), and — **superseded:** "plans whose cycle carries a recurring part
(`recurring`/`hybrid`)" is not live cycle dispatch; `inst-ph-coverage` is scoped by actual recurring lines — **recurring coverage of every phase per sold
`(currency, region)`** (`PHASE_UNCOVERED` otherwise; **superseded:** usage rows are not phase-invariant by terminal-phase inheritance — per-phase lookup and `inst-ph-usage-compatible` replace `PHASE_OVERRIDE_UNIT_MISMATCH` / `PHASE_OVERRIDE_ORPHANED`; `PhaseOverrideBase` / `PhaseOverrideUnits` remain as types and stay unregistered), and — on every plan, phased or not — **refusing a price row
...
copied rows name and minting a fresh one when they name two or more), and stating a trial's
length as the `trial` phase's own `phaseDurationDays` — the single source Subscriptions reads
for trial runtime; there is no `displayTrialDays` projection beside it (**D-377**).
A phase MAY carry an optional operator **`displayName`** (**D-357**): free-form, no behaviour, versioned with the revision, framed by the approval content pin and projected as presentation (`null` when absent); a blank label MUST read back as absent. The kind roster is **`trial` | `interim` | `evergreen`** (**D-358**): `intro` MUST be refused at the write (`400`, `phases.kind`) and at the store, while frozen snapshots taken before the rename keep `"kind": "intro"` and MUST NOT be rewritten.
...
- [ ] A phased plan trial→interim→evergreen publishes its phase map, the trial phase carrying `kind: trial` and its `phaseDurationDays` and no `displayTrialDays` key (D-377); a cyclic `convertsToPhaseId` fails
...
D-373 implementation verification: price content is replaced as a whole; `null` or
omission of `gl_code_ref` / `invoice_line_template` clears an override. Tenant defaults
are read and replaced through `GET/PUT /bss-pricing/v1/config/billing-descriptors` with
mandatory `If-Match` on the write. Their four-key string map is physically checked on
both engines.
