...
| `state` | `enum` | `active \| retired`; retire rejected while referenced by an active published price row (`region`) or an active brand-scoped `PriceOverlay` scope (`brand`); `retired → active` re-activation is allowed (audited; governed once a published row or overlay names the value — D-353, D-355). Re-activation is a `PATCH …/values/{value}` naming `active`: the whole-set `PUT` that once re-activated by re-adding the value is gone (D-353) |
