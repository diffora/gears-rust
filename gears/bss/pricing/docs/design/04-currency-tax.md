...
| `GET` | `/bss-pricing/v1/config/taxonomies/{region\|brand\|partner\|org_tier}` | The whole taxonomy, `active` and `retired` alike (admin read; partner/org_tier added by D-120). A tenant holding no region row reads the seeded `global` (D-354). Each value carries `editGoverned` — whether a published row or overlay names it, hence whether its edit needs a second principal (D-355) — folded from the same reference read the `PATCH` gate performs, one per declared value, and carrying the same `references: { published_price_rows, active_overlay_scopes }` counts as the by-value read, including zeroes, without additional queries (**D-361**). **Neither read's `ETag` covers the flag**: the tag is over the authored fields alone, so a publish elsewhere can move the flag without moving the tag and a conditional read may be answered `304` while it has changed; the flag is advisory and the `PATCH` response is the truth. Each segment is the class's own scope token — the camelCase `orgTier` this row used to spell is refused rather than aliased (**D-241**). The `PUT` that shared this row is **removed by D-353** | ETag (conditional read) |
| `POST` | `/bss-pricing/v1/config/taxonomies/{class}/values` | **Declare one value** (D-353): `201` with the value's own `ETag` and `Location`; the value is its own key — a repeat with the same body replays (`200`), other content for a held value is `409 TAXONOMY_VALUE_EXISTS`. Commits at once: declaring is not the governed act | natural key (the value) |
| `GET/PATCH` | `/bss-pricing/v1/config/taxonomies/{class}/values/{value}` | **Read / edit one value** (D-353). The `GET` hands out the value's **own** `ETag` and, with it, `editGoverned` + `references` (the two reference counts, D-355), so a client knows which door the `PATCH` will be before it opens one; the `PATCH` asserts the tag and is the door of §3 step 4 — `200` immediately when no published row or overlay names the value (D-355); otherwise `202 submitted_for_approval` on submission, followed by atomic application at approve; GET returns the new value without another PATCH (**D-362**); `409 TAXONOMY_VALUE_IN_USE` when a guard refuses; `404` on an undeclared value | ETag (per value) |
...
| `GET` | `/bss-pricing/v1/config/rounding-policies` | The **declared rounding vocabulary** — the set a row's `roundingPolicyRef` and the tenant default are membership-checked against; an empty set constrains nothing (D-334). The `PUT` that shared this row is **removed by D-368**; the set `ETag` stays a read validator and no write asserts it | ETag (conditional read) |
| `POST` | `/bss-pricing/v1/config/rounding-policies/values` | **Declare one reference** (D-368): `201` with the value's own `ETag` and `Location`; the value is its own key — a repeat with the same body replays (`200`), other content for a held value is `409 TAXONOMY_VALUE_EXISTS`. Commits at once — this vocabulary opens no approval unit on any edge (D-334) | natural key (the value) |
| `GET/PATCH` | `/bss-pricing/v1/config/rounding-policies/values/{value}` | **Read / edit one reference** (D-368). The `GET` hands out the value's **own** `ETag`; the `PATCH` asserts it and commits at once. `409 TAXONOMY_VALUE_IN_USE` when a published price row or the tenant default still names a value being retired; `409 STALE_VERSION` on a moved tag; `404` on an undeclared value | ETag (per value) |
| `GET` | `/bss-pricing/v1/config/gl-codes` | The **declared GL-code vocabulary** — the set a plan's billing-descriptor `glCode` is membership-checked against at publish (`inst-ds-glcode`, S2 §3); an empty set constrains nothing. Not this slice's subject either — listed here because this table is where every config vocabulary is, and it is the rounding vocabulary's shape on the descriptor plane (D-356). Its `PUT` is **removed by D-368** too | ETag (conditional read) |
| `POST` | `/bss-pricing/v1/config/gl-codes/values` | **Declare one code** (D-368): the rounding vocabulary's row on the descriptor plane | natural key (the value) |
| `GET/PATCH` | `/bss-pricing/v1/config/gl-codes/values/{value}` | **Read / edit one code** (D-368). `409 TAXONOMY_VALUE_IN_USE` when a **published** plan revision's descriptor set still names a code being retired | ETag (per value) |
...
**Touches**:
- API: `GET /bss-pricing/v1/config/taxonomies/{class}`, `POST /bss-pricing/v1/config/taxonomies/{class}/values`, `GET/PATCH /bss-pricing/v1/config/taxonomies/{class}/values/{value}` (D-353)
