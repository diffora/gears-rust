...
| C2 | Taxonomies are tenant config | `region` and `brand` value sets are tenant-configured taxonomies; membership is validated at save/publish (unknown value fails before publish). **A tenant's region universe is never empty**: until the tenant declares a region row it reads as the seeded `global` (**D-354**) | PRD §4.1 |
...
3. [ ] - `p1` - No FX derivation ever: a missing `(currency, region)` row is simply absent — preview/publish paths fail closed on it, no base-currency fallback (Future `currencyFallbackPolicy` only). **What is *not* a missing row (D-379, 2026-09-20): a region with no row of its own resolves the currency's `global` row**, the price the plan authored for every region. Resolution for `(overlay O, currency C, region R)` at `t` is the covering window on `(O, C, R)`, else `(O, C, global)`, else `(base, C, R)`, else `(base, C, global)`, else refused — no FX, no other currency, and **never another region's row**. An override is a whole row (nothing inherited from `global`); an override whose window ends falls back, which the coverage report discloses (S7); a consumer resolves once at binding and pins the `priceId`. Region is the **buyer's** commercial territory — not where a resource runs, which is a `dimension_key`. **Tax on a `global` row is undecided** (no Tax gear): this step touches neither C4 nor `RegionTaxReadiness`, a `global` row is judged exactly as any region's, and one consequence is owed to the Tax owner — `MarketBasisUniform` groups by the exact pair and so cannot see a `DE` invoice mixing a tax-inclusive `DE` line with a tax-exclusive `global` one - `inst-mc-nofx`
...
5. [ ] - `p1` - **Every tenant's region universe starts as `{global}` (D-354, 2026-09-09).** While a tenant holds **no** region row, the region taxonomy reads as one active value, `global` (label `Global`, no default tax category, `taxRatePresent = false` — the fail-closed markers, since the seed asserts no tax fact nobody declared): `GET …/vocabularies/region`, `GET …/values/global`, `inst-tx-region`'s universe and the `RegionTaxReadiness` reads all answer it, so a fresh tenant publishes `global` rows at once. The seed is **virtual while no row exists** — nothing is written on a read — and the tenant's first region write (any `POST …/values`, a `PATCH` of `global` itself, a bulk `replace`) writes the row first, inside its own transaction and without an audit record: the seed is the gear's declared default, not a tenant's act. Afterwards `global` is an ordinary value, edited under step 4 like any other — and for a tenant that has published nothing against it, step 4's ungoverned arm, so the seed does not reintroduce as a ceremony what it exists to remove (D-355); a tenant that retires every region holds rows and has the empty universe it asked for - `inst-tx-seed`
...
**`pricing_org_tier_taxonomy`** (PK `(tenant_id, value)`; the region table reads as `{global: active}` while a tenant holds no row in it and gains that row on the tenant's first region write — **D-354**; the last two added by D-120 as the
