...
| C2 | Taxonomies are tenant config | `region` and `brand` value sets are tenant-configured taxonomies; membership is validated at save/publish (unknown value fails before publish). **A tenant's region universe starts empty**, like every other, and a tenant declares a region only to price it differently from the currency-wide price; membership is judged on a row that **states** a region, and a row that states none is not its subject (**D-381**, retiring D-354's seed) | PRD §4.1 |
...
3. [ ] - `p1` - No FX derivation ever: a missing `(currency, region)` row is simply absent — preview/publish paths fail closed on it, no base-currency fallback (Future `currencyFallbackPolicy` only). **What is *not* a missing row (D-379, 2026-09-20; re-filed by D-381, 2026-09-21): a region with no row of its own resolves the currency-wide row**, the row that states **no region at all** — the price the plan authored for every region. Resolution for `(overlay O, currency C, buyer region R)` at `t` is the covering window on `(O, C, R)`, else `(O, C, -)`, else `(base, C, R)`, else `(base, C, -)`, else refused — no FX, no other currency, and **never another region's row**; a buyer with **no** region tries the two region-less steps only. An override is a whole row (nothing inherited from the currency-wide one); an override whose window ends falls back, which the coverage report discloses (S7); a consumer resolves once at binding and pins the `priceId`. Region is the **buyer's** commercial territory — not where a resource runs, which is a `dimension_key`. **Tax on a currency-wide row: the verdicts hold by construction** (D-381) — it names no region, so there is no regional default category and no declared rate, and it must state its own `taxCategory` (D-154) while a `taxInclusive = true` one stays policy-governed. One consequence is still owed to the Tax owner — `MarketBasisUniform` groups by the exact market and so cannot see a `DE` invoice mixing a tax-inclusive `DE` line with a tax-exclusive currency-wide one - `inst-mc-nofx`
...
**Steps**:
1. [ ] - `p1` - A `region` a price row **states** MUST be a member of the tenant's configured region taxonomy; an unknown/invalid region fails validation **before** publish. **A row that states no region is not this rule's subject (D-381, 2026-09-21)**: it is the currency-wide price, which is not a taxonomy value and cannot be declared, deprecated or retired — so a tenant that does not segment its market by territory declares nothing and publishes, which is what D-354's retired seed existed to arrange - `inst-tx-region`
...
3. [ ] - `p1` - Taxonomy mutation (add/deprecate/retire a region/brand/partner/orgTier value) is tenant-admin config, audited. **Deprecating** a value — **D-370**'s middle state — is rejected by nothing: it withdraws the value from new use and changes nothing about what already resolves through it, so there is no reference for a guard to protect, and it is the one move available to a value in published use. **Retiring** a value is rejected while it is referenced by an active published price row (`region`) **or an active `PriceOverlay` scope of any taxonomy-backed class** — `brand`, `region`, `partner`, `orgTier` (D-120, 2026-07-31 review fix: the guard previously enumerated price rows and brand overlays only, so a region value retired cleanly while region-scoped overlays still named it) — referential integrity over every referencing shape. **A region's `taxCategory` marker is guarded on the same principle and as a separate act (D-245)**: it may not be *removed* while a published row states no category of its own and resolves through it. The act is distinct because it is reachable by a `PUT` that retires nothing, and because its remedy is different — nothing is retargeted; the operator re-declares the default or authors `taxCategoryRef` onto the dependent rows. It is guarded rather than left to the publish rule because a dependent row may be an immutable `existing_grandfathered` generation that can be neither superseded nor `PATCH`ed, so dropping the default would fail every subsequent publish of its plan on `TAX_BASIS_INCOMPLETE` with no way back. **The guard's counts are deliberately not serialised against a concurrent publish (D-243)** — they are plain reads inside the mutation's transaction, so a retirement judged against zero references can commit beside a publish creating one. The resolving order is stated rather than emergent: `inst-tx-region` refuses a price row whose region is not `active`, so whichever transaction commits first wins and the loser is refused on its **next** publish (held by `domain::publish::rules_tests::a_row_in_an_undeclared_region_fails_the_registered_set`). Locking the counted set was declined — at C1's twenty-currency floor it is large, and it would make a config write block publishing - `inst-tx-mutation`
4. [ ] - `p1` - **The mutation surface is per value, and an edit is governed once something published names the value (D-353, D-355, 2026-09-09).** `POST …/vocabularies/{class}/values` **declares** one value and commits at once — a new value references nothing, so nothing a second principal could protect has moved, and publishing against it is already under materiality. `PATCH …/vocabularies/{class}/values/{value}` **edits** one — a relabel, a **deprecation** (D-370), a retirement or re-activation, a region's tax markers. **The edit is governed only while a published price row (`region`) or a published overlay scope names the value (D-355):** then it is the always-material `taxonomyValueMutation` door (registered in S5's roster) — the door judges the edit first (step 3's retire guard and D-245's cleared-category guard, so a unit that could never commit is never opened), opens the unit over the proposal **and the value as held** (`inst-ap-pin`: a value moving under a submitted unit fails the re-derived pin), answers `202 submitted_for_approval`, and the independent approve atomically applies the patch, decision and audit (**D-362**); both guards are re-judged inside that transaction. Re-read GET afterwards, with no second PATCH. **While nothing published names the value the edit commits at once** — `200`, no unit, one audit record with no `approval_ref` — on the same reasoning that leaves declaration ungoverned: nothing a second principal could protect has moved. Retirement therefore has no governed case at all: a referenced value's retirement is refused by step 3's guard, an unreferenced one's is the operator's own. **Deprecation is the case that does have both** (D-370): a referenced value may be deprecated and nothing else, so `active → deprecated` is precisely the governed edit an operator reaches for — it is the only one the guard admits on a value something published names. The unit's `subject_kind` is `taxonomy_value` — its own token, not `policy`, because the approval store's *one pending policy proposal per tenant* index keys on `policy`. Each commit is one audit record naming the value (`taxonomy/{class}/{value}`), with its state before and after and the unit it ran under; a declaration is one `create` record. The whole-set `PUT` this slice specified is **removed**: it retired and re-labelled values without a second principal, so left standing beside a governed `PATCH` it was an open door next to the gate. Its two truths survive here — a value is retired, never deleted, and absence-from-body is no longer a spelling of retirement at all - `inst-tx-value-mutation`
5. [ ] - `p1` - **RETIRED by D-381 (2026-09-21): nothing is seeded.** D-354 read a tenant's region universe as `{global: active}` until it declared a row, so that `inst-tx-region` — fail-closed, and judging every price row against a taxonomy value — could not refuse a tenant that has no territories. The premise is gone: a price row that states **no** region is the currency-wide price and is not that rule's subject (step 1), so a fresh tenant's region universe is **empty**, like every other universe it holds, and its first region write materialises itself and nothing beside it. `global` is once again an ordinary region a tenant may declare, read, deprecate and retire under step 4 like any other value. The id is kept so citations resolve; the rule it named is withdrawn - `inst-tx-seed`
...
|--------|------|---------|-------------|
| `GET` | `/bss-pricing/v1/plans/{planId}/preview` | Base-price preview per `(currency, region)`; `region` is **optional** and a buyer with no territory is quoted the currency-wide price, `resolvedRegion` answering `null` when it served (D-381); fail closed, overlay disclaimer | — |
| `GET` | `/bss-pricing/v1/config/vocabularies/{region\|brand\|partner\|org_tier}` | The whole taxonomy, `active` and `retired` alike (admin read; partner/org_tier added by D-120). A tenant holding no region row reads an empty set, like every other universe (D-381, retiring D-354's seed). Each value carries `editGoverned` — whether a published row or overlay names it, hence whether its edit needs a second principal (D-355) — folded from the same reference read the `PATCH` gate performs, one per declared value, and carrying the same `references: { published_price_rows, active_overlay_scopes }` counts as the by-value read, including zeroes, without additional queries (**D-361**). **Neither read's `ETag` covers the flag**: the tag is over the authored fields alone, so a publish elsewhere can move the flag without moving the tag and a conditional read may be answered `304` while it has changed; the flag is advisory and the `PATCH` response is the truth. Each segment is the class's own scope token — the camelCase `orgTier` this row used to spell is refused rather than aliased (**D-241**). The `PUT` that shared this row is **removed by D-353** | ETag (conditional read) |
| `POST` | `/bss-pricing/v1/config/vocabularies/{class}/values` | **Declare one value** (D-353): `201` with the value's own `ETag` and `Location`; the value is its own key — a repeat with the same body replays (`200`), other content for a held value is `409 TAXONOMY_VALUE_EXISTS`. Commits at once: declaring is not the governed act | natural key (the value) |
| `GET/PATCH` | `/bss-pricing/v1/config/vocabularies/{class}/values/{value}` | **Read / edit one value** (D-353). The `GET` hands out the value's **own** `ETag` and, with it, `editGoverned` + `references` (the two reference counts, D-355), so a client knows which door the `PATCH` will be before it opens one; the `PATCH` asserts the tag and is the door of §3 step 4 — `200` immediately when no published row or overlay names the value (D-355); otherwise `202 submitted_for_approval` on submission, followed by atomic application at approve; GET returns the new value without another PATCH (**D-362**); `409 TAXONOMY_VALUE_IN_USE` when a guard refuses; `404` on an undeclared value | ETag (per value) |
...
*vocabulary*. `{class}` still carries the four scope classes only; the rounding
and GL-code vocabularies keep their own last segment under the same prefix,
because their door declares no `202` arm, no tax markers and no reference counts
and one template cannot advertise both contracts. Products' twin route family is
`P-D-175` in that gear's register.

**D-364:** Both taxonomy GETs additionally return `pending_approvals` on every
value, including `[]`. Current fields stay current; proposed fields are separate.
Pending metadata and `content_access` are config-read data; only approval-read
access to a unit exposes its `proposed_changes` and `content_matches_pin`.
The same authored-ETag caveat applies: refresh unconditionally for pending and
permission changes. See [wire examples and access rules](./ui-read-contracts.md#taxonomy-pending-proposals).
...
region/partner/org_tier-scoped overlays), `TAXONOMY_VALUE_EXISTS` (409 — a `POST …/values` naming a
value the tenant already declares with other content, or a retired one; the remedy is `PATCH` on
that value — D-353), `TAX_BASIS_INCOMPLETE` (422 — per policy on the
...
**`pricing_org_tier_taxonomy`** (PK `(tenant_id, value)`; the region table **starts empty** like the other three and holds exactly what the tenant declared — **D-381**, retiring D-354's seed, because the currency-wide price states no region and needs no value to stand for it; the last two added by D-120 as the
...
| `state` | `enum` | `active \| deprecated \| retired` (**D-370**); **retire** rejected while referenced by an active published price row (`region`) or an active `PriceOverlay` scope of the class; **deprecate** rejected by nothing — it withdraws the value from new use while everything already published against it keeps resolving, and it is the only move a referenced value has, since retirement is refused for exactly that value. `deprecated → retired` is guarded like `active → retired` (the guard is keyed on the destination — **D-369**). `deprecated → active` and `retired → active` re-activation is allowed (audited; governed once a published row or overlay names the value — D-353, D-355). Re-activation is a `PATCH …/values/{value}` naming `active`: the whole-set `PUT` that once re-activated by re-adding the value is gone (D-353). Only `active` validates anything: every universe read (`active_regions`, `overlay_repo::declares`, the readiness map) filters `state = 'active'`, which is the entirety of what `deprecated` means. The customer-group taxonomy is deliberately **not** in D-370's scope and holds two states |
...
**Touches**:
- API: `GET /bss-pricing/v1/config/vocabularies/{class}`, `POST /bss-pricing/v1/config/vocabularies/{class}/values`, `GET/PATCH /bss-pricing/v1/config/vocabularies/{class}/values/{value}` (D-353)
