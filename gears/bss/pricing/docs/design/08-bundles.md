...
4. [ ] - `p2` - **Which plan rules a row-less bundle answers to (normative, 2026-08-01 review fix, C-1):** a `sum_of_parts` bundle plan carries **no price rows of its own**, so the Slice-2/3/7 rules that quantify over a plan's rows have nothing to bind and are **not** applicable to it: the §17.1 cycle matrix (`inst-cs-onetime`/`inst-cs-recurring`/`inst-cs-usage`/`inst-cs-hybrid` — its effective `frequency` is the **components'** matched one per `inst-bc-frequency` — derived, never an authored shape to satisfy, and undefined when no component recurs (2026-09-19: there is no `billing_cycle`)), D-15 phase coverage (`inst-ph-coverage`; the plan carries only the D-19 implicit terminal phase — `COMPONENT_PHASED` already keeps phases out of composition entirely), the row-borne descriptor elements (`billingTiming`/`taxCategory` ride the **component** rows, D-48/D-110), and `inst-wc-required` window coverage (sellability is the component conjunction, `inst-bc-sellability`). What **does** bind: the mandatory `PlanTier`, the descriptor-set fields, `availableFrom`/`availableTo`, and this slice's own composition rules. `own_price` bundles are the opposite case and answer to all of them (`inst-bb-own`). Stating this is not a widening: read literally, the row-based rules made a `sum_of_parts` bundle unpublishable whatever its lines were. **D-373 (2026-09-16)** updates the dated C-1 census: all four row-borne descriptor elements (`invoiceLineTemplate`, `glCode`, `taxCategory`, `billingTiming`) ride component rows; the row-less bundle still answers to the plan extension-key check on `descriptor_ext` (Slice 2 `inst-ds-required`). The snapshot derives `itemizationRule` from its bundle row, with no descriptor-set fields to author or gate. - `inst-bb-rowless`
...
2. [ ] - `p2` - **Recurring** components MUST match `frequency` (a monthly + annual mix cannot sum onto one invoice line set); components with **no recurring line** — usage-only or one-time-only — are outside this rule (their charges rate per their own rows, or are charged once on phase entry — `inst-bb-sum`). **Whether a component recurs is read off its lines, not its plan row (2026-09-20):** with the plan type gone a plan is what its lines are, and an author may leave a `frequency` on a plan whose only line is `one_time`; judged by the column alone such a component answered `FREQUENCY_MISMATCH` for a charge made once. The row-less bundle's own effective frequency is derived from its compatible recurring components and stays undefined when none needs it - `inst-bc-frequency`
...
4. [ ] - `p1` - **Bundle sellability (normative):** the Slice 7 gate evaluates a bundle as the **conjunction** over its components — sellable at `t` iff **every** referenced component key passes gate predicates (1)–(5) at `t` (plus the bundle's own `availableFrom`/`availableTo`). **Predicate (6) holds over the bundle's whole frozen sale set (D-376, 2026-09-20, superseding the exemption D-46 gave components):** the bundle SKU, every member plan's own SKU and every component those plans price must each be open for new sales in the registry's current published state. `sellable = false` prohibits new sales of the SKU **in any composition**, so closing sales on a member — or on a component of a member — closes new sales of the bundle; every closed SKU is named. *"Composition-only"* is what the **role** `component` says (products P-D-176), not what the flag says: a member is a component **plan**, and an ordinary member plan's own SKU is still an `offer`. For `sum_of_parts` there are no own rows, so components are the only inputs; for `own_price` the bundle's **own** rows must pass **and** the component keys too (the matching-currency component set is part of the offer). The frozen component key set spans `priceEligibility = all_subscriptions` (`cohort = none`) keys **only** — grandfathered generations are never gate inputs (2026-07-28 review fix, confirmed 2026-07-31). One unsellable component makes the bundle unsellable, never partially-sellable - `inst-bc-sellability`
...
3. [ ] - `p2` - `invoiceItemization` (`aggregate | itemize`) persists and MUST preserve per-SKU rev-share either way (Marketplace accrues per SKU regardless of invoice layout), and is the snapshot's `itemizationRule` (**D-373**) - `inst-rs-itemization`
...
publish (`COMPONENT_PHASED` — Future gate), and component `one_time` lines charge on bundle phase
entry as their own traceable occurrences (2026-09-20). A composed bundle's derived
`charge_kinds` are its effective component lines' for `sum_of_parts` and its own lines' alone
for `own_price`. Creation, component add/remove/replace,
