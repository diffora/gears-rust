...
**Physical floor still owed (D-373, spec §10):** verify whether `default_line_templates`
needs a `CHECK` requiring exactly the four `charge_kind` keys or relies on the policy write
door alone; no such `CHECK` is claimed landed here. The new migration follows D-372
(price-row SKU, register entry lands with its own programme); its four price-column guards
and plan `descriptor_ext` carrier are implementation work, not changes to shipped migrations.
