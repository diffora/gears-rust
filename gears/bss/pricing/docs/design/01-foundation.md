...
revision's publish commits — D-90, so at most one revision per plan is ever `published` (partial
`UNIQUE`) and "the current revision" is storage-defined, never a max-scan convention — and
`published → retired` at retire); a shape change on a published
...
fix). Versions store **per-subject deltas** (D-86, subject-typed by D-91): resolving `(pin, subject)`
