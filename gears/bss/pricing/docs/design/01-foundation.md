...
`meter` is frozen derived content: the write door resolves it from the row SKU's `metering_unit`; clients cannot author it. Two SKUs with the same unit remain distinct keys; changing the unit of one SKU does not create another key. Usage rows require a metered SKU; fee rows require an unmetered SKU. The row may name the plan's own SKU or a non-sellable resource SKU, and the registry must report it published. These rules run at save and again at publish using one registry snapshot per request. `dimensionKey` on a fee row is refused with `USAGE_LINE_AXIS_MISMATCH`.

The former D-196 `(meter, dimensionKey)` identity and meter-line index are historical. Migration 000044 replaces their storage mechanism with the required SKU column and two partial scope indexes (§3.7). Authored `sku_id` is carried by the scope key, `dimension_key` by content; neither can move on an existing row. Delete the draft and create another to change its key.
...
- `chargeKind ∈ {recurring, usage, one_time}` — exactly three (2026-09-19; `one_time_setup` is withdrawn and a setup charge is an ordinary `one_time` line) — distinguishes the lines a single plan legitimately carries at once: a plan may hold a `recurring` **and** a `usage` **and** a `one_time` line on one `planId` — so they are **distinct keys**, not duplicates. There is no plan type to declare the mix: a plan is what its lines are.
...
2. **Every other input the rule set reads is held by a named mechanism, and the list is
   ...
   add-on rules): that same revision tag, which every child edit bumps. **D-373** places
   `descriptor_ext` on the draft revision under its tag and descriptor overrides on price
   rows under their own tags; no separate descriptor child remains. The
...
outside:
  ...
  invoice_line_template  billing descriptor - frozen source, D-373
  gl_code_ref            billing descriptor - accounting reference, D-373
