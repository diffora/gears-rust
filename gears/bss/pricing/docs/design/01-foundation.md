...
`retired → draft`/`published` edge, and its open draft revision is deleted in the same
transaction), so **no later publish would ever re-project it** and the read model would
