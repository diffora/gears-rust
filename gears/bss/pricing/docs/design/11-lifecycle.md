...
| `SnapshotSynthesizer` | Builds + freezes a `migrated-origin` `pricingSnapshotRef` with provenance, and serves it on the D-102 read surface — the **only** per-subscription snapshot the catalog composes (the named exception to D-30's Tariffs-composes rule; `inst-sy-surface`) **Implementation status (2026-08-07, Slice 11 strand): tier 2 has no store.** D-76/D-81's reference-set lookup reads `pricing_historical_price`, which **does not exist** — it is Slice 5's `inst-bd-store` and is unbuilt. Tier 2 therefore always resolves empty and synthesis falls to clause (3), **fail closed**, which is the correct behaviour rather than a defect. Tier 1 is buildable today; write the seam so the day the store lands changes no logic. Recorded normatively on D-182's precedent — an absent counterpart stated once beats it being rediscovered by each strand. |
...
presented as transcription (the treatment `m20260802_000024`'s `uq_pricing_bundle_plan`
...
**selection tier** `source` (`live_history | historical_import`, D-76), and the
...
`migrated-origin` line has no `CatalogVersion` to fetch them from, and a tier-2 key may have no
...
D-76) — with the full provenance record, including the selection tier per resolved id **and
...
- [ ] Tier 2 end-to-end (D-81 + D-87): an **open-ended** reference row imported for a fully-legacy key (no `pricing_price` rows) — including a **graduated** one carrying `tier_bands` — covers a `migration`-trigger `t`; the synthesized snapshot freezes the **materialized payload** (`source = historical_import`) and rating charges — and Billing posts, from the payload's plan-level descriptor set (C-5) — from that payload alone, resolving no id through the read model or any `CatalogVersion`; the same key with a **closed, strictly-past** reference interval and a later `t` fails closed into the exception list
