...
**Steps**:
1. [ ] - `p1` - `published → retired` blocks **new** subscriptions only; in-flight subscribers keep resolving their frozen snapshots and active windows until renewal/migration. Retirement, `available_to`, and an assumed zero subscriber count are **not** trailing-void exemptions and write **no** implicit coverage (D-374 / D-182). The abandoned open draft's window intentions are retained for audit and excluded from Working/Committed evaluation (`inst-pl-abandon`) - `inst-re-block`
...
2. [ ] - `p1` - The catalog emits the schedule; **Subscriptions** creates effective-dated `PlanLink`s and executes — the catalog never mutates a subscription and never touches a posted invoice (M1). Whether a migration charges the target plan's `one_time` lines is decided by the **phase-entry occurrence** contract, not by a lifetime rule (2026-09-20, supersedes the once-per-subscription-lifetime `inst-cs-setup-timing` reading): a one-time line charges once per *actual entry* into the phase that carries it, Subscriptions mints a durable `phase_entry_id` per such entry, and a migration that enters the target's phase is an entry like any other — retries of that entry charge nothing more, and a migration that lands mid-phase without a new entry charges nothing (`06-consumer-contracts.md`), and a migrated subscription enters the target's **first non-trial phase** (D-39 — a migration never grants a new `trial`; entering an `interim` phase (`intro` until D-358) is allowed; the entry-phase rule rides the `PlanMigrationScheduled` contract) - `inst-mg-boundary`
...
**plus the plan-level `billing { itemizationRule, ext }` and resolved grant set**, with
`invoiceLineTemplate`, `glCode`, `taxCategory`, `billingTiming` on each row (**D-373**;
`descriptorSetUnavailable` is read per row/plan half, per `inst-sy-payload`) (C-5, 2026-08-01: a
...
plan-level `billing { itemizationRule, ext }` and resolved grant set, with the four row
descriptor elements `invoiceLineTemplate`, `glCode`, `taxCategory`, `billingTiming`
(`inst-sy-payload`, D-87 + C-5, **D-373**; availability evaluated per half)**, served on the
...
what that revision exposes to the contract's subscriptions (priced `(skuId, dimensionKey)` lines,
phase chain, required add-ons, row descriptor elements and the itemization rule, plus
plan `descriptor_ext` — **D-373**) and the plan **MUST NOT** be retired
...
- [ ] Contract lock scope (D-108): a **new revision** of a contract-locked plan that drops a priced `(skuId, dimensionKey)` line, re-shapes the phase chain, removes a required add-on or strips row descriptor elements, the itemization rule or plan `descriptor_ext` (**D-373**) → 409 `CONTRACT_LOCKED` with the contracts enumerated, and retiring the plan → 409; a **supersession / repricing run / cutover** on the same plan's keys **succeeds** (the negotiated rate is Contracts-owned) and the locked subscription's in-flight period still resolves its frozen snapshot; after lock expiry the structural change publishes
