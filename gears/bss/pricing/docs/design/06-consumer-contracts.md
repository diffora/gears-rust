...
4. [ ] - `p1` - **A missing usage tariff is an error.** When the selected phase and market hold no usage price for a metered line, the consumer answers `USAGE_PRICE_MISSING` — never an implicit free tariff, never the previous phase's price, never another market's through FX. *The currency's `global` price is not another market's* (D-379): it is the price the plan authored for every region, and step 4a is how a consumer reaches it. Free usage is an **explicit zero** price, which accepts and accumulates usage like any other. Publish already refuses the catalog side of this (`LINE_MARKET_PRICE_MISSING`), so the runtime code is the defence for a pinned version that predates a market - `inst-uc-missing`
4a. [ ] - `p1` - **Which market price a buyer in a region is sold (normative, D-379, 2026-09-20 — joint with Subscriptions and Rating).** For `(overlay O, currency C, region R)` at instant `t` a consumer resolves, per charge line, the covering window on `(O, C, R)`, else `(O, C, global)`, else `(base, C, R)`, else `(base, C, global)`, else refuses — no FX, no other currency, never another region's row. `R` is the **buyer's commercial territory**; where a resource runs is a `dimension_key` and a separate line. **An override is a whole row**: nothing of the `global` row is inherited, so there is no field-level effective value to compute. **Resolve once, at binding, and pin the `priceId`**: an override published later is a new `CatalogVersion` and never re-prices a bound subscription — the semantics grandfathering already has — and Billing never re-resolves the fallback against the mutable region taxonomy, the rule S4 already states for the tax-category fallback. An override whose window ends hands *new* buyers to `global` from that instant; the coverage report discloses it (S7). Eligibility ranks **after** region: each line (a charge in one eligibility class) resolves its own region, then W3's most-specific-wins ranks the resolved lines - `inst-uc-market-resolution`
