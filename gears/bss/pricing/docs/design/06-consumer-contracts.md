...
**Steps**:
1. [ ] - `p1` - Stable `{skuId, planId, priceId}` exposed on all downstream artifacts; ids never re-used across revisions (append-only rows guarantee this structurally, Foundation §4.3). **The frozen row also names where it sits in the normalized graph (2026-09-20):** `chargeLineId` (the stable logical line), `lineVersionId` (the **immutable** structure version the money is priced against) and `marketPriceId` (the stable currency/region variant), beside `priceId` and the full ten-axis `scopeKey`; and every frozen window interval carries `windowId` and the `priceId` it schedules. A consumer holding a pinned selection therefore never follows a draft line or a mutable "latest version of the line" — two versions of identical content render identical rows, and one market legitimately holds a `superseded` predecessor beside its `published` successor, whose windows are one coverage run that only the interval's `priceId` tells apart. A pinned read is byte-stable across later publications: the pinned version's delta is INSERT-only, and a later revision freezes into its own version - `inst-rc-ids`
...
4. [ ] - `p1` - **A missing usage tariff is an error.** When the selected phase and market hold no usage price for a metered line, the consumer answers `USAGE_PRICE_MISSING` — never an implicit free tariff, never the previous phase's price, never another market's through FX. *The currency-wide price is not another market's* (D-379, re-filed by D-381): it is the price the plan authored for every region — the row that states **no** region — and step 4a is how a consumer reaches it. Free usage is an **explicit zero** price, which accepts and accumulates usage like any other. Publish already refuses the catalog side of this (`LINE_MARKET_PRICE_MISSING`), so the runtime code is the defence for a pinned version that predates a market - `inst-uc-missing`
4a. [ ] - `p1` - **Which market price a buyer in a region is sold (normative, D-379, 2026-09-20; re-filed by D-381, 2026-09-21 — joint with Subscriptions and Rating).** For `(overlay O, currency C, buyer region R)` at instant `t` a consumer resolves, per charge line, the covering window on `(O, C, Some(R))`, else `(O, C, None)`, else `(base, C, Some(R))`, else `(base, C, None)`, else refuses — no FX, no other currency, never another region's row. `None` is the **currency-wide market**: the row that states no region at all, which is not a taxonomy value and so has no lifecycle a tenant could withdraw it through. **A buyer may have no region**, and such a buyer tries the two `None` steps only. `R` is the **buyer's commercial territory**; where a resource runs is a `dimension_key` and a separate line. **An override is a whole row**: nothing of the currency-wide row is inherited, so there is no field-level effective value to compute. **Resolve once, at binding, and pin the `priceId`**: an override published later is a new `CatalogVersion` and never re-prices a bound subscription — the semantics grandfathering already has — and Billing never re-resolves the fallback against the mutable region taxonomy, the rule S4 already states for the tax-category fallback. An override whose window ends hands *new* buyers to the currency-wide price from that instant; the coverage report discloses it (S7). Eligibility ranks **after** region: each line (a charge in one eligibility class) resolves its own region, then W3's most-specific-wins ranks the resolved lines. **Owed jointly:** the SUB-D and T-D counterparts carry the `global` filing and are amended with their owners - `inst-uc-market-resolution`
4b. [ ] - `p1` - **The frozen price payload carries its own ladder (D-378).** A tiered price's `bands` are that market's whole ladder — bounds beside rates — and two markets of one line may differ in all of it; a consumer reads a market's ladder off its own price and **never** joins a line's geometry to a market's rates. What stays the line's is `modelKind`, `packageSize` and the evaluation policy - `inst-uc-own-ladder`
...
| `usage_counter_on_plan_change` | `enum` | `reset (default) \| carry` — the snapshot-frozen plan-change tier-`Q` continuity flag Rating consumes from the **target** plan's snapshot (D-113); `carry` honoured per shared `(skuId, dimensionKey)` line only under the D-82/D-98/D-122 unit-field match, else that line resets; absence = `reset`. Revision-scoped like every plan column |
...
shared `(skuId, dimensionKey)` line (mismatch ⇒ reset, never a cross-denomination carry).
