...
| W2 | Non-overlap key | Window non-overlap is per **canonical scope key**, which after the charge-line split is the **full market key** `(ChargeLineScopeKey, currency, region)` (ADRs `cpt-cf-bss-pricing-adr-canonical-scope-key`, `…-grandfathering-cohort-axis`); a grandfathered generation + successor — and any number of prior generations (distinct `cohort`s) — at the same `t` are distinct keys, never an overlap violation | PRD §2.2 |
...
| W6 | **"The longest billing cycle sold on the key"** (normative, 2026-08-01 review fix, C-2) | The phrase carries three margins — the D-80 coverage horizon (`inst-sg-surface`), the D-04 copy-window bound (`inst-co-bounds`) and the trailing-void floor (`inst-fg-trailing`) — but `usage` and `one_time` keys carry no `frequency` at all, so read literally it was undefined on most keys of a hybrid. It is therefore defined **per plan, not per key**: the longest `frequency` among the plan's **recurring** rows on the key's `(currency, region)`, matching D-121's `H` ("2 × the longest cycle sold on the plan"). On a plan with **no** recurring part (pure one-time or pure usage-with-no-subscription-cycle) the margin is **zero** — a one-time purchase needs no forward coverage — and the horizon predicate reduces to "an active window covers `t`" | D-04, D-80, D-121 |
| W7 | **Independent monetary windows (normative, 2026-09-20)** | A window schedules **one monetary version** of **one market**. Its conflict scope is the row's full market key, so EUR/eu and USD/us — and USD/us beside USD/ca — schedule over the same span without colliding, while two monetary versions of the *same* market at the same `t` are `WINDOW_OVERLAP` however distinct their `price_id`, `market_price_id` or `line_version_id`. Coverage is likewise required and reported **per market**: a covered sibling never excuses a hole. Neither a new monetary version nor a new shared structure version widens or re-scopes an existing window, and no window is created implicitly — a priced market with no authored intention is *uncovered*, which is the coverage rule's answer, not a default | D-374; ADR `cpt-cf-bss-pricing-adr-canonical-scope-key` |
...
2. [ ] - `p1` - Distinct keys hold windows independently: a hybrid's `recurring`/`usage`/`one_time` components and a grandfathered row + successor each carry their own coverage (W2) - `inst-wc-perkey`
...
**Steps**:
1. [ ] - `p1` - Sweep the boundary instants the binding set implies inside the required coverage horizon (each binding's start and end, clipped). At every boundary at which **any** market of the line is bound, every bound market MUST name the **same** `lineVersionId`; two of them naming different ones is `STRUCTURE_CUTOVER_MISMATCH` (422), naming the line, the boundary, and each market's version. Money remains free: two markets whose monetary versions or amounts change on different days are legal exactly as long as the structure they are priced against does not differ. **A tier ladder is money (D-378):** re-tiering a market — another number of bands, other break-points, other rates — is a monetary successor bound to the line's existing `lineVersionId`, so it is **not** a cutover and this rule never fires on it. **The currency-wide market is a market like any other here (D-379; re-filed by D-381):** a region's override and the currency-wide price behind it are two markets of one line and are bound to one structure at every instant — the fallback makes that matter more, since a buyer crosses from one row to the other with no act. A market bound to two versions at one instant is the existing `WINDOW_OVERLAP` - `inst-sc-simultaneous`
...
**Steps**:
0. [ ] - `p1` - **Which key a purchase binds (normative, D-379, 2026-09-20; re-filed by D-381, 2026-09-21):** on `(currency, region)` each line resolves its own market — the region's own key where a window covers `t`, else the currency-wide key, the one that states no region — and W3's most-specific-wins then ranks the resolved lines of one sale; a line **no** step covers is still a gate input and fails predicate (1) on the first key that exists, never dropped (a dropped line is a market whose line the conjunction never looked at). **A buyer with no region at all** binds the currency-wide keys and nothing else — `region` is optional on the sellability read, and a plan priced regionally alone sells such a buyer nothing. An override that ends inside the D-80 horizon still sells where the currency-wide key covers on, judged over the two together, and its surface entry names that key (`falls_back_to`); W6's longest cycle counts the currency-wide recurring row as sold in every region, or a region served by it would get a zero margin and skip the horizon. One implementation, `domain::market_resolution`, shared with preview, completeness and the trailing-void bound - `inst-sg-market-resolution`
...
2b. [ ] - `p1` - **Bundle conjunction:** for a `bundle`-type plan the gate is the **conjunction** — every referenced component key passes predicates (1)–(5) at `t`, **and predicate (6) holds over the bundle's whole frozen sale set** — the bundle SKU, every member plan's own SKU and every component those plans price (**D-376**: the exemption D-46 gave components is withdrawn — `sellable = false` closes new sales of everything that includes the SKU) (and, for `own_price` bundles, the bundle's own rows too), plus the bundle's own `availableFrom`/`availableTo`; the `SellabilitySurface` exposes the frozen component key set for this walk — the set spans `priceEligibility = all_subscriptions` (`cohort = none`) keys **only**, grandfathered generations are never gate inputs (2026-07-28 review fix, confirmed 2026-07-31) — (composition rules normative in Slice 8, `inst-bc-sellability`) - `inst-sg-bundle`
3. [ ] - `p1` - Predicates (1)–(5) are point-in-time evaluable from the pinned read model (no live catalog query at order time beyond the version-pinned read). **Predicate (6) is the exception, and deliberately (D-376, 2026-09-20):** its *operand set* — `saleSkuIds`, every SKU the sale includes: the plan's own, each row's component, each bundle member's and each component of a member plan — is **frozen with the pricing terms** and never supplied by the caller, but its *answer* is the registry's **current published** state (`published`, not deprecated, `sellable = true`), read at evaluation. A tenant who closes sales expects sales to stop, so the flag does **not** ride the pinned `CatalogVersion`; a flip takes effect once the SKU is published and never rewrites an existing subscription. `at` still selects the *pricing* instant, and the surface reports separately when the registry was read (`registry_checked_at`) — it makes no claim about permission at `at`. No exemption for a component, a zero price or a phase not yet entered; an empty set is `not_evaluable`, not vacuously true; every closed SKU is named; an unreadable registry is `not_evaluable`, which a consumer refuses — a registry outage stops new sales, accepted by the owner. **This is a claim about the finished gear, and which predicates a version can answer follows the slices that land their facts (D-167, 2026-08-03, found while building the read side):** predicate (1) needs the `PriceWindow` intervals, states and derived coverage end this slice owns (D-99/D-121); predicate (5) needs Slice 4's GA-gate flags and Slice 10's prepaid-execution gate; predicate (6) needs the registry `sellable` flag (D-46) and therefore the registry gear. A version projected before those exist answers (2) committed-`CatalogVersion` addressability, (3) `availableFrom`/`availableTo` and (4) the plan lifecycle state, and answers **three of six** — which is stated here rather than discovered by the first consumer, because the difference between "this predicate is false" and "this version cannot evaluate this predicate" is the difference between a plan that is not sellable and a gate that is not yet a gate. Foundation §4.4 carries the same statement against the delta payload - `inst-sg-pinned`
...
4a. [ ] - `p1` - **Predecessor flip (normative, D-100, 2026-07-31 review fix):** the successor is a new `all_subscriptions` row on the **same** canonical scope key as the predecessor — only the grandfathered copy moves to a new key (`priceEligibility` + `cohort`) — and the Foundation admits at most **one** published row per key (§3.7 partial `UNIQUE … WHERE lifecycle_state = 'published'`). The cutover commit therefore flips the predecessor **`published → superseded`** inside the same ACID transaction, exactly as a supersession commit does, before inserting the successor. Without the flip the commit inserts a second published row on an occupied key and the whole atomic unit dies on the unique index at commit — **true until 2026-09-19, when the charge-lines plan removed that index on purpose (D-195 amendment): the flip still goes first, but the commit no longer dies without it, so this reason is history and the ordering is now the commit method's property** — while [`03-price-structure.md`](./03-price-structure.md) `inst-ps-supersede` and §1.7's `SupersessionOrchestrator` both called the D-88 supersession unit the *only* path to this transition, so an implementer following the state machine could not produce a committable cutover at all. The cutover is the **second** sanctioned producer of the flip (Foundation §4.3 taxonomy; its trigger column whitelist already anticipated "supersession/**cutover**"); the superseded predecessor stays live-resolvable until the changeover through its shortened window, per the standard flip-at-commit semantics - `inst-co-supersede`
...
| `price_id` | `uuid` | FK `pricing_price` — binds the window to one **monetary version**; that row's `market_price_id` resolves the window's conflict scope (the logical line's axes plus `currency`/`region`) and its `line_version_id` the immutable structure the money is priced against, so neither is re-read as "the latest". Immutable after creation |
...
**Touches**:
- API: `GET /bss-pricing/v1/plans/{planId}/coverage` — each key's entry carries **`falls_back_to {scope_key, from}`** in the `working` and `committed` views alike (D-379; re-filed by D-381): set on a region's override whose own coverage ends (or that has none) where the line carries a currency-wide price, naming that key — whose region segment is the absent-axis token — and the instant its buyers start paying it; `null` on a currency-wide key, on an override that never ends, and on a region's price with nothing behind it, whose ending is the trailing void it always was. The fallback is legal; its invisibility would not be
...
the GA-gate flags: `not_sellable_ga` / prepaid-execution) point-in-time evaluable, plus the
registry's **current** sale permission over the frozen `saleSkuIds` — every SKU the sale includes,
no component exemption, D-376 —
...
evaluates the **conjunction** over it on predicates (1)–(5), and predicate (6) over the bundle's
whole frozen sale set, components included (D-376) (own rows too for `own_price`; Slice 8).
...
- [ ] Bundle conjunction: one unsellable component key (any predicate of (1)–(5) failing) → the bundle is not sellable; a `sellable = false` SKU **anywhere in the frozen sale set** blocks — the bundle SKU, a member plan's SKU, or a component a member plan prices (D-376; every closed SKU is named) — and re-opening and publishing it re-opens the sale with no pricing re-publish; an `own_price` bundle additionally requires its own rows sellable
