...
**Steps**:
0. [ ] - `p1` - **Which key a purchase binds (normative, D-379, 2026-09-20):** on `(currency, region)` each line resolves its own market — the region's own key where a window covers `t`, else the currency's `global` key — and W3's most-specific-wins then ranks the resolved lines of one sale; a line **no** step covers is still a gate input and fails predicate (1) on the first key that exists, never dropped (a dropped line is a market whose line the conjunction never looked at). An override that ends inside the D-80 horizon still sells where the `global` key covers on, judged over the two together, and its surface entry names that key (`falls_back_to`); W6's longest cycle counts the `global` recurring row as sold in every region, or a region served by it would get a zero margin and skip the horizon. One implementation, `domain::market_resolution`, shared with preview, completeness and the trailing-void bound - `inst-sg-market-resolution`
...
**Touches**:
- API: `GET /bss-pricing/v1/plans/{planId}/coverage` — each key's entry carries **`falls_back_to {scope_key, from}`** in the `working` and `committed` views alike (D-379): set on a region's override whose own coverage ends (or that has none) where the line carries a `global` price, naming that key and the instant its buyers start paying it; `null` on a `global` key, on an override that never ends, and on a region's price with nothing behind it, whose ending is the trailing void it always was. The fallback is legal; its invisibility would not be
