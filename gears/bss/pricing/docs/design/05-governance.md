...
| `POST /v1/pricing/plans/{id}/cutovers`, `POST /v1/pricing/plans/{id}/supersessions` (D-88), `PATCH /v1/pricing/prices/{id}/grandfather-until` (S7) | `plan × write` (+ material approval per the standard price-delta evaluation) |
...
| `POST /v1/pricing/migrations/{id}:start` / `:complete` (S11 — the Subscriptions execution handshake, D-65, 2026-07-29) | `plan × migrate` — called by the Subscriptions **service identity**, not a human role (the service-to-service row below grants only `plan × read`, so this lane is granted explicitly) |
...
| `POST` | `/v1/pricing/approvals/{id}/withdraw` | Submitter/CatalogAdmin voids a pending record without mutating the subject (audited; frees the pinned scope key — 2026-07-31 review fix) | per decision |
...
`unit_price_minor` vector on `graduated`/`volume` iff geometry unchanged;
...
`PlanTier` override, `invoiceGroupingKey`)) **MUST**
...
- [ ] Delta domain (D-115): with a threshold configured, a supersession changing only `manual_quantity` (10 → 1000), only a band bound (`[0,1000)` → `[0,10)`, unit prices identical), or only `billingTiming` blocks as material; a plan revision changing only its GL code or a phase duration blocks as material; a band-kind row's band-wise `unit_price_minor` change below threshold on unchanged geometry auto-publishes; a percent-only policy over a `$0` baseline row evaluates material
