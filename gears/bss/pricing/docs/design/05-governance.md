...
2. [ ] - `p1` - Validate zero downstream billable effect: imported rows are reference/history only — no window scheduling, no event that warms a rateable state for past periods. The `BACKDATE_SIDE_EFFECT` predicate is enumerated: an imported row whose effective/`availableFrom` dates reach `now` or later, or whose effective range intersects a not-yet-closed billing period, is rejected (422); for strictly-past, closed-period rows the no-effect guarantee holds by construction (2026-07-28 review fix, flagged for veto) - `inst-bd-noeffect`
2a. [ ] - `p1` - **Row-shape pipeline (D-13):** every imported row runs the fail-closed pipeline's row-shape subset — taxonomy membership, ISO 4217 minor-unit precision, scope-key uniqueness (incl. vs other imported rows), modelKind/evaluation-policy shape, **and the Slice-6 consumer-contract field presence on recurring rows** (`billingAnchorPolicy`/`prorationBasis`/`creditOnDowngrade`/`billingTiming` — reference rows shape `migrated-origin` snapshots consumers compute from, so the S6 presence promise must hold for them too; 2026-07-28 review fix, flagged for veto); window-coverage, sellability and `CatalogVersion`-addressability checks do **not** apply (reference rows are resolved only via snapshot-synthesis provenance, never via window resolution). An import can never create a row regular authoring would reject - `inst-bd-pipeline`
...
| `gts.cf.bss.pricing.historical_import.v1~` | Governed backdated reference import — its OWN resource so the restricted `BackdateGrant` is targetable without any other authority | `write`, `read` (the imported row set, so the second person can review what they approve — 2026-07-29 review fix) |
...
| `POST /v1/pricing/migrations/{id}:start` / `:complete` (S11 — the Subscriptions execution handshake, 2026-07-29) | `plan × migrate` — called by the Subscriptions **service identity**, not a human role (the service-to-service row below grants only `plan × read`, so this lane is granted explicitly) |
...
**Reviewability invariant (normative, 2026-07-29 review fix)** — for **every** registered
...
the matching `read` grant here — and `GET /v1/pricing/approvals/{id}` MUST return the **pinned
...
re-resolutions / bulk group moves, GA-gate-clearing re-publishes,
**threshold-policy mutations themselves — D-10**) **MUST**
