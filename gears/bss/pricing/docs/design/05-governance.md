...
5. [ ] - `p1` - The five rules above decide **materiality**; the tenant's `N` decides **how many approvers** a material change needs. At `N = 0` a material change is authorized without an approver and its record is born satisfied — the rules still run, the verdict and its reason are still recorded, and the audit row is still written. Materiality is not switched off, and a reader who thinks it is will look for the verdict in the wrong place (D-380) - `inst-mat-quorum`
...
- [ ] A tenant at `N = 0` completes a first publish, a new market row, a bundle composition, an overlay mutation, a retirement, a cutover and a threshold-policy change **with no approver**; each still writes a satisfied record carrying its verdict, its reason and `quorumReduced`. A tenant at the default is refused every one of them without a second principal. The list is every act, not publish alone: seven of the gear's twelve materiality sites opened a unit unconditionally before D-380, so an AC naming only publish would pass against a gear that reached none of them.
- [ ] A tenant at the default **cannot reach `N = 0` alone**: the proposal answers 202 and the policy in force is unchanged until an independent principal approves it. A tenant with no policy at all is at the G1 fail-safe, so its first `PUT` is material too — there is no one-person path from the shipped state.
