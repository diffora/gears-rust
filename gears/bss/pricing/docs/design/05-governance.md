...
| G6 | Approver count | The tenant's `N` — approvers **beside** the author, each distinct from the author and from each other — is a value of the approval-threshold **policy version**: **default 1, floor 0** (D-380; products' twin P-D-177). `N = 0` means the tenant publishes without an approver **under every rule of §3**, not only the threshold one. Not configurable: `N` is reached only by explicit configuration (absent policy ⇒ default), self-approval stays refused at every `N >= 1`, and a change to `N` is itself material under the then-current `N` (D-10) | D-380 |
...
**The version carries `approver_count` (D-380, 2026-09-21)** — stored on every entry row **and** on the tombstone, derived per version with disagreement refused as a corrupt row: the same shape `effective_from` already has here, for the same reason. A tombstone is a version, and a version with no `N` would make the type partial.
...
`credit_on_downgrade`, tax fields, `quantity_source`, `invoice_line_template`, `gl_code_ref`)
and plan-shape revision content (`descriptor_ext`, phase graph/durations, add-on rules,
cycle/frequency, availability dates, `PlanTier` override, composite meter definitions, and the
plan-level period floor/cap — **D-319**)) **MUST**
route through the same evaluator (**D-373**: row descriptor overrides and plan extensions
replace the set; the grouping key is removed, and bundle itemization remains material); grant-price changes evaluate as ordinary price deltas,
...
- [ ] A tenant at `N = 0` completes a first publish, a new market row, a bundle composition, an overlay mutation, a retirement, a cutover and a threshold-policy change **with no approver**; each writes **no approval record** — see `inst-mat-quorum` for why this gear cannot — and each appends its audit row carrying the act. A tenant at the default is refused every one of them without a second principal. The list is every act, not publish alone: seven of the gear's twelve materiality sites opened a unit unconditionally before D-380, so an AC naming only publish would pass against a gear that reached none of them.
- [ ] A tenant at the default **cannot reach `N = 0` alone**: the proposal answers 202 and the policy in force is unchanged until an independent principal approves it. A tenant with no policy at all is at the G1 fail-safe, so its first `PUT` is material too — there is no one-person path from the shipped state. What makes this hold rather than merely intend to is the **direction** of the effective-version walk (D-188 as D-380 amends it): the count read for a version with no approved unit is the one standing **beneath** it, so a proposal cannot authorize itself by declaring the quorum it asks for.
