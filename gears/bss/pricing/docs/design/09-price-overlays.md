...
| L8 | Money only, per market (2026-09-20) | An overlay line adjusts **money** — a percent or an amount set — and an amount set is stated per currency, so the adjustment stays market-specific after the charge-line split. It **cannot** reach the shared half of a line: model kind, tier geometry, package size and the usage policy live on the immutable charge-line version every market of the line is priced against, and an overlay has no member that names them. A discounted ladder is the base ladder's bounds with adjusted rates, never a second geometry | Charge-line split; `07-pricewindow-linkage.md` W7 |
| L9 | An overlay outranks a region | A price is resolved for `(overlay O, currency C, buyer region R)` as the covering window on `(O, C, Some(R))`, else `(O, C, None)`, else `(base, C, Some(R))`, else `(base, C, None)`, else refused, where `None` is the currency-wide market — the row that states no region (**D-379**, owner ruling 4: a partner price list is commercially more specific than a territory; re-filed by **D-381**). **A consumer contract, not a branch this gear reaches:** by L1 every price row authored here is `base`, and an overlay's amounts are keyed on currency alone, so no price key exists on which the first two steps can be built. `domain::market_resolution` iterates the overlay as its outer loop so that the day the axis gains a value there is one reading of this order, and Tariffs — which composes overlays onto the resolved base row — owns applying it | D-379 |
