...
| `adjustment_value` | `int` | **basis points; `magnitude_kind = percent_bp` lines only** (NULL otherwise) — amount-based magnitudes live in `pricing_price_overlay_line_amount` (D-08) |

Key constraints: `UNIQUE (price_overlay_id, plan_id, target_sku)` (null-safe — one default line,
one line per plan, one per `(plan, sku)`); `CHECK ((magnitude_kind = 'percent_bp') =
