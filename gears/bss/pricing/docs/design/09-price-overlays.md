...
| L9 | An overlay outranks a region | A price is resolved for `(overlay O, currency C, region R)` as the covering window on `(O, C, R)`, else `(O, C, global)`, else `(base, C, R)`, else `(base, C, global)`, else refused (**D-379**, owner ruling 4: a partner price list is commercially more specific than a territory). **A consumer contract, not a branch this gear reaches:** by L1 every price row authored here is `base`, and an overlay's amounts are keyed on currency alone, so no price key exists on which the first two steps can be built. `domain::market_resolution` iterates the overlay as its outer loop so that the day the axis gains a value there is one reading of this order, and Tariffs — which composes overlays onto the resolved base row — owns applying it | D-379 |
