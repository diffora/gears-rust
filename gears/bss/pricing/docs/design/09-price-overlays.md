...
| `lifecycle_state` | `enum` | `draft \| published \| superseded`; flip-at-commit per D-90 semantics. **No `abandoned` tombstone, and the number is re-minted in consequence (D-231, 2026-08-06):** `pricing_plan` has one precisely so *"the number a discarded draft consumed is never attached to a different shape"* (D-145); an overlay's discarded draft leaves by DELETE, so the next open re-mints its number and a client holding the discarded revision's entity tag finds a different revision under the same identity. Owed: the fourth state |
