...
| `adjustment_value` | `int` | **basis points; percent lines only** — amount-based magnitudes live in `pricing_price_overlay_line_amount` (D-08) |

Key constraint: `UNIQUE (price_overlay_id, plan_id, target_sku)` (null-safe — one default line,
one line per plan, one per `(plan, sku)`); resolution order `(plan, sku)` > `(plan)` > default
