...
| `cpt-cf-account-management-fr-children-query` | Paginated children query with status filtering via OData `$filter` on the `parent_id` index. `recursive=true` widens the depth pin to `parent_id IN (SELECT descendant_id FROM tenant_closure WHERE ancestor_id = :tenant_id AND barrier = 0)` — the direct-child carve-out generalised over the visible subtree — intersects that parent set with tenants visible under the caller's original PDP-emitted scope (including any `descendant_status` restriction), and attaches per-row ancestor chains read from `tenant_closure`. |
