...
- **Design**: [DESIGN.md](../DESIGN.md) -- UsageType Catalog component (§3.2), Register / Delete UsageType sequences (§3.6), the SPI catalog-row payload shape (`plugin-spi.md` §"Domain Model" `CatalogRow`), PRD→DESIGN realization for fr-usage-type-registration / fr-usage-type-deletion / fr-counter-semantics / fr-gauge-semantics (§5.3), operational metric inventory rows for the catalog-owned `uc_usage_type_requests_total` counter and `uc_usage_types` gauge ([§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002))
...
- Plugin transport or persistence failure — propagated as a deterministic platform error envelope (HTTP `503` for `PluginUnavailable` / `TypesRegistryUnavailable` / `ServiceUnavailable` (plugin-side `Transient` and host-side per-call deadline expirations both lift to `ServiceUnavailable`); HTTP `500` for a plugin-surfaced `Internal` error (uncategorized backend failure)); no synthesized UsageType handle.
...
8. [x] - `p1` - **CATCH** plugin SPI error - `inst-register-usage-type-spi-catch`
   ...
   2. [x] - `p1` - **ELSE** (transport / availability / persistence error from the plugin) **RETURN** the propagated platform-error envelope (HTTP `503` for `PluginUnavailable` / `TypesRegistryUnavailable` / `ServiceUnavailable` (plugin-side `Transient` and host-side per-call deadline expirations both lift to `ServiceUnavailable`); HTTP `500` for a plugin-surfaced `Internal` error (uncategorized backend failure)) - `inst-register-usage-type-spi-fail`
9. [x] - `p2` - The `uc_usage_types` gauge is **not** refreshed on the create path — catalog-count observability is owned by the periodic `serve` refresher (`inst-serve-usage-types-gauge-refresh`), decoupled from the mutation path so a slow/failing plugin never touches a committed create - per the gauge's update trigger in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002)
10. [x] - `p1` - **RETURN** HTTP `201` with the canonical `UsageType` resource body (as the `UsageTypeDto` REST projection) and `Location: /usage-collector/v1/usage-types/{gts_id}` per `usage-collector-v1.yaml` (or, on the SDK path, return `Ok(UsageType)` to the trait caller) - `inst-register-usage-type-return`
11. [x] - `p2` - On every completion of this flow (each **RETURN** above, on either surface), increment `uc_usage_type_requests_total` exactly once with `operation="create"` and the completed branch's outcome pair per DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002): successful registration (`inst-register-usage-type-return`) ⇒ `outcome="success"`, `error_category="none"`; the **reserved/defensive** `missing_security_context` branch (`outcome="error"`, `error_category="missing_security_context"`) is **not emitted in v1** — an unauthenticated REST call is rejected by ToolKit gateway middleware upstream of this gateway-service emit point and the SDK trait requires `ctx: &SecurityContext`, so no attempt reaches this step without a resolved `SecurityContext`; it is reserved to satisfy the closed §3.11.5 vocabulary and enumerated uniformly across the four flows per the NFR: Operational Visibility DoD; PDP deny (`inst-register-usage-type-pdp-deny`) ⇒ `outcome="denied"`, `error_category="authz"`; rejection at the `UsageTypeGtsId::new` / `UsageKind::from_str` boundary or by the shape-validation algorithm (`inst-register-usage-type-invalid-shape`) ⇒ `outcome="error"`, `error_category="validation"`; duplicate registration (`inst-register-usage-type-duplicate`, HTTP `409`) ⇒ `outcome="error"`, `error_category="conflict"`; plugin transport / availability / persistence failure (`inst-register-usage-type-spi-fail`) ⇒ `outcome="error"`, `error_category="plugin_error"` - `inst-register-usage-type-requests-metric`
...
- Plugin transport or persistence failure — propagated as a deterministic platform error envelope (HTTP `503` for `PluginUnavailable` / `TypesRegistryUnavailable` / `ServiceUnavailable` (plugin-side `Transient` and host-side per-call deadline expirations both lift to `ServiceUnavailable`); HTTP `500` for a plugin-surfaced `Internal` error (uncategorized backend failure)).
...
3. [x] - `p1` - **CATCH** plugin SPI error - `inst-delete-usage-type-spi-catch`
   ...
   3. [x] - `p1` - **ELSE** (transport / availability / persistence error from the plugin) **RETURN** the propagated platform-error envelope (HTTP `503` for `PluginUnavailable` / `TypesRegistryUnavailable` / `ServiceUnavailable`; HTTP `500` for a plugin-surfaced `Internal` error) - `inst-delete-usage-type-spi-fail`
4. [x] - `p2` - The `uc_usage_types` gauge is **not** refreshed on the delete path — catalog-count observability is owned by the periodic `serve` refresher (`inst-serve-usage-types-gauge-refresh`), decoupled from the mutation path so a slow/failing plugin never touches a committed delete - per the gauge's update trigger in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002)
5. [x] - `p1` - On a successful delete, **RETURN** HTTP `204 No Content` (REST) or `Ok(())` (SDK trait) per `usage-collector-v1.yaml` - `inst-delete-usage-type-spi-delete-return`
6. [x] - `p2` - On every completion of this flow (each **RETURN** above, on either surface), increment `uc_usage_type_requests_total` exactly once with `operation="delete"` and the completed branch's outcome pair per DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002): successful delete (`inst-delete-usage-type-spi-delete-return`) ⇒ `outcome="success"`, `error_category="none"`; the **reserved/defensive** `missing_security_context` branch (`outcome="error"`, `error_category="missing_security_context"`) is **not emitted in v1** — an unauthenticated REST call is rejected by ToolKit gateway middleware upstream of this gateway-service emit point and the SDK trait requires `ctx: &SecurityContext`, so no attempt reaches this step without a resolved `SecurityContext`; it is reserved to satisfy the closed §3.11.5 vocabulary and enumerated uniformly across the four flows per the NFR: Operational Visibility DoD; PDP deny (`inst-delete-usage-type-pdp-authorize`) ⇒ `outcome="denied"`, `error_category="authz"`; `UsageTypeNotFound` (`inst-delete-usage-type-not-found`) ⇒ `outcome="error"`, `error_category="not_found"`; the referentially-unsafe delete rejection `UsageTypeReferenced` (`inst-delete-usage-type-referenced`, HTTP `409`) ⇒ `outcome="error"`, `error_category="referenced"`; plugin transport / availability / persistence failure (`inst-delete-usage-type-spi-fail`) ⇒ `outcome="error"`, `error_category="plugin_error"` - `inst-delete-usage-type-requests-metric`
...
7. [x] - `p2` - On every completion of this flow (each **RETURN** above, on either surface), increment `uc_usage_type_requests_total` exactly once with `operation="list"` and the completed branch's outcome pair per DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002): successful page composition (`inst-list-usage-types-return`) ⇒ `outcome="success"`, `error_category="none"`; the **reserved/defensive** `missing_security_context` branch (`outcome="error"`, `error_category="missing_security_context"`) is **not emitted in v1** — an unauthenticated REST call is rejected by ToolKit gateway middleware upstream of this gateway-service emit point and the SDK trait requires `ctx: &SecurityContext`, so no attempt reaches this step without a resolved `SecurityContext`; it is reserved to satisfy the closed §3.11.5 vocabulary and enumerated uniformly across the four flows per the NFR: Operational Visibility DoD; PDP deny (`inst-list-usage-types-pdp-deny`) ⇒ `outcome="denied"`, `error_category="authz"`; plugin transport / availability / persistence failure on the paginated catalog read ⇒ `outcome="error"`, `error_category="plugin_error"` - `inst-list-usage-types-requests-metric`
...
8. [x] - `p2` - On every completion of this flow (each **RETURN** above, on either surface), increment `uc_usage_type_requests_total` exactly once with `operation="get"` and the completed branch's outcome pair per DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002): successful lookup (`inst-get-usage-type-return`) ⇒ `outcome="success"`, `error_category="none"`; the **reserved/defensive** `missing_security_context` branch (`outcome="error"`, `error_category="missing_security_context"`) is **not emitted in v1** — an unauthenticated REST call is rejected by ToolKit gateway middleware upstream of this gateway-service emit point and the SDK trait requires `ctx: &SecurityContext`, so no attempt reaches this step without a resolved `SecurityContext`; it is reserved to satisfy the closed §3.11.5 vocabulary and enumerated uniformly across the four flows per the NFR: Operational Visibility DoD; PDP deny (`inst-get-usage-type-pdp-deny`) ⇒ `outcome="denied"`, `error_category="authz"`; `UsageTypeNotFound` (`inst-get-usage-type-not-found`) ⇒ `outcome="error"`, `error_category="not_found"`; plugin transport / availability / persistence failure on the catalog-get dispatch ⇒ `outcome="error"`, `error_category="plugin_error"` - `inst-get-usage-type-requests-metric`
...
The system **MUST** keep the read endpoints `GET /usage-collector/v1/usage-types` and `GET /usage-collector/v1/usage-types/{gts_id}` (and their SDK trait counterparts `UsageCollectorClientV1::list_usage_types` / `get_usage_type`) available whenever the bound storage plugin is available — catalog reads dispatch through `cpt-cf-usage-collector-contract-storage-plugin` against the plugin's `usage_type_catalog` table per call. When the plugin is unavailable, the read endpoints surface deterministic platform-error envelopes (HTTP `503` for `PluginUnavailable` / `TypesRegistryUnavailable` / `ServiceUnavailable` (plugin-side `Transient` and host-side per-call deadline expirations both lift to `ServiceUnavailable`); HTTP `500` for a plugin-surfaced `Internal` error (uncategorized backend failure)) per `usage-collector-v1.yaml`, and the SDK trait surface returns the matching `UsageCollectorError` variant. Plugin binding is lazy (resolved on first dispatch via the `GtsPluginSelector`), and gear init does not block on plugin readiness — the plugin's availability is observed per-call via the `uc_plugin_ready` gauge but is NOT a hard gear-readiness gate that withholds the REST router from the gateway.
...
- [x] `p2` - **ID**: `cpt-cf-usage-collector-dod-usage-type-lifecycle-nfr-operational-visibility`

The system **MUST** emit the two catalog-owned operational instruments inventoried for `cpt-cf-usage-collector-component-usage-type-catalog` in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002) (the inventory owns the catalog contract — names, label vocabularies, update triggers; this DoD owns the emit points and the implementation obligations):

- **`uc_usage_type_requests_total` counter** — incremented **exactly once per completed UsageType-lifecycle attempt** on both the REST and the SDK trait surfaces (emit points `inst-register-usage-type-requests-metric`, `inst-delete-usage-type-requests-metric`, `inst-list-usage-types-requests-metric`, `inst-get-usage-type-requests-metric`), carrying the closed `operation` / `outcome` / `error_category` value sets defined in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002) (cited, not restated here). `error_category="none"` **MUST** be emitted only when `outcome="success"`; `outcome="denied"` pairs with `error_category="authz"` (a completed PDP deny decision — PDP failures and timeouts are not lifecycle-attempt outcomes and land on the Foundation-owned `uc_pdp_failures_total`). The `missing_security_context` category is a **handler-boundary / defensive** member: an unauthenticated REST call is rejected by ToolKit gateway middleware upstream of the gateway-service emit point (so it is recorded only if / when handler-boundary emission is added), and the SDK trait requires `ctx: &SecurityContext` so it is unreachable there — it is reserved to satisfy the closed §3.11.5 vocabulary, and the four flows' metric steps enumerate it uniformly for that reason. The two distinct HTTP `409` conflicts **MUST** stay distinguishable: duplicate registration on create (`UsageTypeAlreadyExists`) ⇒ `error_category="conflict"`, and the referentially-unsafe delete rejection (`UsageTypeReferenced`) ⇒ `error_category="referenced"`.
- **`uc_usage_types` gauge** (no labels) — set to the current entry count of the plugin-owned `usage_type_catalog` table as observed by the gateway via `cpt-cf-usage-collector-contract-storage-plugin`. Refreshed by a periodic gear-lifecycle task (the gear's `serve` entry-point, 60 s interval; emit point `inst-serve-usage-types-gauge-refresh`) that **fully paginates** `list_usage_types` (following `next_cursor` to the last page) so the count is exact for catalogs larger than one page. The refresh is best-effort and bounded by `USAGE_TYPES_GAUGE_REFRESH_TIMEOUT`: a failed, timed-out, undecodable-cursor, page-capped, or unbound-plugin read leaves the gauge at its last observed value (a failed refresh is a no-op, never a reset, and a partial pagination is never published). There is **no boot seed and no mutation-triggered refresh** — the gauge is populated within one interval of the storage plugin becoming resolvable, independent of any `create_usage_type` / `delete_usage_type`. Foundation's gear-bootstrap flow `cpt-cf-usage-collector-flow-foundation-plugin-host-binding` provides the scoped `Meter` substrate at `inst-binding-meter-bootstrap`; this feature declares `uc_usage_types` on that shared meter. **Per-instance semantics**: each gear replica reports the whole-catalog count independently, so this series is aggregated across replicas with `max`/`last`, never `sum` (see DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002)).

Label cardinality **MUST** stay bounded to the enumerated vocabularies above: unbounded identifiers (`tenant_id`, UsageType `gts_id`, `request_id`, `trace_id`, idempotency keys) **MUST NOT** appear as labels on either instrument — they belong in structured logs and distributed traces per the label-cardinality rule in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002). Both instruments are declared on the gear's scoped `Meter` and pushed via OTLP per `cpt-cf-usage-collector-principle-otlp-push-emission` — no `/metrics` scrape endpoint. The PDP instruments observed on this component's authorize step (`uc_pdp_failures_total`, `uc_pdp_duration_seconds`, `uc_authz_decisions_total`, `uc_pdp_ready`) and the plugin-host instruments (`uc_plugin_ready`, `uc_plugin_accept_errors_total`, `uc_plugin_call_duration_seconds`) are owned by `cpt-cf-usage-collector-feature-foundation` and are inherited by this feature, not re-specified here. This DoD realizes the UsageType-catalog-path share of `cpt-cf-usage-collector-nfr-operational-visibility`; the NFR itself is foundation-owned per DECOMPOSITION §2.1, realized in per-component shares per §2.7.

**Checkbox convention.** The four UsageType-lifecycle flows (`…-register-usage-type`, `…-delete-usage-type`, `…-list-usage-types`, `…-get-usage-type`) carry `[ ]` **only** because the appended telemetry emit-point steps (the four `inst-*-requests-metric` counter steps and the single `inst-serve-usage-types-gauge-refresh` gauge-refresher step) are unchecked (`[ ]`) pending wiring; their functional steps remain `[x]` and are done in gear source. The functional DoDs implementing those flows (e.g. `cpt-cf-usage-collector-dod-usage-type-lifecycle-fr-usage-type-registration`, `cpt-cf-usage-collector-dod-usage-type-lifecycle-fr-usage-type-deletion`) therefore stay `[x]` — the not-yet-wired telemetry obligation is tracked solely by **this** DoD, the single `[ ]` element that reopens on the telemetry work.
...
- Telemetry (specified; **not yet wired** in gear source): `uc_usage_type_requests_total` counter, `uc_usage_types` gauge
- Entities: `UsageType`
...
| Plugin `Internal(detail)` (uncategorized backend error) | `500` | `Internal(detail)`                |
...
A plugin's uncategorized backend error is surfaced as `Internal(detail)` and lifts to the unclassified `Internal` envelope (HTTP 500) until a documented retryable-kind taxonomy is defined; once real plugins ship with one, retryable kinds will route to dedicated `#[non_exhaustive]` SDK variants.
...
Coverage totals count **DECOMPOSITION §2.2 coverage items** (not matrix rows): FR=4, NFR=1, Principle=1, Design constraint=2, Design component=1, Sequence=2, Data=1, Domain entity=1, API=4 — 17 §2.2 items, zero gaps. Each item that maps to a realizing DoD appears once in the closing matrix; the `Data: None` item and the `adr-0012` reference item are covered by reference and carry no dedicated matrix row (so the matrix has fewer rows than the 17-item total — this is coverage-by-reference, not a gap or duplicate).

`cpt-cf-usage-collector-dod-usage-type-lifecycle-nfr-operational-visibility` is additive to the §2.2 coverage set above: it carries the catalog component's share of the gear-wide `cpt-cf-usage-collector-nfr-operational-visibility` NFR — distributed to this feature by the operational metric inventory in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002), which assigns `uc_usage_type_requests_total` and `uc_usage_types` to `cpt-cf-usage-collector-component-usage-type-catalog` — and does not map to a DECOMPOSITION §2.2 catalog item, so the §2.2 coverage totals above are unchanged.
...
- [ ] `p1` - `GET /usage-collector/v1/usage-types` / `UsageCollectorClientV1::list_usage_types` and `GET /usage-collector/v1/usage-types/{gts_id}` / `UsageCollectorClientV1::get_usage_type` are served via a paginated/single-row dispatch through `cpt-cf-usage-collector-contract-storage-plugin` against the unified usage-type catalog per call; when the bound plugin is unavailable the endpoints surface deterministic 503 / 500 platform-error envelopes (`PluginUnavailable` / `TypesRegistryUnavailable` / `ServiceUnavailable` ⇒ 503; a plugin-surfaced `Internal` error ⇒ 500), `cpt-cf-usage-collector-dod-usage-type-lifecycle-api-list-usage-types`, and `cpt-cf-usage-collector-dod-usage-type-lifecycle-api-get-usage-type` — the gear does not withhold the REST router on plugin absence (plugin binding is lazy).
...
- [ ] `p2` - Every completed UsageType-lifecycle attempt on either surface (REST or SDK trait) increments `uc_usage_type_requests_total` exactly once with the completed branch's `operation` / `outcome` / `error_category` values drawn from the closed sets defined in DESIGN [§3.11.5](../DESIGN.md#3115-operational-metric-inventory-ops-design-002): `error_category="none"` appears only with `outcome="success"`, a PDP deny yields (`denied`, `authz`), the two distinct HTTP `409` conflicts stay distinguishable — a duplicate-create (`UsageTypeAlreadyExists`) yields (`error`, `conflict`) while a referenced-delete rejection (`UsageTypeReferenced`) yields (`error`, `referenced`) — and no label value outside the enumerated vocabularies (in particular no `gts_id`, `tenant_id`, `request_id`, or `trace_id`) is ever emitted (request-counter completeness and bounded cardinality).
- [ ] `p2` - The `uc_usage_types` gauge is refreshed by the gear's periodic `serve` lifecycle task (60 s) to the current entry count of the plugin-owned `usage_type_catalog` table, read via **full cursor pagination** over `cpt-cf-usage-collector-contract-storage-plugin` (exact for multi-page catalogs), bounded by `USAGE_TYPES_GAUGE_REFRESH_TIMEOUT`; a failed/timed-out/partial read leaves the prior value (no reset, no partial count), and the gauge is populated within one interval of plugin readiness with no boot seed and no mutation-triggered refresh (gauge refresh discipline). Across replicas the series is aggregated with `max`/`last`, never `sum`.
