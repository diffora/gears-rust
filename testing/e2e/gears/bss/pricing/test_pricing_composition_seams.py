...
def test_a_customer_group_overlay_cannot_be_submitted_in_this_strand(
    client, auth_headers
):
    """`customer_group` is refused at submit **by design**, and the code says why.

    `taxonomy_declares` answers `false` unconditionally for this class: the
    membership half that owns `pricing_customer_group_taxonomy` is not built in
    this strand, so there is no universe to validate a value against. Answering
    `true` would let such an overlay publish against a universe that does not
    exist, so the fail-closed reading is a refusal that names what is missing.

    Declaring the value first is what makes the case load-bearing rather than
    trivial: the taxonomy route **accepts** the value and reads it back, and the
    overlay validator still refuses — so the two halves of "declared" are not the
    same thing yet, and a client that offers this class by default offers the one
    scope that can never be raised for approval.
    """
