    ...
    async def test_shared_secret_inherited_by_child(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                ...
                json={"reference": ref, "value": "parent-shared-value", "sharing": "shared"},
            )
    ...
    async def test_tenant_secret_not_inherited_by_child(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                secrets_url,
                headers=root_headers,
                json={"reference": ref, "value": "parent-tenant-value", "sharing": "tenant"},
            )
            assert resp.status_code == 201, resp.text
    ...
    async def test_private_secret_not_inherited_by_child(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                secrets_url,
                headers=root_headers,
                json={"reference": ref, "value": "parent-private-value", "sharing": "private"},
            )
            assert resp.status_code == 201, resp.text
    ...
    async def test_resolution_is_upward_only(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                secrets_url,
                headers=l1a_headers,
                json={"reference": ref, "value": "child-shared-value", "sharing": "shared"},
            )
            assert resp.status_code == 201, resp.text
    ...
    async def test_sibling_does_not_leak(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                secrets_url,
                headers=l1a_headers,
                json={"reference": ref, "value": "l1a-shared-value", "sharing": "shared"},
            )
            assert resp.status_code == 201, resp.text
    ...
    async def test_child_shadows_parent_and_fallback_after_delete(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                secrets_url,
                headers=root_headers,
                json={"reference": ref, "value": "parent-value", "sharing": "shared"},
            )
            assert resp.status_code == 201, resp.text
            resp = await client.post(
                secrets_url,
                headers=l1a_headers,
                json={"reference": ref, "value": "child-override", "sharing": "tenant"},
            )
            assert resp.status_code == 201, resp.text
    ...
    async def test_deleting_shared_secret_revokes_child_access(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.post(
                secrets_url,
                headers=root_headers,
                json={"reference": ref, "value": "to-be-revoked", "sharing": "shared"},
            )
            resp = await client.get(f"{secrets_url}/{ref}", headers=l1a_headers)
            assert resp.status_code == 200
    ...
    async def test_private_and_tenant_coexist_private_wins_for_owner(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                ...
                json={"reference": ref, "value": "tenant-wide", "sharing": "tenant"},
            )
            assert resp.status_code == 201, resp.text
            # The private class coexists with the tenant one under the same
            # reference, so it is its own create (create-only conflicts are
            # per sharing class) — a PUT would address the not-yet-existing
            # private row and 409.
            resp = await client.post(
                secrets_url,
                headers=tenant_a_headers,
                json={"reference": ref, "value": "owner-only", "sharing": "private"},
            )
            assert resp.status_code == 201, resp.text
