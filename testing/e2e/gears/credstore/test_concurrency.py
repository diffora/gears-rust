...
``version`` = the per-generation monotonic counter). PUT and DELETE require
``If-Match`` — either ``*`` (target must exist) or ``"<id>.<version>"``.
    ...
    async def test_recreated_secret_rejects_previous_generation_validator(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.post(
                secrets_url,
                headers=tenant_a_headers,
                json={"reference": ref, "value": "gen1", "sharing": "tenant"},
            )
            resp = await client.get(f"{secrets_url}/{ref}", headers=tenant_a_headers)
            ...
            await client.post(
                secrets_url,
                headers=tenant_a_headers,
                json={"reference": ref, "value": "gen2", "sharing": "tenant"},
            )
            resp = await client.get(f"{secrets_url}/{ref}", headers=tenant_a_headers)
...
class TestConcurrentRacingRequests:
    ...
    async def test_racing_guarded_puts_exactly_one_wins(
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.post(
                secrets_url,
                headers=tenant_a_headers,
                json={"reference": ref, "value": "v0", "sharing": "tenant"},
            )
            resp = await client.get(f"{secrets_url}/{ref}", headers=tenant_a_headers)
            etag = resp.headers["etag"]
    ...
    @pytest.mark.asyncio
    async def test_racing_star_puts_never_leak_and_heal_on_rewrite(
        self, secrets_url, tenant_a_headers, unique_ref, cleanup
    ):
        """Concurrent ``If-Match: *`` PUTs are the crosswise dual-write case.
        The fence may legitimately fail closed (404) on a poisoned interleave,
        so a read afterwards is either one writer's value or 404 — never a
        value no one wrote. A settling rewrite always heals the ref back to a
        known-good value (self-heal on rewrite)."""
        ref = cleanup(tenant_a_headers, unique_ref("e2e-race"))
        n = 8
        written = {f"w-{i}" for i in range(n)} | {"w-init"}

        async with httpx.AsyncClient(timeout=10.0) as client:
            resp = await client.post(
                secrets_url,
                headers=tenant_a_headers,
                json={"reference": ref, "value": "w-init", "sharing": "tenant"},
            )
            ...
            assert all(s == 204 for s in statuses), statuses

            # By-design outcomes only: one writer's value (200) or fail-closed
            ...
            # When the fence failed closed the ref may be gone, in which case
            # PUT (never-creates) conflicts and the heal is a re-creation.
            resp = await client.put(
                f"{secrets_url}/{ref}",
                headers={**tenant_a_headers, "If-Match": "*"},
            ...
            if resp.status_code == 409:
                resp = await client.post(
                    secrets_url,
                    headers=tenant_a_headers,
                    json={"reference": ref, "value": "settled", "sharing": "tenant"},
                )
                assert resp.status_code == 201, resp.text
            else:
                assert resp.status_code == 204, resp.text
