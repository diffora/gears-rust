...
**Steps**:
1. [ ] - `p1` - API: POST /v1/pricing/plans/{planId}/cutovers — payload: the cutover instant + **per-selected-key entries** (scope-key selector → successor row ref, per key), with an optional **per-key `grandfatherUntil`** overriding an optional unit-wide default (D-28; 2026-07-28 review fix, flagged for veto) - `inst-gc-api`
