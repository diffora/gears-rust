...
**Direction**: provided by this PRD to Subscriptions — **two-way**: one inbound feedback lane (grandfathering re-bind reporting; recorded 2026-07-28, previously implicit)

**Protocol/Format**: eligible plans; migration targets + `PlanMigrationScheduled`; phase map + `convertsToPhaseId` + `displayTrialDays`; proration input contract (`billingAnchorPolicy`, `prorationBasis`, `creditOnDowngrade`); plan-change contract (`allowedChangeTargets`, `comparabilityRank`); entitlement grant set; sellability gate (all six predicates: active window, committed version, availability dates, lifecycle state, GA flags, registry `sellable` flag — standalone lines only, D-46); grandfathering eligibility surface (`priceEligibility`/`cohort`/`grandfatherUntil` + the read-time `EligibilityExpirySignal`) with the **inbound re-bind feedback**: at each renewal of a subscription bound to an expired generation, Subscriptions reports the outcome — the released generation on re-bind, still-bound otherwise — the input the `pricing.grandfather.expiry_signal_backlog` alarm consumes (Design 07) (Design).
