...
`tests/test_backtest.py` runs the invariants against it to measure how many of that tree's real,
already-known defects the checker rediscovers.

## Generation

```sh
git archive 10073c36 gears/bss/pricing/docs | tar -x -C .claude/skills/spec-check/tests/fixtures
```
...
# then: diff -r <that dir>/gears .claude/skills/spec-check/tests/fixtures/gears
```

## Why it must never be hand-edited

`tests/test_backtest.py` pins the score this fixture produces per invariant (`PINNED_P1`,
...
- **A pinned count may only move when the checker changed**, with the new number verified by
  ...
  `tests/test_backtest.py`, which names each fix that has moved a count so far.

If a change to `spec_check.invariants.{propagation,fr_coverage,closure}` makes a pinned count
move, that is a real claim about the checker's effectiveness. Verify it, then update the
constant deliberately, in the same commit, with the reasoning in the commit message. Never
edit a pin to re-baseline a failing run back to green, and never edit this fixture to protect
a pin.

These pins were originally verified against the Rust implementation at
`tools/spec-check`, removed on 2026-07-29 after the Python port reproduced all
four of its oracles byte for byte. Recovering that implementation, if a pin ever
needs re-deriving from it, means checking out `06c46d6d`.
