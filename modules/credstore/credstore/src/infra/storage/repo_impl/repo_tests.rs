...
use crate::domain::secret::model::NewSecret;
...
#[tokio::test]
async fn reap_provisioning_removes_stale_only() {
    let repo = setup().await;
    let tenant = Uuid::new_v4();
    seed_provisioning(&repo, tenant, Uuid::new_v4(), "stale", SharingMode::Tenant).await;

    // Far-future cutoff would match nothing (rows newer than now-100000s).
    let kept = repo.reap_provisioning(100_000).await.expect("reap none");
    assert_eq!(kept, 0, "fresh provisioning row survives a stale-only reap");

    // older_than 0 -> cutoff = now, so the just-inserted row is reaped.
    let reaped = repo.reap_provisioning(0).await.expect("reap stale");
    assert_eq!(reaped, 1);
}
...
async fn private_and_nonprivate_versions_are_independent() {
    ...
    repo.touch(&scope, nonprivate.id, SharingMode::Tenant, None)
        .await
        .expect("touch")
        .expect("row");
    repo.touch(&scope, nonprivate.id, SharingMode::Tenant, None)
