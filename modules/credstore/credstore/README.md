...
Stateful credential-storage gateway module. Owns per-secret metadata in its own
...
> (stateful gateway, `credstore_secrets` table, PDP-scope authz, versioning/ETag,
...
- **Local metadata** — a gateway-owned `credstore_secrets` table (SecureORM /
  sea-orm, migration `m0001`) holding sharing, owner, status, and `version`
...
- **Versioning** — strong `ETag` on `GET`, `If-Match` optimistic concurrency on `PUT`/`DELETE`
...
`cyberware-static-credstore-plugin`, or an OpenBao-backed plugin).
...
The module requires a `database:` section (it is stateful). Module config:
