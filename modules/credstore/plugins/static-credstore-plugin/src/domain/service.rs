...
///
/// The `private`/`tenant` classes mirror the two classes the gateway selects
...
/// tenant key class. Sharing, hierarchy and policy live in the gateway, not
/// here.
///
/// The store is seeded from configuration and stays mutable at runtime, so the
/// stateful gateway's write saga (`put`/`delete`) can use it as a development
...
impl Service {
    ...
    pub fn put_value(
        ...
        match owner_id {
            Some(owner) => {
                store.private.insert((*tenant_id, *owner, key.clone()), value);
            }
            None => {
    ...
    ///
    /// For the tenant class the `shared` then global fallbacks are also swept,
    /// so a delete clears whichever map served the matching read.
    pub fn delete_value(&self, tenant_id: &TenantId, key: &SecretRef, owner_id: Option<&OwnerId>) {
        ...
        if let Some(owner) = owner_id {
            ...
            if store.tenant.remove(&map_key).is_none() && store.shared.remove(&map_key).is_none() {
                store.global.remove(key);
            }
        }
    }
}
